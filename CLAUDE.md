...
- **로컬 Fact-Lock 번들(CI에 가까운 순서)**: 저장소 루트에서 `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run_fact_lock_bundle.ps1`. 기본에 CI `dual-regime-integrity.yml`과 동일 **Two-track + Multi-symbol**(pytest 6, 생략 `-SkipTwoTrackSubmissionAndMultiSymbolSmoke`)·**Aramaic**(pytest 27, 생략 `-SkipAramaicBtrackGraphPipelineSmoke`)·**STT audit + 쇼룸 Trust viz thin slice**(pytest 3, **3d3a**; 생략 `-SkipSttRoutingAuditShowroomTrustSlice`). 절차·맥락: `docs/final/MULTI_LENS_INTERMEDIATE_LAYER_WORKLIST.md` 하단 “한 번에 돌리는 명령”.
...
- **Control-Integrity Golden/LoRA eval (B-track, 번들 포함):** 동 번들에 pytest 스모크가 **기본 포함**; 로컬에서만 생략 시 `-SkipMkmControlIntegritySmoke`. 헬스 체인만: `scripts/run_workspace_automation_health.ps1 -IncludeMkmControlIntegritySmoke`(단축 `-MkmControlIntegritySmokeOnly`). **GPU 인접 권장 번들(oracle + Pack 0-B):** `scripts/Run-MkmGpuRecommendedBundle_v1.ps1` · 헬스 `-IncludeMkmGpuRecommendedBundle` · 단축 `-MkmGpuRecommendedBundleOnly` · 페르소나 `Invoke-MkmPersonaHealth_v1.ps1 -Persona GpuRecommendedBundle` · 주간 예약 `Register-MkmGpuRecommendedBundleWeeklyTask.ps1`. 표: `docs/final/CONSTITUTION_INFERENCE_IMPLEMENTATION_FACTS.md` §1.2.1.
...
- **사상 PR 미러 `_pr_sasang_promotion` (헬스 선택):** `run_workspace_automation_health.ps1 -IncludePrSasangPromotionMirrorSync`(전체 헬스 중 P0 직후) 또는 `-PrSasangPromotionMirrorSyncOnly`(P0+미러만). 단독: `scripts/Sync-PrSasangPromotionMirror_v1.ps1`.
...
- **보조 번들(Prophecy 정렬)**: `projects/bitcoin-trading/ops/v2/tasks/run_prophecy_alignment_pytest.ps1`는 해당 디렉터리에서 실행. 워크스페이스 테스트 목록을 CI `dual-regime-integrity`와 맞출 때는 `CONSTITUTION_INFERENCE_IMPLEMENTATION_FACTS.md` §6 표·**CI tail 포인터**·`dual-regime-integrity.yml` 순서를 확인한다.
...
- **환자 통합 번들 v1 (임상·렌즈 격리, 대외 카피 정합):** `CONSTITUTION_INFERENCE_IMPLEMENTATION_FACTS.md` §9 · `scripts/Invoke-PatientCareBundleAssemblePatientFacing_v1.ps1` · `docs/final/PUBLIC_FACING_SECURITY_AND_IP_COPY_CHECKLIST_V1.md` §3 (Silver Tech + engineering cross-check).
