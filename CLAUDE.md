...
- **Git / SSH 재발 방지:** `scripts/Verify-GitWorkspaceSanity.ps1`(`.git/info/exclude`의 `tools/*`·`scripts/*` 무방지 차단, `origin`/upstream 알림) 또는 `scripts/run_workspace_automation_health.ps1 -IncludeGitSanity`(실패 게이트: `-StrictGitSanity`). SSH 터미널 cwd: `docs/final/SSH_CURSOR_JEMA12_DEPLOY_RUNBOOK.md` §2.0.
