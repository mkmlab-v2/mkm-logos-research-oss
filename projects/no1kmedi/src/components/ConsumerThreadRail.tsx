...
export function ConsumerThreadRail({ threads, activeId, onSelect, onDelete }: ConsumerThreadRailProps) {
  const sorted = [...threads].sort((a, b) => b.updatedAt - a.updatedAt);

  return (
    <div className="thread-rail" role="navigation" aria-label="대화 기록">
        ...
        {sorted.map((t) => (
          <li key={t.id} className={`thread-rail-item${t.id === activeId ? " is-active" : ""}`}>
            <button type="button" className="thread-rail-select" onClick={() => onSelect(t.id)} title={t.title}>
              ...
              <span className="thread-rail-meta">{new Date(t.updatedAt).toLocaleString("ko-KR", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
            </button>
            <button
              type="button"
