...
const PUBLIC_LEAK_RE =
  /lemma:gnosis:|shared_lemma=|Lemma\s*연결\s*이웃\s*구절/i;
...
function buildFiveSectionS4(input: {
  ...
  if (hasAllSections) {
    return { body, missing_sections: [], recomposed: false };
  }

  const lines = body.split("\n").map((line) => line.trim()).filter(Boolean);
  const sentences = splitSentences(body);
  const refs = (input.verseRefs?.length ? input.verseRefs : extractRefsFromBody(body)).slice(0, 4);
  const coreClaim = firstNonEmpty([
    bullets[0] || "",
    sentences[0] || "",
    lines[0] || "",
    "질문의 중심 논지를 citation lock 앵커 기준으로 해석합니다.",
  ]);
  const evidenceLine =
    refs.length > 0
      ? refs.map((ref) => `- ${ref}`).join("\n")
      : "- 명시 구절이 부족해 재질의가 필요합니다. (권·장·절 앵커 권장)";
  const counterLine = firstNonEmpty([
    bullets[1] || "",
    sentences[1] || "",
    "대안 해석 가능성을 병기하며 단일 해석을 절대화하지 않습니다.",
  ]);
  const limitLine = firstNonEmpty([
    bullets[2] || "",
    sentences[2] || "",
    "본 응답은 연구 요약이며 교리 확정/실행 지시가 아닙니다.",
  ]);
  const actionLine = firstNonEmpty([
    bullets[3] || "",
...
export function applyInquiryS4QualityGate(input: {
  ...
  return {
    body: stripPublicResearchTagsForS4(composed.body),
