...
foreach ($t in $workspaceTests) {
    if (Test-Path -LiteralPath $t) {
        $existingWorkspaceTests += $t
    } else {
        Write-Host "[alignment] skip missing test: $t" -ForegroundColor DarkGray
    }
}

if ($existingWorkspaceTests.Count -eq 0) {
    Write-Host "[alignment] no workspace tests found; skipping second bundle." -ForegroundColor Yellow
    exit 0
}

& py -m pytest @existingWorkspaceTests -q --tb=short
exit $LASTEXITCODE
