...
workspace_tests=(
  ...
  # KOSPI structural-entropy spike: re-add paths when tests exist (P0 tracker).
...
existing_tests=()
for t in "${workspace_tests[@]}"; do
  if [[ -f "$t" ]]; then
    existing_tests+=("$t")
  else
    echo "[alignment] skip missing test: $t"
  fi
done

if [[ "${#existing_tests[@]}" -eq 0 ]]; then
  echo "[alignment] no workspace tests found; skipping second bundle."
  exit 0
fi

exec "${PY}" -m pytest "${existing_tests[@]}" -q --tb=short
