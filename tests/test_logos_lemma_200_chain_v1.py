"""P6 Lemma-200 spike chain (B-track, HYPO)."""
...
def test_lemma_hit_anchors_at_least_two_hundred(lemma_200_chain: None) -> None:
    bridge = json.loads(BRIDGE.read_text(encoding="utf-8"))
    assert bridge["summary"]["lemma_hit_anchors"] >= 200


def test_p6_spike_path_id(lemma_200_chain: None) -> None:
    doc = json.loads(SPIKE.read_text(encoding="utf-8"))
    if int(doc.get("target_lemma_hit_anchors") or 0) == 200:
        assert doc["path_id"] == "lemma_spike_p6_v1"


def test_p6_drift_no_regression_flags(lemma_200_chain: None) -> None:
