"""P5 Lemma-100 spike chain (B-track, HYPO)."""

from __future__ import annotations

import json
import subprocess
import sys
from pathlib import Path

import pytest

ROOT = Path(__file__).resolve().parents[1]
BRIDGE = ROOT / "docs/final/artifacts/logos_cosmic_anchor_graph_bridge_v1_latest.json"
DRIFT = ROOT / "docs/final/artifacts/logos_lemma_spike_drift_check_v1_latest.json"


@pytest.fixture(scope="module")
def lemma_100_chain() -> None:
    proc = subprocess.run(
        [
            sys.executable,
            "scripts/run_logos_lemma_60_chain_v1.py",
            "--target",
            "100",
            "--skip-pytest",
            "--skip-baseline",
            "--skip-bridge-refresh",
        ],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=False,
    )
    assert proc.returncode == 0, proc.stderr or proc.stdout


def test_lemma_hit_anchors_at_least_one_hundred(lemma_100_chain: None) -> None:
    bridge = json.loads(BRIDGE.read_text(encoding="utf-8"))
    assert bridge["summary"]["lemma_hit_anchors"] >= 100


def test_p5_drift_no_regression_flags(lemma_100_chain: None) -> None:
    if not DRIFT.is_file():
        pytest.skip("drift check artifact missing")
    doc = json.loads(DRIFT.read_text(encoding="utf-8"))
    assert doc.get("pass") is True
    assert doc.get("drift_flags") == []
