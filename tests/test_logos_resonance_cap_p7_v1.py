"""P7 resonance_cap 40→48 bloom deepen — narrative count frozen at 200."""
...
def p7_chain() -> None:
    proc = subprocess.run(
        [
            sys.executable,
            "scripts/run_logos_resonance_cap_p7_chain_v1.py",
            "--skip-pytest",
            "--skip-cdn-purge",
        ],
        cwd=ROOT,
...
def test_resonance_cap_forty_eight(p7_chain: None) -> None:
    for path in (SLICE_ART, SLICE_PUBLIC):
        doc = json.loads(path.read_text(encoding="utf-8"))
        assert doc.get("resonance_cap") == 48
        assert len(doc.get("resonance_edges_top") or []) == 48


def test_narrative_still_two_hundred(p7_chain: None) -> None:
