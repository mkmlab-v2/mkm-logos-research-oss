"""P3d narrative 12→16 curation + eval contract."""
...
def p3d_chain() -> None:
    proc = subprocess.run(
        [
            sys.executable,
            "scripts/run_logos_narrative_path_p3d_chain_v1.py",
            "--skip-pytest",
            "--skip-router",
        ],
        cwd=ROOT,
...
def test_narrative_sample_count_sixteen(p3d_chain: None) -> None:
    bridge = json.loads(BRIDGE.read_text(encoding="utf-8"))
    assert bridge["summary"]["narrative_sample_count"] >= 16
...
def test_p3d_eval_contract(p3d_chain: None) -> None:
    doc = json.loads(EVAL.read_text(encoding="utf-8"))
    assert doc["narrative_sample_count"] >= 16
    assert doc["summary"]["path_ok_rate"] == 1.0
    assert doc["summary"]["sample_pass_rate"] == 1.0


def test_p3d_bloom_presets(p3d_chain: None) -> None:
    doc = json.loads(SLICE.read_text(encoding="utf-8"))
    pmap = doc.get("preset_narrative_map") or {}
    assert pmap["motif_ark"] == "ark_to_star_eschaton"
    assert pmap["motif_abundance"] == "oil_to_honey_abundance"
    assert len(doc.get("narrative_path_samples") or []) >= 16


def test_p3d_inter_hop_and_lemma(p3d_chain: None) -> None:
    inter = json.loads(INTER.read_text(encoding="utf-8"))
    overlap = json.loads(OVERLAP.read_text(encoding="utf-8"))
    assert inter["summary"]["inter_hop_pair_count"] >= 18
