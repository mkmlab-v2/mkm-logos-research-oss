"""P15 bloom resonance_cap floor — SSOT may be >112 after P16 bloom; read-only floor gate."""
...
def p15_bloom_chain() -> None:
    proc = subprocess.run(
        [
            sys.executable,
            "scripts/run_logos_resonance_cap_p15_bloom_chain_v1.py",
            "--skip-pytest",
            "--skip-cdn-purge",
            "--skip-deploy",
        ],
...
def test_resonance_cap_at_least_one_hundred_twelve(p15_bloom_chain: None) -> None:
...
def test_narrative_still_two_hundred(p15_bloom_chain: None) -> None:
