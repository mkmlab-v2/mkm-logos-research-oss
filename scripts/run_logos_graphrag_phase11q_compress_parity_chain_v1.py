#!/usr/bin/env python3
"""Phase 11-Q: Golden-40 compress parity (active report profile) + GATE_SPEC enable [HYPO]."""
...
PY = sys.executable
DEFAULT_OUT = ROOT / "reports/logos_graphrag_phase11q_compress_parity_chain_v1_latest.json"
...
def main() -> int:
    ...
    if not args.skip_comp_bench:
        steps.append(_run("comp_atom02_parity", [PY, "scripts/comp_atom02_lexicon_must_keep_analysis_v1.py"]))
    ...
    ablation = comp_doc.get("compression_ablation") or {}
    strict = ablation.get("lexicon_on") or {}
    saving = parity.get("global_token_saving_rate")
    jaccard = parity.get("avg_reconstruction_fidelity_jaccard")
    would_pass = (
    ...
    all_ok = all(s.get("ok") for s in steps) and would_pass and compress_enabled

    report = {
        "schema": "logos_graphrag_phase11q_compress_parity_chain_v1",
        "version": "1.0.0",
        ...
        "all_ok": all_ok,
        "compress_parity": {
            "active_report_saving_pct": round(float(saving) * 100, 4) if saving is not None else None,
            "active_report_jaccard": jaccard,
            "strict_lexicon_on_saving_pct": round(float(strict.get("global_token_saving_rate") or 0) * 100, 4),
            "live_minus_disk_delta": parity.get("live_minus_disk_saving_delta"),
        ...
        "steps": steps,
        "reproduce": "py scripts/run_logos_graphrag_phase11q_compress_parity_chain_v1.py",
    }
        ...
        json.dumps(
            {
                ...
                "parity_saving_pct": round(float(saving or 0) * 100, 2),
                "research_ready_decision": ev.get("research_ready_decision"),
            },
            ensure_ascii=False,
        )
