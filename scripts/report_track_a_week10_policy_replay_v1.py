...
# Balance: 93
# Purpose: Consolidate Week-10 experiments into final policy replay decision.
# Keywords: track_a, week10, policy_replay, decision, commercialization
#!/usr/bin/env python3
...
E1 = ROOT / "docs" / "final" / "artifacts" / "track_a_week10_domain_tiered_hybrid_sweep_v1.json"
E2 = ROOT / "docs" / "final" / "artifacts" / "track_a_week10_saving_floor_candidate_blend_sweep_v1.json"
E3 = ROOT / "docs" / "final" / "artifacts" / "track_a_week10_dynamic_cap_fallback_sweep_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week10_policy_replay_v1.json"
...
def _read_e4_config(pack: dict[str, Any]) -> tuple[float, bool]:
    ...
    e4 = next((x for x in exps if str(x.get("id")) == "W10-E4"), {})
...
def main() -> int:
    ...
    decision = (
        "GO_W10_POLICY_REPLAY"
        if policy_floor_ok and integrity_ok and quality_tradeoff_ok
        else "HOLD_W10_POLICY_REPLAY"
    )

    out_doc = {
        "schema": "track_a_week10_policy_replay_v1",
        ...
        "week10_summary": {
            "entry_decision": (pack.get("entry_condition") or {}).get("week9_decision"),
        ...
        "decision": decision,
        "next_action": (
            "Keep Track A commercialization HOLD and design Week-11 hypothesis pack."
            if decision == "HOLD_W10_POLICY_REPLAY"
