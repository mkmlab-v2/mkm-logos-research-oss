<#
.SYNOPSIS
  Register (or remove) weekly Scheduled Task for A2A full repro bundle.

.DESCRIPTION
  Runs: scripts/Run-A2aWeeklyReproBundle_v1.ps1
  Default: Sunday 09:00 local (after RQ-019 weekly smoke 08:30 if both registered).

.NOTES
  Verify: scripts/Verify-A2aScheduledTasksReadiness_v1.ps1
#>
param(
    [switch]$Remove,
    [string]$TaskName = "MKM_A2A_Weekly_Repro_Bundle",
...
$runner = Join-Path $resolvedRoot "scripts\Run-A2aWeeklyReproBundle_v1.ps1"
...
if ($SkipDialogueBench) { $argLine += " -SkipDialogueBench" }
...
$description = "Weekly: A2A repro bundle (dialogue bench + tp01-03 + L1L2 + dogfood + IR + peer check). B-track research_only. Workspace: $resolvedRoot"
