...
def main():
    ...
    # F2: conservative gate contract (should_trade=False and rollback=True).
    bad_gate_rows=0
    for it in qna_items:
        evidence=it.get('evidence') if isinstance(it.get('evidence'),dict) else {}
        gate=evidence.get('gate_eval') if isinstance(evidence.get('gate_eval'),dict) else {}
        if gate.get('should_trade') is not False or gate.get('rollback') is not True:
            bad_gate_rows += 1
    ...
    checks=[
        ...
        {'id':'F2','name':'gate_eval_conservative_default','pass':f2_pass,'detail':{'qna_items':qna_count,'bad_gate_rows':bad_gate_rows}},
    ...
    for min_survivor in [1,3,5,7]:
        for min_ci in [0.0,0.2,0.4,0.6]:
            ...
            p=(1 if f1_pass else 0)+(1 if f2_pass else 0)+(1 if s_f3 else 0)+(1 if s_f4 else 0)+(1 if f5_pass else 0)
            sensitivity.append({'min_survivor_count':min_survivor,'min_ci_low_defense_contrib':min_ci,'pass_count':p,'total_checks':5,'suite_status':suite_status_from_passes(p,5)})
    sdoc={'schema':'two_track_falsification_sensitivity_v1','generated_at_utc':now(),'research_only':True,'promotion_required':True,'source_track':'K','fixed_inputs':{'qna_count':qna_count,'missing_fields':missing_fields,'bad_gate_rows':bad_gate_rows,'shift_score':shift_score,'ci_low_defense_contrib':ci_low,'claim_boundary':boundary},'active_thresholds':{'min_survivor_count':int(a.min_survivor_count),'min_shift_score':float(a.min_shift_score),'min_ci_low_defense_contrib':float(a.min_ci_low_defense_contrib)},'grid':sensitivity}
