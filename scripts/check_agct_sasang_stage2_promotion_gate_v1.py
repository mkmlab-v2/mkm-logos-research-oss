...
def main() -> int:
    ap = argparse.ArgumentParser(description="Check AGCT stage2 final promotion gate from compare history.")
    ...
    min_samples = int(th.get("min_samples", 7))
    min_go_ratio = float(th.get("min_go_candidate_ratio", 0.7))
    max_hold_ratio = float(th.get("max_hold_ratio", 0.0))
    require_latest_not_hold = bool(th.get("require_latest_not_hold", True))
    ...
    decision = "GO_STAGE2_FINAL" if all_ok else "HOLD_STAGE2_FINAL"

    payload = {
        "schema": "agct_sasang_stage2_promotion_gate_v1",
