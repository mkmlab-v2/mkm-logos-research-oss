#!/usr/bin/env python3
"""TKM encounter_sequence P50 gate: post-extension passive observation [HYPO]."""
...
OUT = ROOT / "docs/final/artifacts/tkm_encounter_sequence_p50_gate_v1_latest.json"
...
def build() -> dict[str, Any]:
    ...
    obs = _load(OBS)
    weekly = _load(WEEKLY)
    po = weekly.get("post_extension_observation_kpi") if isinstance(weekly.get("post_extension_observation_kpi"), dict) else {}

    checks = {
        "p49_gate_ok": {"passed": p49.get("gate_ok") is True},
        "observation_ok": {"passed": obs.get("observation_ok") is True},
        "observation_artifact_mirrored": {"passed": ARTIFACT.is_file()},
        "full_extension_closure_ok": {"passed": obs.get("full_extension_closure_ok") is True},
        ...
        "weekly_post_extension_sync_ok": {"passed": po.get("post_extension_observation_headline_ok") is True},
    ...
    return {
        "schema": "tkm_encounter_sequence_p50_gate_v1",
        "version": "1.0.0",
        ...
        "send_gate": "HOLD",
        "research_only": True,
        "tkm_encounter_sequence_p50_status": "post_extension_passive_observation_wire_ok" if gate_ok else "incomplete",
        "gate_ok": gate_ok,
        "checks": checks,
...
def main() -> int:
    ...
    print(json.dumps({"ok": doc["gate_ok"], "status": doc["tkm_encounter_sequence_p50_status"]}))
