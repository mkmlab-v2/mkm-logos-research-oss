...
# Balance: 93
# Purpose: Run Week-11 E2 off-path quota backpressure sweep.
# Keywords: track_a, week11, offpath_quota, backpressure, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week11_offpath_quota_backpressure_sweep_v1.json"
...
def _read_e2_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[int]]:
    ...
    e2 = next((x for x in experiments if str(x.get("id")) == "W11-E2"), {})
    cfg = e2.get("config") or {}
    offpath_quota_grid = [float(x) for x in (cfg.get("offpath_quota_grid") or [])]
    backpressure_alpha_grid = [float(x) for x in (cfg.get("backpressure_alpha_grid") or [])]
    candidate_pool_grid = [int(x) for x in (cfg.get("candidate_pool_grid") or [])]
    return (
        offpath_quota_grid or [0.08, 0.1, 0.12],
        backpressure_alpha_grid or [0.25, 0.3, 0.35],
...
def main() -> int:
    ...
    offpath_quotas, backpressure_alphas, candidate_pools = _read_e2_config(pack_doc)
    ...
    case_ids = list(on_map.keys())

    runs: list[dict[str, Any]] = []
    for quota in offpath_quotas:
        for alpha in backpressure_alphas:
            for pool in candidate_pools:
                selected: list[dict[str, Any]] = []
                switched_to_off = 0
                backpressure_blocked = 0
                total = 0
                allowed_off = max(0, int(len(case_ids) * max(0.0, min(0.5, quota))))
                for cid, on_case in on_map.items():
                    ...
                    quality_drop = max(0.0, on_j - off_j)
                    pressure = quality_drop * alpha * (1.0 + (pool - 2) * 0.15)
                    gate = _hash_unit(f"w11-e2:{cid}:{quota}:{alpha}:{pool}")
                    want_off = gate <= quota
                    if want_off and (switched_to_off >= allowed_off or pressure > 0.015):
                        backpressure_blocked += 1
                        want_off = False
                    selected.append(off_case if want_off else on_case)
                    if want_off:
                        switched_to_off += 1
                    total += 1
                ...
                row = {
                    "offpath_quota": quota,
                    "backpressure_alpha": alpha,
                    "candidate_pool": pool,
                    "offpath_quota_limit_cases": allowed_off,
                    "switched_to_router_off_cases": switched_to_off,
                    "backpressure_blocked_cases": backpressure_blocked,
    ...
    decision = "GO_W11_E3_DYNAMIC_CAP_CONFIDENCE_VETO" if recommended else "HOLD_W11_E2_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week11_offpath_quota_backpressure_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "offpath_quota_grid": offpath_quotas,
            "backpressure_alpha_grid": backpressure_alphas,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W11-E3 dynamic-cap confidence-veto sweep."
            if decision == "GO_W11_E3_DYNAMIC_CAP_CONFIDENCE_VETO"
            else "Keep baseline and continue to W11-E3 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
