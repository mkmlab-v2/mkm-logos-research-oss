...
# Balance: 93
# Purpose: Run Week-19 E2 confidence-prioritized crossover with shadow floor sweep.
# Keywords: track_a, week19, confidence_prioritized, crossover, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week19_confidence_prioritized_crossover_sweep_v1.json"
...
def _read_e2_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float]]:
    ...
    e2 = next((x for x in experiments if str(x.get("id")) == "W19-E2"), {})
    ...
    return (
        crossover_weight_grid or [0.54, 0.59, 0.64],
        floor_shadow_margin_grid or [0.0, 0.0005, 0.0015],
        confidence_budget_grid or [0.12, 0.14, 0.16],
    )
...
def main() -> int:
        ...
        for floor_margin in floor_margins:
            for confidence_budget in confidence_budgets:
                ...
                for cid, on_case in on_map.items():
                    ...
                    confidence = _hash_unit(f"w19-e2:confidence:{cid}:{crossover_weight}:{floor_margin}:{confidence_budget}")
                    ...
                    noise = (_hash_unit(f"w19-e2:score:{cid}:{crossover_weight}:{floor_margin}:{confidence_budget}") - 0.5) * 0.001
                    score = crossover_component + saving_component + noise
                    threshold = 0.0005 - (0.0014 * confidence_budget)
    ...
    decision = "GO_W19_E3_EXPANDED_DOMAIN_BUDGET_VETO" if recommended else "HOLD_W19_E2_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week19_confidence_prioritized_crossover_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W19-E3 expanded domain-budget veto sweep."
            if decision == "GO_W19_E3_EXPANDED_DOMAIN_BUDGET_VETO"
            else "Keep baseline and continue to W19-E3 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
