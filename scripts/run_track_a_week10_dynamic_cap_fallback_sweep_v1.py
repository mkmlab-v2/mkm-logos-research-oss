...
# Balance: 93
# Purpose: Run Week-10 E3 dynamic-cap fallback sweep.
# Keywords: track_a, week10, dynamic_cap, fallback, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week10_dynamic_cap_fallback_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float]]:
    ...
    e3 = next((x for x in experiments if str(x.get("id")) == "W10-E3"), {})
    cfg = e3.get("config") or {}
    confidence_band_grid = [float(x) for x in (cfg.get("confidence_band_grid") or [])]
    ...
    return (
        confidence_band_grid or [0.6, 0.64, 0.68],
...
def main() -> int:
    ...
    confidence_bands, dynamic_caps, rollback_penalties = _read_e3_config(pack_doc)
        ...
        for dcap in dynamic_caps:
            for penalty in rollback_penalties:
                ...
                for cid, on_case in on_map.items():
                    ...
                    confidence = _hash_unit(f"w10-e3:confidence:{cid}:{band}")
                    # High-confidence cases prefer router_on to protect quality.
                    if confidence >= band:
                        selected.append(on_case)
                        rollback_to_on += 1
                        total += 1
                    ...
                    off_score += (off_j - on_j) * 0.35
                    use_off = off_score >= on_score
                    # Dynamic cap rollback: revert off path when cap overshoot penalty is severe.
    ...
    decision = "GO_W10_E4_POLICY_REPLAY" if recommended else "HOLD_W10_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week10_dynamic_cap_fallback_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "confidence_band_grid": confidence_bands,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W10-E4 policy replay."
            if decision == "GO_W10_E4_POLICY_REPLAY"
            else "Keep baseline and continue to W10-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
