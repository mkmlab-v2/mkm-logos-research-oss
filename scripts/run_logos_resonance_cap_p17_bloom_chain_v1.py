#!/usr/bin/env python3
"""P17 bloom resonance_cap 120→128 — final scheduled deepen (B-track, HYPO).

Oracle lane label P28; narrative count frozen at 200. After this step SSOT sets
bloom_deepen_frozen=true — no further +8 deepen unless observation gate fails.

Reproducible:
  py scripts/run_logos_resonance_cap_p17_bloom_chain_v1.py
"""
...
def main() -> int:
    ...
    if not args.skip_deploy:
        open_next = (
            ROOT
            / "projects/mkm/mkm-life/.open-next/assets/data/logos_cosmic_anchor_graph_bloom_slice_v1.json"
        )
        public = ROOT / "projects/mkm/mkm-life/public/data/logos_cosmic_anchor_graph_bloom_slice_v1.json"
        open_next.parent.mkdir(parents=True, exist_ok=True)
        open_next.write_bytes(public.read_bytes())
        proc = subprocess.run(
            [
                "powershell",
                "-NoProfile",
                ...
                "-SkipPostDeploySmoke",
                "-SkipRestartDevAfterDeploy",
            ],
            cwd=ROOT,
            check=False,
        )
        if proc.returncode != 0:
            print("FAIL: mkmlife deploy", file=sys.stderr)
            return proc.returncode
        print("OK: Deploy-CloudflareMkmlife.ps1")
    ...
    if not args.skip_pytest:
        proc = subprocess.run(
            [
                ...
                "-m",
                "pytest",
                "tests/test_logos_resonance_cap_p17_bloom_v1.py",
                "-q",
            ],
            cwd=ROOT,
            check=False,
        ...
        if proc.returncode != 0:
            return proc.returncode
        print("OK: pytest resonance_cap p17 bloom")

    print(
        f"narratives={n_samples} resonance_cap={cap} resonance_top={top_n} "
        f"lemma_hits={bridge['summary']['lemma_hit_anchors']} bloom_deepen_frozen=128"
    )
    return 0
