#!/usr/bin/env python3
"""Record counsel sign-off for Saving the News (human gate — real counsel reference required)."""
...
DEFAULT_OUT = ART / "saving_the_news_legal_counsel_signoff_v1_latest.json"
HANDOFF = ART / "saving_the_news_legal_counsel_handoff_v1_latest.json"
...
def build(
    *,
    ...
    return {
        "schema": "saving_the_news_legal_counsel_signoff_v1",
        ...
        "boundary_ack": (
            "Counsel review recorded; external send still requires separate commander approval "
            "unless ready_for_external_send was explicitly set with counsel scope."
        ),
    }
...
def main() -> int:
    ...
    ap.add_argument("--apply-scope", action="store_true", help="Patch handoff JSON counsel_signoff block")
    args = ap.parse_args()
    doc = build(
