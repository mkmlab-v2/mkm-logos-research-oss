#!/usr/bin/env python3
"""Backfill encounter_sequence ledger with l6_logos_ref cosmic anchor sidecars [HYPO]."""
...
OUT = ROOT / "reports/tkm_encounter_sequence_logos_sidecar_apply_v1_latest.json"
...
def run(*, dry_run: bool = False, force: bool = False) -> dict[str, Any]:
    ...
    for row in records:
        ...
        if not seq_id:
            continue
        if isinstance(row.get("l6_logos_ref"), dict) and not force:
            existing += 1
            skipped += 1
            continue
        new_row = logos_mod.attach_sidecar(row, force=force)
        sep_errs = logos_mod.validate_logos_lens_separation(new_row)
        if sep_errs:
    ...
    return {
        "schema": "tkm_encounter_sequence_logos_sidecar_apply_v1",
