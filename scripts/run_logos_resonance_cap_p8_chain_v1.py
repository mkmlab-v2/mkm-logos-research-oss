#!/usr/bin/env python3
"""P8 resonance_cap 48→56 bloom slice deepen (B-track, HYPO).

Narrative count frozen at 200; inter-hop not re-run.

Reproducible:
  py scripts/run_logos_resonance_cap_p8_chain_v1.py
"""
...
def main() -> int:
    ...
    if not args.skip_pytest:
        proc = subprocess.run(
            [sys.executable, "-m", "pytest", "tests/test_logos_resonance_cap_p8_v1.py", "-q"],
            cwd=ROOT,
            check=False,
        )
        if proc.returncode != 0:
            return proc.returncode
        print("OK: pytest resonance_cap p8")
