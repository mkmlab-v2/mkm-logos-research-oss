#!/usr/bin/env python3
"""Verify Logos vs sasang/myeongni lens separation on encounter_sequence [HYPO]."""
...
OUT = ROOT / "reports/tkm_logos_sasang_lens_separation_v1_latest.json"
...
def check() -> dict[str, Any]:
    ...
    for row in records:
        ...
        if not isinstance(row.get("l6_logos_ref"), dict):
            continue
        with_sidecar += 1
        errs = logos_mod.validate_logos_lens_separation(row)
        if errs:
            violations.append({"sequence_id": seq_id, "errors": errs})

    ok = with_sidecar >= 1 and not violations
    return {
        "schema": "tkm_logos_sasang_lens_separation_v1",
        ...
        "send_gate": "HOLD",
        "reproduce": "py scripts/check_tkm_logos_sasang_lens_separation_v1.py",
    }
...
def main() -> int:
    ...
    print(json.dumps({"ok": doc.get("separation_ok"), "with_sidecar": doc.get("with_sidecar_count")}))
