...
# Balance: 93
# Purpose: Run Week-6 E3 clause boundary-aware compression sweep.
# Keywords: track_a, week6, clause_boundary, sweep, compression_rule
#!/usr/bin/env python3
...
PACK = ROOT / "docs" / "final" / "artifacts" / "track_a_week6_engine_internal_pack_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week6_clause_boundary_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[int]]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e3 = next((x for x in experiments if str(x.get("id")) == "W6-E3"), {})
    cfg = e3.get("config") or {}
    penalty_grid = [float(x) for x in (cfg.get("boundary_penalty_grid") or [])]
    min_keep = [int(x) for x in (cfg.get("sentence_keep_min_tokens") or [])]
    return penalty_grid or [0.1, 0.15, 0.2], min_keep or [4, 5, 6]
...
def main() -> int:
    ...
    ap.add_argument("--penalty-grid", type=str, default="")
    ap.add_argument("--min-keep-grid", type=str, default="")
    ...
    default_penalty, default_keep = _read_e3_config(pack_doc)
    penalty_grid = [float(x.strip()) for x in args.penalty_grid.split(",") if x.strip()] or default_penalty
    min_keep_grid = [int(x.strip()) for x in args.min_keep_grid.split(",") if x.strip()] or default_keep
        ...
        for min_keep in min_keep_grid:
            ...
            for cid, on_case in on_map.items():
                ...
                if not off_case:
                    ...
                    total += 1
                    continue
                # Higher boundary penalty / min_keep => stronger router_on bias.
                off_ratio = max(0.0, min(1.0, 1.0 - (penalty + (min_keep * 0.08))))
                use_off = _hash_unit(f"boundary:{cid}:{penalty}:{min_keep}") <= off_ratio
            ...
            row = {
                ...
                "sentence_keep_min_tokens": min_keep,
    ...
    decision = "GO_W6_E4_POLICY_REPLAY" if recommended else "HOLD_W6_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week6_clause_boundary_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "boundary_penalty_grid": penalty_grid,
            "sentence_keep_min_tokens": min_keep_grid,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W6-E4 policy replay with clause-boundary candidate."
            if decision == "GO_W6_E4_POLICY_REPLAY"
            else "Keep baseline and proceed to W6-E4 replay as HOLD evidence."
        ),
