...
def _build_lines(backtest: dict[str, Any], coord: dict[str, Any], veto: dict[str, Any]) -> dict[str, str]:
...
4. **[검증 경계]** 본 문장들은 버전 고정 아티팩트와 제한된 평가 윈도우에만 근거하며, 프로덕션 승격은 정식 Promotion Loop(`B -> Commander approval -> A`)를 따릅니다.
...
def main() -> int:
    ...
    lines = _build_lines(backtest, coord, veto)
