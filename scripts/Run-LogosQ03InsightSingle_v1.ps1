# q03 insight chain — CPU ANN + typology boost (covenant/judgment/restoration + mercy bridge).
param(
    [switch]$SkipAnnLite,
    [switch]$DryRun
)
...
Write-Host "[q03] CPU-only insight rebuild (covenant/judgment/restoration typology)" -ForegroundColor Cyan
& py @args
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
Write-Host "[OK] q03 insight + gold eval" -ForegroundColor Green
