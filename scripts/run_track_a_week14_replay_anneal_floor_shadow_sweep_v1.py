...
# Balance: 93
# Purpose: Run Week-14 E2 replay anneal blend with floor shadow sweep.
# Keywords: track_a, week14, replay_anneal, floor_shadow, sweep
#!/usr/bin/env python3
...
PACK = ROOT / "docs" / "final" / "artifacts" / "track_a_week14_hypothesis_pack_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week14_replay_anneal_floor_shadow_sweep_v1.json"
...
def _read_e2_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[int]]:
    ...
    e2 = next((x for x in experiments if str(x.get("id")) == "W14-E2"), {})
    cfg = e2.get("config") or {}
    anneal_weight_grid = [float(x) for x in (cfg.get("anneal_weight_grid") or [])]
    ...
    return (
        anneal_weight_grid or [0.62, 0.66, 0.7],
...
def main() -> int:
    ...
    anneal_weights, floor_margins, candidate_pools = _read_e2_config(pack_doc)
    ...
    for anneal_weight in anneal_weights:
            ...
            for candidate_pool in candidate_pools:
                ...
                for cid, on_case in on_map.items():
                    ...
                    if off_s < floor_shadow:
                        ...
                        total += 1
                        continue
                    anneal_component = (off_j - on_j) * anneal_weight
                    saving_component = (off_s - on_s) * (1.0 - anneal_weight)
                    noise = (_hash_unit(f"w14-e2:{cid}:{anneal_weight}:{floor_margin}:{candidate_pool}") - 0.5) * 0.001
                    score = anneal_component + saving_component + noise
                ...
                row = {
                    ...
                    "anneal_win_cases": anneal_win_cases,
    ...
    decision = "GO_W14_E3_CONFIDENCE_THROTTLE" if recommended else "HOLD_W14_E2_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week14_replay_anneal_floor_shadow_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "anneal_weight_grid": anneal_weights,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W14-E3 confidence throttle sweep."
            if decision == "GO_W14_E3_CONFIDENCE_THROTTLE"
            else "Keep baseline and continue to W14-E3 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
