...
# Balance: 93
# Purpose: Run Week-20 E1 staged guard unlock sweep.
# Keywords: track_a, week20, staged_guard, unlock, sweep
#!/usr/bin/env python3
...
PACK = ROOT / "docs" / "final" / "artifacts" / "track_a_week20_hypothesis_pack_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week20_staged_guard_unlock_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float], list[str]]:
    ...
    e1 = next((x for x in experiments if str(x.get("id")) == "W20-E1"), {})
    ...
    return (
        unlock_ratio_grid or [0.16, 0.18, 0.2],
        jaccard_guard_grid or [0.841, 0.843, 0.845],
        guard_relax_margin_grid or [0.003, 0.004, 0.005],
...
def main() -> int:
            ...
            for guard_relax_margin in guard_relax_margins:
                ...
                for cid, on_case in on_map.items():
                    ...
                    if off_j < guard_threshold:
                        ...
                        total += 1
                        continue
                    unlock_signal = _hash_unit(f"w20-e1:unlock:{cid}:{unlock_ratio}:{jaccard_guard}:{guard_relax_margin}")
    ...
    decision = "GO_W20_E2_CONFIDENCE_FIRST_CROSSOVER" if recommended else "HOLD_W20_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week20_staged_guard_unlock_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W20-E2 confidence-first crossover sweep."
            if decision == "GO_W20_E2_CONFIDENCE_FIRST_CROSSOVER"
            else "Keep baseline and continue to W20-E2 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
