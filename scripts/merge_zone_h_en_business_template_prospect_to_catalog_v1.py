#!/usr/bin/env python3
"""Merge zone_h_en_business prospect templates into production catalog (gated).
...
from scripts.merge_zone_h_en_business_template_prospect_v1_lib import (  # noqa: E402
    load_catalog_rows,
    plan_prospect_merge,
    write_catalog_jsonl,
)
...
DEFAULT_REPORT = ROOT / "reports/zone_h_en_business_template_catalog_merge_v1_latest.json"
DEFAULT_ARTIFACT = ROOT / "docs/final/artifacts/zone_h_en_business_template_catalog_merge_v1_latest.json"
...
def build_merge_report(
    *,
    ...
    return {
        "schema": "zone_h_en_business_template_catalog_merge_v1",
        ...
        "apply_command": (
            "py scripts/merge_zone_h_en_business_template_prospect_to_catalog_v1.py "
            "--human-approve-merge --reviewer commander --rebuild-gate"
        ),
        "forbidden_without_approve": ["automatic_production_catalog_overwrite"],
        "reproduce": "py scripts/merge_zone_h_en_business_template_prospect_to_catalog_v1.py",
    }


def main() -> int:
    ap = argparse.ArgumentParser(description="Merge en business prospect templates into production (gated)")
