...
def _coding_corpora_wire_match_full(coverage: dict[str, Any]) -> bool:
    corpora = coverage.get("corpora") or []
    coding_rows = [c for c in corpora if int(c.get("snippet_candidates_total") or 0) > 0]
    if not coding_rows:
        return False
    return all(float(c.get("wire_match_rate") or 0) >= 1.0 for c in coding_rows)
...
def build_checklist(
    *,
    ...
    coverage_path = (
        ROOT / coverage_rel
        if coverage_rel
        else ROOT / "docs/final/artifacts/zone_f_code_template_catalog_coverage_v1_latest.json"
    )
    ...
    checklist = {
        ...
        and coverage.get("schema") == "zone_f_code_template_catalog_coverage_v1",
        "coverage_coding_corpora_wire_match_full": _coding_corpora_wire_match_full(coverage),
    ...
    return {
        ...
        "btrack_coverage_note": (
            "Wire-match coverage on JSONL corpora is B-track evidence only — not customer SLA or SEND metric."
        ),
        "operator_action": {
