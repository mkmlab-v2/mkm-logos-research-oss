...
# Balance: 93
# Purpose: Run Week-6 E2 n-gram preservation rule injection sweep.
# Keywords: track_a, week6, ngram, preserve, compression_rule
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week6_ngram_preserve_sweep_v1.json"
...
def _read_e2_config(pack_doc: dict[str, Any]) -> tuple[list[int], list[float], list[str]]:
    ...
    e2 = next((x for x in experiments if str(x.get("id")) == "W6-E2"), {})
    cfg = e2.get("config") or {}
    n_grid = [int(x) for x in (cfg.get("ngram_n_grid") or [])]
    ratio_grid = [float(x) for x in (cfg.get("preserve_ratio_grid") or [])]
    focus = [str(x).strip().lower() for x in (cfg.get("domain_focus") or []) if str(x).strip()]
    return n_grid or [2, 3], ratio_grid or [0.15, 0.2, 0.25], focus or ["ssot", "timing"]
...
def main() -> int:
    ...
    ap.add_argument("--ngram-grid", type=str, default="")
    ap.add_argument("--preserve-ratio-grid", type=str, default="")
    ...
    default_n, default_ratio, domain_focus = _read_e2_config(pack_doc)
    n_grid = [int(x.strip()) for x in args.ngram_grid.split(",") if x.strip()] or default_n
    ratio_grid = [float(x.strip()) for x in args.preserve_ratio_grid.split(",") if x.strip()] or default_ratio
    focus_set = set(domain_focus)
    ...
    for n in n_grid:
        for ratio in ratio_grid:
            ...
            for cid, on_case in on_map.items():
                ...
                base_off_ratio = max(0.0, min(1.0, 1.0 - (ratio / max(1.0, float(n)))))
                if domain in focus_set:
                    base_off_ratio *= 0.7
                use_off = _hash_unit(f"{domain}:{cid}:ngram:{n}:{ratio}") <= base_off_ratio
    ...
    decision = "GO_W6_E3_CLAUSE_BOUNDARY" if recommended else "HOLD_W6_E2_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week6_ngram_preserve_sweep_v1",
        ...
        "inputs": {
            ...
            "pack": str(args.pack),
            "ngram_n_grid": n_grid,
            "preserve_ratio_grid": ratio_grid,
            "domain_focus": sorted(focus_set),
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W6-E3 clause-boundary sweep."
            if decision == "GO_W6_E3_CLAUSE_BOUNDARY"
            else "Keep baseline and continue to W6-E3 boundary-aware rule path."
        ),
