...
# Balance: 90
# Purpose: Build phase-2 promotion decision from canary observation window.
# Keywords: canary, promotion, phase2, decision, ops
#!/usr/bin/env python3
...
DEFAULT_OUT = ART / "l1_inverse_decoder_mode_router_v3_phase2_promotion_decision_v1.json"
...
def main() -> int:
    ...
    ap.add_argument("--min-observation-points", type=int, default=4)
    ...
    total_points = len(in_window)
    keep_points = sum(1 for r in in_window if r.get("action") == "KEEP_CANARY")
    rollback_points = sum(1 for r in in_window if r.get("action") == "ROLLBACK_TO_V4")
    ...
    if not min_points_ok:
        ...
        decision = "GO_PHASE2_30PCT"
    else:
        decision = "HOLD_PHASE1_OR_ROLLBACK"

    out_doc = {
        "schema": "l1_inverse_decoder_mode_router_v3_phase2_promotion_decision_v1",
        ...
        "decision": decision,
        "next_action": "Promote canary traffic to 30% with same rollback guard." if decision == "GO_PHASE2_30PCT" else (
            "Continue phase_1 monitoring until enough observation points." if decision == "WAIT_MORE_OBSERVATION" else "Keep phase_1 or rollback based on guard action."
        ),
    }
    ...
    print(json.dumps({"ok": True, "out": str(out_path), "decision": decision, "points": total_points}, ensure_ascii=False))
    return 0
