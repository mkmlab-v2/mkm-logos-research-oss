...
# Balance: 93
# Purpose: Run Week-19 E1 progressive guard unlock sweep.
# Keywords: track_a, week19, progressive_guard, unlock, sweep
#!/usr/bin/env python3
...
PACK = ROOT / "docs" / "final" / "artifacts" / "track_a_week19_hypothesis_pack_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week19_progressive_guard_unlock_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float], list[str]]:
    ...
    e1 = next((x for x in experiments if str(x.get("id")) == "W19-E1"), {})
    ...
    guard_relax_margin_grid = [float(x) for x in (cfg.get("guard_relax_margin_grid") or [])]
    protected_domains = [str(x).strip().lower() for x in (cfg.get("protected_domains") or []) if str(x).strip()]
    return (
        unlock_ratio_grid or [0.14, 0.16, 0.18],
        jaccard_guard_grid or [0.842, 0.844, 0.846],
        guard_relax_margin_grid or [0.002, 0.003, 0.004],
...
def main() -> int:
    ...
    for unlock_ratio in unlock_ratios:
        for jaccard_guard in jaccard_guards:
            for guard_relax_margin in guard_relax_margins:
                ...
                for cid, on_case in on_map.items():
                    ...
                    guard_threshold = max(jaccard_guard - guard_relax_margin, on_j - 0.0015)
                    if off_j < guard_threshold:
                        selected.append(on_case)
                        guard_blocked += 1
                        total += 1
                        continue
                    unlock_signal = _hash_unit(f"w19-e1:unlock:{cid}:{unlock_ratio}:{jaccard_guard}:{guard_relax_margin}")
                    use_off = unlock_signal <= unlock_ratio
                    if not use_off:
                        unlock_blocked += 1
                ...
                row = {
                    ...
                    "guard_relax_margin": guard_relax_margin,
                    ...
                    "unlock_blocked_cases": unlock_blocked,
    ...
    decision = "GO_W19_E2_CONFIDENCE_PRIORITIZED_CROSSOVER" if recommended else "HOLD_W19_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week19_progressive_guard_unlock_sweep_v1",
        ...
        "inputs": {
            ...
            "guard_relax_margin_grid": guard_relax_margins,
            "protected_domains": sorted(protected_set),
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W19-E2 confidence-prioritized crossover sweep."
            if decision == "GO_W19_E2_CONFIDENCE_PRIORITIZED_CROSSOVER"
            else "Keep baseline and continue to W19-E2 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
