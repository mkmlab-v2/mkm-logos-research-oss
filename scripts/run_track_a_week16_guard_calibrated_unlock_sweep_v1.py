...
# Balance: 93
# Purpose: Run Week-16 E1 guard-calibrated selective lane unlock sweep.
# Keywords: track_a, week16, guard_calibrated, lane_unlock, sweep
#!/usr/bin/env python3
...
PACK = ROOT / "docs" / "final" / "artifacts" / "track_a_week16_hypothesis_pack_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week16_guard_calibrated_unlock_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[str]]:
    ...
    e1 = next((x for x in experiments if str(x.get("id")) == "W16-E1"), {})
    ...
    return (
        ...
        jaccard_guard_grid or [0.845, 0.847, 0.849],
...
def main() -> int:
    ...
    unlock_ratios, jaccard_guards, protected_domains = _read_e1_config(pack_doc)
        ...
        for jaccard_guard in jaccard_guards:
            ...
            for cid, on_case in on_map.items():
                ...
                if off_j < max(jaccard_guard, on_j - 0.0015):
                    ...
                    total += 1
                    continue
                unlock_signal = _hash_unit(f"w16-e1:unlock:{cid}:{unlock_ratio}:{jaccard_guard}")
    ...
    decision = "GO_W16_E2_DUAL_THRESHOLD_CROSSOVER" if recommended else "HOLD_W16_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week16_guard_calibrated_unlock_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W16-E2 dual-threshold crossover sweep."
            if decision == "GO_W16_E2_DUAL_THRESHOLD_CROSSOVER"
            else "Keep baseline and continue to W16-E2 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
