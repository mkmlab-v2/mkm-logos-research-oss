#!/usr/bin/env python3
"""TKM encounter_sequence P60 gate: full grand-stack final closure [HYPO]."""
...
P59_GATE = ROOT / "docs/final/artifacts/tkm_encounter_sequence_p59_gate_v1_latest.json"
...
OUT = ROOT / "docs/final/artifacts/tkm_encounter_sequence_p60_gate_v1_latest.json"
...
def build() -> dict[str, Any]:
    ...
    weekly = _load(WEEKLY)
    fgsc = weekly.get("full_grand_stack_final_closure_kpi") if isinstance(
        weekly.get("full_grand_stack_final_closure_kpi"), dict
    ) else {}
    gates = closure.get("gates") if isinstance(closure.get("gates"), dict) else {}

    checks = {
        "p59_gate_ok": {"passed": p59.get("gate_ok") is True},
        ...
        "weekly_full_grand_stack_sync_ok": {"passed": fgsc.get("full_grand_stack_final_closure_headline_ok") is True},
    ...
    return {
        "schema": "tkm_encounter_sequence_p60_gate_v1",
        "version": "1.0.0",
        ...
        "send_gate": "HOLD",
        "research_only": True,
        "tkm_encounter_sequence_p60_status": "full_grand_stack_final_closure_wire_ok" if gate_ok else "incomplete",
        "gate_ok": gate_ok,
        "checks": checks,
...
def main() -> int:
    ...
    print(json.dumps({"ok": doc["gate_ok"], "status": doc["tkm_encounter_sequence_p60_status"]}))
