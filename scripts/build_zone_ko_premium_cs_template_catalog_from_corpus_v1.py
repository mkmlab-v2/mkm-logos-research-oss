#!/usr/bin/env python3
"""Build zone_ko_premium_cs prospect template catalog from JSONL corpora.
...
DEFAULT_BODYONLY = ROOT / "data/compression/stateless_poc_prospect_wtt-premium-cs-customer-v1_v1_bodyonly_v1.jsonl"
DEFAULT_REPORT = ROOT / "reports/zone_ko_premium_cs_template_catalog_extract_v1_latest.json"
DEFAULT_ARTIFACT = ROOT / "docs/final/artifacts/zone_ko_premium_cs_template_catalog_extract_v1_latest.json"
SPEC_OUT = ROOT / "docs/final/artifacts/compression_ko_premium_cs_deep_pack_spec_v1_latest.json"
...
def build_extract_report(
    *,
    ...
    mask_ok = all("███" in str(r.get("snippet") or "") for r in prospect_rows) if prospect_rows else True
    return {
        "schema": "zone_ko_premium_cs_template_catalog_extract_v1",
        ...
        "twin_preview": {
            ...
            "mask_token_in_all_snippets": mask_ok,
        ...
        "reproduce": (
            "py scripts/build_zone_ko_premium_cs_template_catalog_from_corpus_v1.py "
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Extract zone_ko_premium_cs template seeds from JSONL")
    ap.add_argument("--input-jsonl", type=Path, default=DEFAULT_BODYONLY)
