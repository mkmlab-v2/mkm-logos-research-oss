...
# Balance: 93
# Purpose: Run Week-13 E1 domain-priority selective unlock sweep.
# Keywords: track_a, week13, domain_priority, selective_unlock, sweep
#!/usr/bin/env python3
...
PACK = ROOT / "docs" / "final" / "artifacts" / "track_a_week13_hypothesis_pack_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week13_domain_priority_unlock_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[dict[str, float], list[float], list[float]]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e1 = next((x for x in experiments if str(x.get("id")) == "W13-E1"), {})
    cfg = e1.get("config") or {}
    raw_weights = cfg.get("priority_domain_weights") or {}
    priority_domain_weights = {str(k).strip().lower(): float(v) for k, v in raw_weights.items() if str(k).strip()}
    unlock_ratio_grid = [float(x) for x in (cfg.get("unlock_ratio_grid") or [])]
    jaccard_guard_grid = [float(x) for x in (cfg.get("jaccard_guard_grid") or [])]
    if "default" not in priority_domain_weights:
        priority_domain_weights["default"] = 0.65
    return (
        priority_domain_weights,
        unlock_ratio_grid or [0.08, 0.1, 0.12],
...
def main() -> int:
    ...
    domain_weights, unlock_ratios, jaccard_guards = _read_e1_config(pack_doc)
    ...
    for unlock_ratio in unlock_ratios:
        for jaccard_guard in jaccard_guards:
            ...
            for cid, on_case in on_map.items():
                ...
                domain_weight = float(domain_weights.get(domain, domain_weights.get("default", 0.65)))
                on_j = float(on_case.get("reconstruction_fidelity_jaccard", 0.0))
                off_j = float(off_case.get("reconstruction_fidelity_jaccard", 0.0))
                if off_j < max(jaccard_guard, on_j - 0.002):
                    selected.append(on_case)
                    guard_blocked += 1
                    total += 1
                    continue
                unlock_signal = _hash_unit(f"w13-e1:{cid}:{domain}:{unlock_ratio}:{jaccard_guard}")
                effective_unlock = max(0.0, min(1.0, unlock_ratio * domain_weight))
                use_off = unlock_signal <= effective_unlock
                if not use_off:
                    weighted_blocked += 1
            ...
            row = {
                ...
                "priority_domain_weights": domain_weights,
                "switched_to_router_off_cases": switched_to_off,
                "guard_blocked_cases": guard_blocked,
                "weighted_blocked_cases": weighted_blocked,
    ...
    decision = "GO_W13_E2_RECOVERY_BLEND_SHADOW" if recommended else "HOLD_W13_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week13_domain_priority_unlock_sweep_v1",
        ...
        "inputs": {
            ...
            "pack": str(args.pack),
            "priority_domain_weights": domain_weights,
            "unlock_ratio_grid": unlock_ratios,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W13-E2 recovery blend floor-shadow sweep."
            if decision == "GO_W13_E2_RECOVERY_BLEND_SHADOW"
            else "Keep baseline and continue to W13-E2 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
