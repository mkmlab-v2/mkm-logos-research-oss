#!/usr/bin/env python3
"""P15 resonance_cap 104→112 bloom slice deepen (B-track, HYPO).

Oracle lane label P26; narrative count frozen at 200; inter-hop not re-run.
(Not to be confused with run_logos_router_hit_pilot_p15_chain_v1.py — router regression.)

Reproducible:
  py scripts/run_logos_resonance_cap_p15_bloom_chain_v1.py
"""
    ...
    if not args.skip_pytest:
        proc = subprocess.run(
            [
                ...
                "-m",
                "pytest",
                "tests/test_logos_resonance_cap_p15_bloom_v1.py",
                "-q",
            ],
            cwd=ROOT,
            check=False,
        ...
        if proc.returncode != 0:
            return proc.returncode
        print("OK: pytest resonance_cap p15 bloom")
