#!/usr/bin/env python3
"""Register MKM_THEORY_MATHEMATIZATION NL notebook in notebooklm-mcp library."""
...
OUT = ROOT / "reports" / "notebooklm_theory_mathematization_mcp_register_latest.json"

NOTEBOOK_MCP_ID = "23-mkm-theory-mathematization-2026q2"
NOTEBOOK_NAME = "23_MKM_THEORY_MATHEMATIZATION_2026Q2"
DESCRIPTION = (
    "MKM theory mathematization internal canon — 75-formula HYPO index, forbidden list, "
    "WORLDVIEW §5. Track A·K-Startup·live trading cross-cite forbidden."
)
    ...
    if listed.returncode == 0:
        ...
        for row in rows:
            if str(row.get("title", "")).strip() == NOTEBOOK_NAME and row.get("id"):
                url = f"https://notebooklm.google.com/notebook/{row['id']}"
        ...
        async with ClientSession(read, write) as session:
            ...
            if existing:
                ...
                add_args = {
                    ...
                    "use_cases": [
                        "75식 HYPO·금지목록 브리핑",
                        "WORLDVIEW §5 서사↔수학 경계",
                        "Track A·압축 SLA cross-talk 방지",
                    ],
                }
