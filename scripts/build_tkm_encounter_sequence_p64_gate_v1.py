#!/usr/bin/env python3
"""TKM encounter_sequence P64 gate: ultra-grand export bundle vault sync [HYPO]."""
...
OUT = ROOT / "docs/final/artifacts/tkm_encounter_sequence_p64_gate_v1_latest.json"
...
def build() -> dict[str, Any]:
    ...
    sync = _load(SYNC)
    weekly = _load(WEEKLY)
    ugeb = weekly.get("ultra_grand_export_bundle_vault_sync_kpi") if isinstance(
        weekly.get("ultra_grand_export_bundle_vault_sync_kpi"), dict
    ) else {}
    vault = sync.get("vault_mirror") if isinstance(sync.get("vault_mirror"), dict) else {}

    checks = {
        "p63_gate_ok": {"passed": p63.get("gate_ok") is True},
        "ultra_grand_export_bundle_vault_sync_ok": {
            "passed": sync.get("ultra_grand_export_bundle_vault_sync_ok") is True
        },
        "sync_artifact_mirrored": {"passed": ARTIFACT.is_file()},
        "manifest_artifact_present": {"passed": MANIFEST.is_file()},
        "ultra_grand_post_export_closure_ok": {"passed": sync.get("ultra_grand_post_export_closure_ok") is True},
        ...
        "weekly_ultra_grand_export_vault_sync_ok": {
            "passed": ugeb.get("ultra_grand_export_bundle_vault_sync_headline_ok") is True
        },
    ...
    return {
        "schema": "tkm_encounter_sequence_p64_gate_v1",
        "version": "1.0.0",
        ...
        "send_gate": "HOLD",
        "research_only": True,
        "tkm_encounter_sequence_p64_status": "ultra_grand_export_bundle_vault_sync_wire_ok" if gate_ok else "incomplete",
        "gate_ok": gate_ok,
        "checks": checks,
...
def main() -> int:
    ...
    print(json.dumps({"ok": doc["gate_ok"], "status": doc["tkm_encounter_sequence_p64_status"]}))
