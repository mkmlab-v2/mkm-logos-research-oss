...
<#
.SYNOPSIS
  Import and publish dedicated n8n webhook workflow for Logos Research pilot leads.

.EXAMPLE
  powershell -NoProfile -ExecutionPolicy Bypass -File scripts\Register-LogosResearchLeadN8nWebhook_v1.ps1 -WriteWorkspaceEnv
#>
param(
...
$workflowName = "logos-research-lead-webhook"
$workflowId = "LrLdWhk001Pilot1"
$webhookPath = "logos-research-lead"
$workflowJson = Join-Path $root "projects\no1kmedi\ops\n8n\logos_research_lead_webhook_v1.json"
...
function Resolve-DedicatedWebhookUrl {
    $explicit = Get-EnvAny "LOGOS_RESEARCH_LEAD_WEBHOOK_URL"
    if (-not $explicit) { $explicit = Read-DotEnvKey $workspaceEnv "LOGOS_RESEARCH_LEAD_WEBHOOK_URL" }
...
Write-Host "[logos-lead-n8n] dedicated webhook (host only): $($dedicatedUrl -replace '/webhook/.*','/webhook/...')" -ForegroundColor Cyan

if ($WhatIfOnly) {
    Write-Host "[logos-lead-n8n] WhatIf: would import $workflowName on VPS and probe webhook"
    exit 0
}
...
$remoteJson = "/tmp/logos_research_lead_webhook_v1.json"
& scp @($sshArgs + @($workflowJson, "${remote}:${remoteJson}"))
if ($LASTEXITCODE -ne 0) { throw "scp workflow json failed exit $LASTEXITCODE" }

$remoteCmd = @'
set -e
docker cp '__REMOTE_JSON__' n8n:/tmp/logos_research_lead_webhook_v1.json
docker exec n8n n8n import:workflow --input=/tmp/logos_research_lead_webhook_v1.json
docker exec n8n n8n publish:workflow --id=__WORKFLOW_ID__ || true
docker restart n8n >/dev/null
sleep 10
echo '[logos-lead-n8n] n8n restarted'
'@ -replace '__REMOTE_JSON__', $remoteJson `
   -replace '__WORKFLOW_ID__', $workflowId

Write-Host "[logos-lead-n8n] VPS import/publish/probe" -ForegroundColor Cyan
...
$probeScript = @"
fetch('$dedicatedUrl',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({probe:true,lead_id:'probe_local',email:'probe@example.com',organization:'Probe',tier:'pilot',send_gate:'HOLD'})})
...
if ($WriteWorkspaceEnv) {
    Upsert-DotEnvKey $workspaceEnv "LOGOS_RESEARCH_LEAD_WEBHOOK_URL" $dedicatedUrl
    Write-Host "[logos-lead-n8n] updated $workspaceEnv LOGOS_RESEARCH_LEAD_WEBHOOK_URL" -ForegroundColor Green
}

Write-Host "[logos-lead-n8n] OK" -ForegroundColor Green
