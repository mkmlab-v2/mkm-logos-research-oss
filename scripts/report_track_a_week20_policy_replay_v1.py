...
# Balance: 93
# Purpose: Consolidate Week-20 experiments into final policy replay decision.
# Keywords: track_a, week20, policy_replay, decision, commercialization
#!/usr/bin/env python3
...
E1 = ROOT / "docs" / "final" / "artifacts" / "track_a_week20_staged_guard_unlock_sweep_v1.json"
E2 = ROOT / "docs" / "final" / "artifacts" / "track_a_week20_confidence_first_crossover_sweep_v1.json"
E3 = ROOT / "docs" / "final" / "artifacts" / "track_a_week20_widened_domain_budget_veto_sweep_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week20_policy_replay_v1.json"
...
def _read_e4_config(pack: dict[str, Any]) -> tuple[float, bool]:
    ...
    e4 = next((x for x in exps if str(x.get("id")) == "W20-E4"), {})
...
def main() -> int:
    ...
    decision = (
        "GO_W20_POLICY_REPLAY"
        if policy_floor_ok and integrity_ok and quality_tradeoff_ok
        else "HOLD_W20_POLICY_REPLAY"
    )

    out_doc = {
        "schema": "track_a_week20_policy_replay_v1",
        ...
        "week20_summary": {
            "entry_decision": (pack.get("entry_condition") or {}).get("week19_decision"),
        ...
        "decision": decision,
        "next_action": (
            "Keep Track A commercialization HOLD; register Week-21 hypothesis pack when ready."
            if decision == "HOLD_W20_POLICY_REPLAY"
