"""Plan and apply prospect → production zone_h_en_business template catalog merge."""
...
from scripts.extract_zone_h_en_business_template_seeds_v1_lib import normalize_snippet, snippet_hash

_TEMPLATE_ID_RE = re.compile(r"^eb_t(\d+)$", re.IGNORECASE)
_PROSPECT_ID_RE = re.compile(r"^eb_p(\d+)$", re.IGNORECASE)
...
def next_production_template_id(existing_rows: list[dict[str, Any]]) -> str:
    ...
    for row in existing_rows:
        ...
        if n is not None:
            max_n = max(max_n, n)
    return f"eb_t{max_n + 1:03d}"


def max_prospect_index(prospect_rows: list[dict[str, Any]], *, prefix: str = "eb_p") -> int:
    max_n = 0
...
def production_row_from_prospect(prospect: dict[str, Any], *, template_id: str) -> dict[str, Any]:
    row: dict[str, Any] = {
        ...
        "shard_id": str(prospect.get("shard_id") or "zone_h_en_business_v1"),
        "language": str(prospect.get("language") or "en"),
