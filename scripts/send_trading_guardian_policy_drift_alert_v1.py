...
def _resolve_webhook(cli_webhook: str, workspace_root: Path) -> str:
    ...
    env_value = os.getenv("TRADING_GUARDIAN_POLICY_WEBHOOK_URL", "").strip()
    if env_value:
        return env_value
    dotenv = workspace_root / ".env"
    dot_value = _read_dotenv_value(dotenv, "TRADING_GUARDIAN_POLICY_WEBHOOK_URL")
    if dot_value:
        return dot_value
...
def main() -> int:
    ...
    drift = _read_json(root / args.drift_json)
    if not drift:
        print("trading_guardian_policy_alert_skipped=missing_drift_report")
        return 0
    ...
    if not webhook:
        print("trading_guardian_policy_alert_skipped=missing_webhook")
        return 0

    payload = {
        "schema": "trading_guardian_policy_drift_alert_v1",
        "ts_utc": datetime.now(timezone.utc).isoformat(),
        "alert": "TRIGGERED",
        "status": drift.get("status"),
        "policy_path": drift.get("policy_path"),
        "baseline_sha256": drift.get("baseline_sha256"),
        "current_sha256": drift.get("current_sha256"),
    }
    try:
