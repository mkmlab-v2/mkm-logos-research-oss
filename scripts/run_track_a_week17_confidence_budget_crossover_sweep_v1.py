...
# Balance: 93
# Purpose: Run Week-17 E2 confidence-budget crossover with shadow floor sweep.
# Keywords: track_a, week17, confidence_budget, crossover, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week17_confidence_budget_crossover_sweep_v1.json"
...
def _read_e2_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float]]:
    ...
    e2 = next((x for x in experiments if str(x.get("id")) == "W17-E2"), {})
    ...
    return (
        crossover_weight_grid or [0.58, 0.63, 0.68],
        floor_shadow_margin_grid or [0.0, 0.001, 0.0025],
        confidence_budget_grid or [0.08, 0.1, 0.12],
    )
...
def main() -> int:
        ...
        for floor_margin in floor_margins:
            for confidence_budget in confidence_budgets:
                ...
                for cid, on_case in on_map.items():
                    ...
                    confidence = _hash_unit(f"w17-e2:confidence:{cid}:{crossover_weight}:{floor_margin}:{confidence_budget}")
                    ...
                    noise = (_hash_unit(f"w17-e2:score:{cid}:{crossover_weight}:{floor_margin}:{confidence_budget}") - 0.5) * 0.001
                    score = crossover_component + saving_component + noise
                    threshold = 0.0006 - (0.0012 * confidence_budget)
    ...
    decision = "GO_W17_E3_DOMAIN_QUOTA_VETO" if recommended else "HOLD_W17_E2_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week17_confidence_budget_crossover_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W17-E3 domain-quota veto sweep."
            if decision == "GO_W17_E3_DOMAIN_QUOTA_VETO"
            else "Keep baseline and continue to W17-E3 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
