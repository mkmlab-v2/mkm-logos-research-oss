#!/usr/bin/env python3
"""Sasang rail P11: curated apply + enrich refresh + KOSPI timeseries probe [HYPO]."""
...
PY = sys.executable
OUT = ROOT / "reports/sasang_rail_p11_chain_v1_latest.json"
...
def main() -> int:
    ...
    ap.add_argument("--skip-p10-refresh", action="store_true")
    ap.add_argument("--apply-promote", action="store_true", help="Ingest curated rows into benchmark")
    ap.add_argument("--out", type=Path, default=OUT)
    args = ap.parse_args()

    steps: list[dict[str, Any]] = []
    if not args.skip_p10_refresh:
        steps.append(_run("p10_refresh", "run_sasang_rail_p10_chain_v1.py", ["--skip-pytest", "--skip-p9-refresh"]))

    if not steps or all(s["ok"] for s in steps):
        steps.append(_run("literature_enrich_refresh", "run_sasang_literature_auto_enrich_refresh_chain_v1.py"))
    ...
    if steps and all(s["ok"] for s in steps):
        ...
        steps.append(
            {
                "name": "protocol_4agent_timeseries_kospi",
    ...
    if steps and all(s["ok"] for s in steps):
        steps.append(_run("p11_gate", "build_sasang_rail_p11_gate_v1.py"))

    if steps and all(s["ok"] for s in steps):
        steps.append(_run("master_gate_p11", "build_sasang_rail_master_gate_v1.py"))

    if steps and all(s["ok"] for s in steps) and not args.skip_pytest:
        for test_path in ("tests/test_sasang_rail_p11_v1.py",):
            t0 = time.perf_counter()
    ...
    p11_path = ROOT / "docs/final/artifacts/sasang_rail_p11_gate_v1_latest.json"
    p11 = json.loads(p11_path.read_text(encoding="utf-8-sig")) if p11_path.is_file() else {}
    ...
    doc = {
        "schema": "sasang_rail_p11_chain_v1",
        "generated_at_utc": _utc(),
        "lane": "track_b_hypo",
        "steps": steps,
        "all_ok": all(s["ok"] for s in steps),
        "p11_gate_ok": p11.get("gate_ok"),
        "sasang_rail_p11_status": p11.get("sasang_rail_p11_status"),
        "master_gate_ok": master.get("gate_ok"),
        "sasang_rail_master_status": master.get("sasang_rail_master_status"),
        "send_gate": "HOLD",
        "reproduce": "py scripts/run_sasang_rail_p11_chain_v1.py --apply-promote",
    }
    ...
    print(
        json.dumps(
            {
                "ok": doc["all_ok"],
                "sasang_rail_p11_status": doc.get("sasang_rail_p11_status"),
