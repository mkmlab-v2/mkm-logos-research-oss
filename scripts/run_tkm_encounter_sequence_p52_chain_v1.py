#!/usr/bin/env python3
"""TKM encounter_sequence P52: NotebookLM export sync chain [HYPO]."""
...
PY = sys.executable
OUT = ROOT / "reports/tkm_encounter_sequence_p52_chain_v1_latest.json"
...
def main() -> int:
    ...
    ap.add_argument("--skip-p51-refresh", action="store_true")
    ...
    if not args.skip_p51_refresh:
        extra = ["--skip-http", "--skip-p50-refresh"]
        if args.with_gpu_interpret_train:
            extra.append("--with-gpu-interpret-train")
        steps.append(_run("p51_refresh", "run_tkm_encounter_sequence_p51_chain_v1.py", extra))
    ...
    if not steps or all(s["ok"] for s in steps):
        steps.extend(
            [
                ...
                _run("p52_gate", "build_tkm_encounter_sequence_p52_gate_v1.py"),
    ...
    gate_path = ROOT / "docs/final/artifacts/tkm_encounter_sequence_p52_gate_v1_latest.json"
    gate = json.loads(gate_path.read_text(encoding="utf-8-sig")) if gate_path.is_file() else {}
    doc = {
        "schema": "tkm_encounter_sequence_p52_chain_v1",
        ...
        "p52_gate_ok": gate.get("gate_ok"),
        "tkm_encounter_sequence_p52_status": gate.get("tkm_encounter_sequence_p52_status"),
        "send_gate": "HOLD",
        "reproduce": "py scripts/run_tkm_encounter_sequence_p52_chain_v1.py --skip-http",
    }
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(doc, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"ok": doc["all_ok"], "status": doc.get("tkm_encounter_sequence_p52_status")}))
