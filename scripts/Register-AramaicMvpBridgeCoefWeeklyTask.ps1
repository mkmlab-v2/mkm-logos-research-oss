...
<#
.SYNOPSIS
  Register (or remove) weekly Scheduled Task: Aramaic regime-shift weight sweep → bridge coef apply.

.DESCRIPTION
  Runs scripts/Run-AramaicMvpBridgeCoefWeeklyChain_v1.ps1. Default: Sunday 08:55 local (after threshold weekly default).
#>
param(
    [switch]$Remove,
    [string]$TaskName = "MKM-AramaicMvp-BridgeCoef-Weekly",
    [string]$SundayAt = "08:55",
...
$runner = Join-Path $WorkspaceRoot "scripts\Run-AramaicMvpBridgeCoefWeeklyChain_v1.ps1"
...
if ($parts.Count -lt 2) {
    throw "SundayAt must be HH:mm (e.g. 08:55), got: $SundayAt"
}
$hour = [int]$parts[0]
$minute = [int]$parts[1]
...
$desc = "Weekly Aramaic regime-shift weight sweep → bridge coefficient recommendation (B-track)."
