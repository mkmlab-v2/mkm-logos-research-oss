...
# Balance: 93
# Purpose: Run Week-7 E1 two-stage architecture sweep.
# Keywords: track_a, week7, two_stage, planner, executor
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week7_two_stage_arch_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[str]]:
    ...
    e1 = next((x for x in experiments if str(x.get("id")) == "W7-E1"), {})
    cfg = e1.get("config") or {}
    planner = [float(x) for x in (cfg.get("planner_budget_grid") or [])]
    executor = [float(x) for x in (cfg.get("executor_budget_grid") or [])]
    locks = [str(x).strip().lower() for x in (cfg.get("risk_domain_lock") or []) if str(x).strip()]
    return planner or [0.1, 0.12, 0.14], executor or [0.32, 0.34, 0.36], locks or ["ssot", "timing"]
...
def main() -> int:
    ...
    planner_grid, executor_grid, risk_locks = _read_e1_config(pack_doc)
    risk_lock_set = set(risk_locks)
    ...
    for planner_budget in planner_grid:
        for executor_budget in executor_grid:
            ...
            for cid, on_case in on_map.items():
                ...
                if domain in risk_lock_set:
                    use_off = False
                else:
                    # planner_budget lowers candidate off ratio; executor_budget restores some saving.
                    off_ratio = max(0.0, min(1.0, executor_budget - planner_budget))
                    use_off = _hash_unit(f"twostage:{domain}:{cid}:{planner_budget}:{executor_budget}") <= off_ratio
            ...
            row = {
                ...
                "risk_domain_lock": sorted(risk_lock_set),
    ...
    decision = "GO_W7_E2_SEMANTIC_CHUNK" if recommended else "HOLD_W7_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week7_two_stage_arch_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "planner_budget_grid": planner_grid,
            "executor_budget_grid": executor_grid,
            "risk_domain_lock": sorted(risk_lock_set),
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W7-E2 semantic chunker sweep."
            if decision == "GO_W7_E2_SEMANTIC_CHUNK"
            else "Keep baseline and continue to W7-E2 architecture module path."
        ),
