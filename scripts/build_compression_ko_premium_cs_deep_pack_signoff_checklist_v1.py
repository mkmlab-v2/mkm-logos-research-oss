#!/usr/bin/env python3
"""Build commander human-review checklist for ko premium cs deep pack signoff."""
...
DEFAULT_ENVELOPE = ROOT / "docs/final/artifacts/compression_ko_premium_cs_deep_pack_promotion_signoff_envelope_v1_latest.json"
DEFAULT_FALLBACK = ROOT / "docs/final/artifacts/compression_ko_premium_cs_deep_pack_fallback_spec_v1_latest.json"
DEFAULT_SPEC = ROOT / "docs/final/artifacts/compression_ko_premium_cs_deep_pack_spec_v1_latest.json"
DEFAULT_OUT = ROOT / "docs/final/artifacts/compression_ko_premium_cs_deep_pack_signoff_checklist_v1_latest.json"
TEMPLATE_MANIFEST = ROOT / "codebook/templates/zone_ko_premium_cs_templates_manifest_v1.json"
EN_BUSINESS_GATE = ROOT / "docs/final/artifacts/compression_en_business_deep_pack_gate_v1_latest.json"
...
def _catalog_has_mask_tokens() -> bool:
    ...
    for line in TEMPLATES.read_text(encoding="utf-8").splitlines():
        ...
        if not line:
            continue
        row = json.loads(line)
        if "███" not in str(row.get("snippet") or ""):
            return False
    return True
...
def build_checklist(
    *,
    ...
    coverage_path = (
        ROOT / coverage_rel
        if coverage_rel
        else ROOT / "docs/final/artifacts/zone_ko_premium_cs_template_catalog_coverage_v1_latest.json"
    )
    ...
    checklist = {
        ...
        "wire_family_cs_mask": str(gate.get("wire_family") or "") == "CS_MASK",
        "roundtrip_path_template_catalog_wire_v1": roundtrip_path == "template_catalog_wire_v1",
        "mask_token_in_all_catalog_snippets": _catalog_has_mask_tokens(),
        ...
        and fallback.get("schema") == "compression_ko_premium_cs_deep_pack_fallback_spec_v1",
        "vertical_spec_present": spec_path.is_file()
        and spec.get("schema") == "compression_ko_premium_cs_deep_pack_spec_v1",
        "template_manifest_present": TEMPLATE_MANIFEST.is_file(),
        "separate_from_en_business_ack": EN_BUSINESS_GATE.is_file(),
        ...
        and coverage.get("schema") == "zone_ko_premium_cs_template_catalog_coverage_v1",
    ...
    return {
        "schema": "compression_ko_premium_cs_deep_pack_signoff_checklist_v1",
        ...
        "commander_review_items": [
            ...
            "CS_MASK wire only — do not merge BIZ_MASK en business or ZF_MASK coding headlines.",
            "███ masked tokens must round-trip byte-exact; wtt shortcap axis stays separate.",
        ...
        "evidence_paths": {
            ...
            "en_business_gate_separation": _rel(EN_BUSINESS_GATE),
        ...
        "btrack_coverage_note": (
            "Wire-match coverage on KO premium CS JSONL corpora is B-track evidence only — not customer SLA."
        ),
        "operator_action": {
        ...
        "reproduce": [
            ...
            "py -m pytest tests/test_compression_ko_premium_cs_deep_pack_v1.py tests/test_build_compression_ko_premium_cs_deep_pack_signoff_checklist_v1.py -q",
        ],
    }


def main() -> int:
    ap = argparse.ArgumentParser(description="Build ko premium cs deep pack commander signoff checklist")
