...
# Balance: 93
# Purpose: Consolidate Week-9 experiments into final policy replay decision.
# Keywords: track_a, week9, policy_replay, decision, commercialization
#!/usr/bin/env python3
...
E1 = ROOT / "docs" / "final" / "artifacts" / "track_a_week9_domain_frozen_router_mix_sweep_v1.json"
E2 = ROOT / "docs" / "final" / "artifacts" / "track_a_week9_jaccard_prior_rerank_sweep_v1.json"
E3 = ROOT / "docs" / "final" / "artifacts" / "track_a_week9_confidence_band_fallback_sweep_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week9_policy_replay_v1.json"
...
def _read_e4_config(pack: dict[str, Any]) -> tuple[float, bool]:
    ...
    e4 = next((x for x in exps if str(x.get("id")) == "W9-E4"), {})
...
def main() -> int:
    ...
    decision = (
        ...
        else "HOLD_W9_POLICY_REPLAY"
    )

    out_doc = {
        "schema": "track_a_week9_policy_replay_v1",
        ...
        "week9_summary": {
            "entry_decision": (pack.get("entry_condition") or {}).get("week8_decision"),
        ...
        "decision": decision,
        "next_action": (
            "Keep Track A commercialization HOLD and design Week-10 hypothesis pack."
            if decision == "HOLD_W9_POLICY_REPLAY"
