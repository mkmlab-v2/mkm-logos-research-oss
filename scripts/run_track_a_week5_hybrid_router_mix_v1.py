...
# Balance: 93
# Purpose: Run Week-5 E3 hybrid router mix sweep.
# Keywords: track_a, week5, hybrid_router, sweep, routing
#!/usr/bin/env python3
...
PACK = ROOT / "docs" / "final" / "artifacts" / "track_a_week5_engine_routing_pack_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week5_hybrid_router_mix_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[str], list[str], list[float]]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e3 = next((x for x in experiments if str(x.get("id")) == "W5-E3"), {})
    cfg = e3.get("config") or {}
    hard_on = [str(x).strip().lower() for x in (cfg.get("hard_router_on_domains") or []) if str(x).strip()]
    soft = [str(x).strip().lower() for x in (cfg.get("soft_penalty_domains") or []) if str(x).strip()]
    alphas = [float(x) for x in (cfg.get("penalty_alpha_grid") or [])]
    return hard_on or ["ssot", "timing"], soft or ["general"], alphas or [0.2, 0.3, 0.4]
...
def main() -> int:
    ...
    ap.add_argument("--alpha-grid", type=str, default="")
    ap.add_argument("--saving-floor", type=float, default=0.485)
    ap.add_argument("--jaccard-floor", type=float, default=0.84)
    ...
    hard_on_domains, soft_domains, default_alphas = _read_e3_config(pack_doc)
    alphas = [float(x.strip()) for x in args.alpha_grid.split(",") if x.strip()] or default_alphas

    hard_on_set = set(hard_on_domains)
    soft_set = set(soft_domains)
    ...
    for alpha in alphas:
        ...
        for cid, on_case in on_map.items():
            ...
            if domain in hard_on_set:
                use_off = False
            elif domain in soft_set:
                # alpha↑ => stronger router_on bias
                off_ratio_target = max(0.0, min(1.0, 1.0 - alpha))
                use_off = _hash_unit(f"soft:{domain}:{cid}") <= off_ratio_target
            else:
                off_ratio_target = max(0.0, min(1.0, 1.0 - (alpha * 0.8)))
                use_off = _hash_unit(f"other:{domain}:{cid}") <= off_ratio_target
        ...
        row = {
            "alpha": alpha,
            "hard_router_on_domains": sorted(hard_on_set),
            "soft_penalty_domains": sorted(soft_set),
    ...
    decision = "GO_W5_E4_POLICY_REPLAY" if recommended else "HOLD_W5_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week5_hybrid_router_mix_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W5-E4 policy replay with hybrid routing candidate."
            if decision == "GO_W5_E4_POLICY_REPLAY"
            else "Keep baseline router-on and proceed to W5-E4 replay as HOLD evidence."
        ),
