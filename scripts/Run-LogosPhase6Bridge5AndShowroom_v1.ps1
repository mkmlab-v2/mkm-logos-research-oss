# Phase6: Gemini bridge #5 + commander signoff + router/CDIM + showroom smoke + VPS (no RefreshStaging)
param(
    [switch]$DryRunGemini,
...
Write-Host "==> Phase6: Gemini bridge #5 (regime watchfulness)" -ForegroundColor Cyan
$geminiArgs = @("scripts/build_logos_concept_bridge_regime_watchfulness_gemini_v1.py")
if ($DryRunGemini) {
    $geminiArgs = @(
        "scripts/build_logos_concept_bridge_gemini_v1.py",
        "--concept-ko", "체제 전환 속 깨어 있음과 경계",
        "--concept-id", "concept:regime_transition_watchfulness",
        "--output-json", "docs/final/artifacts/logos_concept_bridge_regime_watchfulness_gemini_v1_latest.json",
        "--dry-run"
    )
}
& $py @geminiArgs
if ($LASTEXITCODE -ne 0) { throw "bridge #5 failed" }
...
Write-Host "==> commander signoff (bridges + gold)" -ForegroundColor Cyan
...
& $py scripts/mark_logos_rag_gold_commander_direct_signoff_v1.py
if ($LASTEXITCODE -ne 0) { throw "gold signoff failed" }
& $py @regArgs

Write-Host "==> q01 theology lock (no union revert)" -ForegroundColor Cyan
...
& $py scripts/run_logos_subgraph_graphrag_router_v1.py --query-id q01 --top-bridges 5
...
Write-Host "==> showroom v6 smoke" -ForegroundColor Cyan
...
Write-Host "==> Phase6 done" -ForegroundColor Green
