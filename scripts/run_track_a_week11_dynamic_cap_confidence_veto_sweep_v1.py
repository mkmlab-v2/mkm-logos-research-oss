...
# Balance: 93
# Purpose: Run Week-11 E3 dynamic-cap confidence-veto sweep.
# Keywords: track_a, week11, dynamic_cap, confidence_veto, sweep
#!/usr/bin/env python3
...
PACK = ROOT / "docs" / "final" / "artifacts" / "track_a_week11_hypothesis_pack_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week11_dynamic_cap_confidence_veto_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float]]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e3 = next((x for x in experiments if str(x.get("id")) == "W11-E3"), {})
    cfg = e3.get("config") or {}
    dynamic_cap_grid = [float(x) for x in (cfg.get("dynamic_cap_grid") or [])]
    confidence_veto_grid = [float(x) for x in (cfg.get("confidence_veto_grid") or [])]
    rollback_penalty_grid = [float(x) for x in (cfg.get("rollback_penalty_grid") or [])]
    return (
        dynamic_cap_grid or [0.5, 0.52, 0.54],
        confidence_veto_grid or [0.62, 0.66, 0.7],
        rollback_penalty_grid or [0.03, 0.05, 0.07],
    )
...
def main() -> int:
    ...
    dynamic_caps, confidence_vetos, rollback_penalties = _read_e3_config(pack_doc)
    ...
    for dcap in dynamic_caps:
        for veto in confidence_vetos:
            for penalty in rollback_penalties:
                selected: list[dict[str, Any]] = []
                switched_to_off = 0
                veto_blocked = 0
                rollback_to_on = 0
                total = 0
                effective_cap = max(args.saving_floor, min(0.56, dcap))
                for cid, on_case in on_map.items():
                    ...
                    confidence = _hash_unit(f"w11-e3:conf:{cid}:{veto}")
                    if confidence >= veto:
                        selected.append(on_case)
                        veto_blocked += 1
                        total += 1
                    ...
                    off_score = off_s - max(0.0, off_s - effective_cap) * (1.0 + penalty)
                    on_score = on_s - max(0.0, on_s - effective_cap) * (1.0 + penalty)
                    off_score += (off_j - on_j) * 0.4
                    use_off = off_score >= on_score

                    overshoot = max(0.0, off_s - effective_cap)
                    if use_off and overshoot > penalty:
                        use_off = False
                        rollback_to_on += 1
                ...
                row = {
                    "dynamic_cap": dcap,
                    "effective_dynamic_cap": effective_cap,
                    "confidence_veto": veto,
                    "rollback_penalty": penalty,
                    "switched_to_router_off_cases": switched_to_off,
                    "confidence_veto_blocked_cases": veto_blocked,
                    "rollback_to_router_on_cases": rollback_to_on,
    ...
    decision = "GO_W11_E4_POLICY_REPLAY" if recommended else "HOLD_W11_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week11_dynamic_cap_confidence_veto_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "dynamic_cap_grid": dynamic_caps,
            "confidence_veto_grid": confidence_vetos,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W11-E4 policy replay."
            if decision == "GO_W11_E4_POLICY_REPLAY"
            else "Keep baseline and continue to W11-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
