# q07 insight chain — CPU ANN + typology boost (covenant love/forgiveness).
param(
    [switch]$SkipAnnLite,
    [switch]$DryRun
)
...
Write-Host "[q07] CPU-only insight rebuild (covenant love/forgiveness typology)" -ForegroundColor Cyan
& py @args
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
Write-Host "[OK] q07 insight + gold eval" -ForegroundColor Green
