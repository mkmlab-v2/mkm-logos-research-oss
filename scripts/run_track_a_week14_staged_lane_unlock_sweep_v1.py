...
# Balance: 93
# Purpose: Run Week-14 E1 staged lane unlock with strict guard sweep.
# Keywords: track_a, week14, staged_unlock, strict_guard, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week14_staged_lane_unlock_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[str]]:
    ...
    e1 = next((x for x in experiments if str(x.get("id")) == "W14-E1"), {})
    cfg = e1.get("config") or {}
    stage_unlock_ratio_grid = [float(x) for x in (cfg.get("stage_unlock_ratio_grid") or [])]
    ...
    return (
        stage_unlock_ratio_grid or [0.1, 0.12, 0.14],
...
def main() -> int:
    ...
    stage_unlock_ratios, jaccard_guards, protected_domains = _read_e1_config(pack_doc)
    ...
    for stage_unlock_ratio in stage_unlock_ratios:
        for jaccard_guard in jaccard_guards:
            ...
            for cid, on_case in on_map.items():
                ...
                if off_j < max(jaccard_guard, on_j - 0.0015):
                    ...
                    total += 1
                    continue
                stage_signal = _hash_unit(f"w14-e1:stage:{cid}:{stage_unlock_ratio}:{jaccard_guard}")
                use_off = stage_signal <= stage_unlock_ratio
                if not use_off:
                    stage_blocked += 1
            ...
            row = {
                "stage_unlock_ratio": stage_unlock_ratio,
                ...
                "stage_blocked_cases": stage_blocked,
    ...
    decision = "GO_W14_E2_REPLAY_ANNEAL" if recommended else "HOLD_W14_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week14_staged_lane_unlock_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "stage_unlock_ratio_grid": stage_unlock_ratios,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W14-E2 replay anneal floor-shadow sweep."
            if decision == "GO_W14_E2_REPLAY_ANNEAL"
            else "Keep baseline and continue to W14-E2 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
