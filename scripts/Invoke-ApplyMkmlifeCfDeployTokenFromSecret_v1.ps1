...
<#
.SYNOPSIS
  Apply mkmlife CF deploy token (Workers+KV) from local secret JSON -> .env deploy keys.
#>
param(
    [string]$SecretJson = "",
...
if ([string]::IsNullOrWhiteSpace($SecretJson)) {
    $SecretJson = Join-Path $WorkspaceRoot "reports\cloudflare_mkmlife_deploy_token_secret_LOCAL.json"
}
...
$tok = [string]($doc.MKM_MKMLIFE_CF_DEPLOY_TOKEN)
...
$keys = @("MKM_MKMLIFE_CF_DEPLOY_TOKEN", "CLOUDFLARE_API_TOKEN")
$out = New-Object System.Collections.Generic.List[string]
$done = @{}
foreach ($line in $lines) {
    if ($line -match '^\s*(MKM_MKMLIFE_CF_DEPLOY_TOKEN|CLOUDFLARE_API_TOKEN)=') {
...
Write-Host "Updated .env (MKM_MKMLIFE_CF_DEPLOY_TOKEN + CLOUDFLARE_API_TOKEN)" -ForegroundColor Green
