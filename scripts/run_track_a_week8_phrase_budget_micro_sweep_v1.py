...
# Balance: 93
# Purpose: Run Week-8 E1 domain-locked phrase budget micro sweep.
# Keywords: track_a, week8, phrase_budget, micro_tuning, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week8_phrase_budget_micro_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[str]]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e1 = next((x for x in experiments if str(x.get("id")) == "W8-E1"), {})
    cfg = e1.get("config") or {}
    budget_grid = [float(x) for x in (cfg.get("phrase_budget_grid") or [])]
    target_domains = [str(x).strip().lower() for x in (cfg.get("target_domains") or []) if str(x).strip()]
    return budget_grid or [0.04, 0.06, 0.08], target_domains or ["ssot", "timing"]
...
def main() -> int:
    ...
    budget_grid, target_domains = _read_e1_config(pack_doc)
    ...
    for budget in budget_grid:
        ...
        for cid, on_case in on_map.items():
            ...
            if domain in target_set:
                # Domain-locked phrase budget: force router_on for protected domains.
                selected.append(on_case)
                total += 1
                continue
            off_ratio = max(0.0, min(1.0, budget * 2.0))
            use_off = _hash_unit(f"w8-e1:{cid}:{domain}:{budget}") <= off_ratio
        ...
        row = {
            ...
            "target_domains": sorted(target_set),
    ...
    decision = "GO_W8_E2_CONFIDENCE_RESTORE" if recommended else "HOLD_W8_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week8_phrase_budget_micro_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "phrase_budget_grid": budget_grid,
            "target_domains": sorted(target_set),
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W8-E2 confidence restore gate sweep."
            if decision == "GO_W8_E2_CONFIDENCE_RESTORE"
            else "Keep baseline and continue to W8-E2 confidence restore path."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
