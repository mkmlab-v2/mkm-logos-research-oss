...
# Balance: 93
# Purpose: Run Week-7 E3 dual-objective pareto scorer sweep.
# Keywords: track_a, week7, pareto, scorer, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week7_pareto_scorer_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], float]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e3 = next((x for x in experiments if str(x.get("id")) == "W7-E3"), {})
    cfg = e3.get("config") or {}
    sw = [float(x) for x in (cfg.get("saving_weight_grid") or [])]
    jw = [float(x) for x in (cfg.get("jaccard_weight_grid") or [])]
    integ_gate = float(cfg.get("integrity_hard_gate", 1.0))
    return sw or [0.45, 0.5, 0.55], jw or [0.55, 0.5, 0.45], integ_gate
...
def main() -> int:
    ...
    saving_weights, jaccard_weights, integrity_gate = _read_e3_config(pack_doc)
    ...
    for sw in saving_weights:
        for jw in jaccard_weights:
            ...
            for cid, on_case in on_map.items():
                ...
                if not off_case:
                    ...
                    total += 1
                    continue
                # Higher saving weight increases off usage; higher jaccard weight decreases off usage.
                off_ratio = max(0.0, min(1.0, sw - (jw * 0.6)))
                use_off = _hash_unit(f"pareto:{cid}:{sw}:{jw}") <= off_ratio
            ...
            score = (sw * mixed["global_token_saving_rate"]) + (jw * mixed["avg_reconstruction_fidelity_jaccard"])
            row = {
                "saving_weight": sw,
                "jaccard_weight": jw,
                "pareto_score": score,
                ...
                "gate": {
                    ...
                    "integrity_floor_ok": mixed["avg_sensitive_integrity"] >= max(args.integrity_floor, integrity_gate),
                },
            }
            runs.append(row)

    viable = [r for r in runs if all(r["gate"].values())]
    recommended = sorted(viable, key=lambda r: (r["pareto_score"], r["metrics"]["avg_reconstruction_fidelity_jaccard"], r["metrics"]["global_token_saving_rate"]), reverse=True)[0] if viable else None
    decision = "GO_W7_E4_POLICY_REPLAY" if recommended else "HOLD_W7_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week7_pareto_scorer_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "saving_weight_grid": saving_weights,
            "jaccard_weight_grid": jaccard_weights,
            "integrity_hard_gate": integrity_gate,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W7-E4 policy replay with pareto winner."
            if decision == "GO_W7_E4_POLICY_REPLAY"
            else "Keep baseline and proceed to W7-E4 replay as HOLD evidence."
        ),
        "runs": sorted(runs, key=lambda r: (r["pareto_score"], r["metrics"]["avg_reconstruction_fidelity_jaccard"]), reverse=True),
    }
