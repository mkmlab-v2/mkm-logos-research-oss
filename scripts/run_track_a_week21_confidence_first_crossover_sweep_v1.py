...
# Balance: 93
# Purpose: Run Week-21 E2 confidence-first crossover with micro shadow floor sweep.
# Keywords: track_a, week21, confidence_first, crossover, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week21_confidence_first_crossover_sweep_v1.json"
...
def _read_e2_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float]]:
    ...
    e2 = next((x for x in experiments if str(x.get("id")) == "W21-E2"), {})
    ...
    return (
        crossover_weight_grid or [0.55, 0.6, 0.65],
        floor_shadow_margin_grid or [0.0, 0.0002, 0.0008],
...
def main() -> int:
        ...
        for floor_margin in floor_margins:
            for confidence_budget in confidence_budgets:
                ...
                for cid, on_case in on_map.items():
                    ...
                    confidence = _hash_unit(f"w21-e2:confidence:{cid}:{crossover_weight}:{floor_margin}:{confidence_budget}")
                    ...
                    noise = (_hash_unit(f"w21-e2:score:{cid}:{crossover_weight}:{floor_margin}:{confidence_budget}") - 0.5) * 0.001
                    score = crossover_component + saving_component + noise
                    threshold = 0.00045 - (0.0015 * confidence_budget)
    ...
    decision = "GO_W21_E3_WIDENED_DOMAIN_BUDGET_VETO" if recommended else "HOLD_W21_E2_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week21_confidence_first_crossover_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W21-E3 widened domain-budget veto sweep."
            if decision == "GO_W21_E3_WIDENED_DOMAIN_BUDGET_VETO"
            else "Keep baseline and continue to W21-E3 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
