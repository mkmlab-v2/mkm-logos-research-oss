# Phase8: Gemini bridge #7 (wisdom/q04) + signoff + router/CDIM + smoke + closure (VPS optional)
param(
    [switch]$DryRunGemini,
    [switch]$SkipVpsSync,
...
Write-Host "==> Phase8: Gemini bridge #7 (wisdom / q04)" -ForegroundColor Cyan
$geminiArgs = @("scripts/build_logos_concept_bridge_wisdom_uncertainty_gemini_v1.py")
if ($DryRunGemini) { $geminiArgs += "--dry-run" }
& $py @geminiArgs
if ($LASTEXITCODE -ne 0) { throw "bridge #7 failed" }
...
Write-Host "==> GraphRAG router q04 + CDIM" -ForegroundColor Cyan
& $py scripts/run_logos_subgraph_graphrag_router_v1.py --query-id q04 --top-bridges 7
if ($LASTEXITCODE -ne 0) { throw "router q04 failed" }
...
Write-Host "==> Phase8 done" -ForegroundColor Green
