#!/usr/bin/env python3
"""[HYPO] Micro-oscillator test-fold tie-break ablation on frozen KOSPI lens WF stack."""
...
DEFAULT_OUT = ROOT / "reports" / "prophecy_micro_oscillator_tiebreak_ablation_v1_latest.json"
WF = ROOT / "scripts" / "run_prophecy_per_date_combo_walkforward_v1.py"

VARIANTS: list[dict[str, Any]] = [
    {"id": "baseline_no_micro", "args": ["--micro-oscillator-mode", "off"]},
    {"id": "mom_ret1_neutral", "args": ["--micro-oscillator-mode", "mom_ret1"]},
    {"id": "revert_ret1_neutral", "args": ["--micro-oscillator-mode", "revert_ret1"]},
    {"id": "mom_ret3_neutral", "args": ["--micro-oscillator-mode", "mom_ret3"]},
    {
        "id": "composite_ret1_ret3_neutral",
        "args": ["--micro-oscillator-mode", "composite_ret1_ret3"],
    },
    {
        "id": "vol_gated_mom_ret1_neutral",
        "args": ["--micro-oscillator-mode", "vol_gated_mom_ret1", "--micro-oscillator-vol-ratio-min", "1.0"],
    },
    {
        "id": "mom_ret1_dz001_neutral",
        "args": ["--micro-oscillator-mode", "mom_ret1", "--micro-oscillator-dz", "0.001"],
    },
    {
        "id": "mom_ret1_override_all",
        "args": ["--micro-oscillator-mode", "mom_ret1", "--no-micro-oscillator-only-neutral"],
    },
    {
        "id": "revert_ret1_override_all",
        "args": ["--micro-oscillator-mode", "revert_ret1", "--no-micro-oscillator-only-neutral"],
    },
]
...
def _run_variant(
    *,
    ...
    out = tmp_dir / f"wf_micro_{variant_id}.json"
    cmd = [
        sys.executable,
        str(WF),
        "--score-json",
    ...
    row.update(
        {
            ...
            "micro_oscillator_dz": inputs.get("micro_oscillator_dz"),
            "micro_oscillator_only_neutral": inputs.get("micro_oscillator_only_neutral"),
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Micro-oscillator tie-break ablation (research).")
    ap.add_argument("--score-json", type=Path, default=DEFAULT_SCORE)
    ap.add_argument("--output", type=Path, default=DEFAULT_OUT)
    ap.add_argument("--n-folds", type=int, default=6)
    ap.add_argument("--tmp-dir", type=Path, default=ROOT / "reports" / "tmp_micro_oscillator_ablation")
    args = ap.parse_args()
    ...
    def _rank_gt(r: dict[str, Any]) -> tuple[float, float]:
        return (
    ...
    best_mean = max(ok_rows, key=lambda r: float(r.get("mean_test_accuracy") or 0.0)) if ok_rows else None

    payload: dict[str, Any] = {
        "schema": "prophecy_micro_oscillator_tiebreak_ablation_v1",
        ...
        "inputs": {
            ...
            "n_folds": int(args.n_folds),
            "frozen_stack": "v1_dual+regime_adaptive_kospi_lens",
            "note": "Micro layer applies on TEST folds only when base pred is neutral (default).",
        },
        "variants": rows,
        "failed_variant_count": len(failed),
        "baseline": baseline,
        "best_by_gt_frac": best_gt,
        "best_by_mean_test_accuracy": best_mean,
        "verdict": None,
    }
    if baseline and best_gt:
        ...
        best_id = str(best_gt.get("variant_id") or "")
        override_all = best_id.endswith("_override_all")
        payload["verdict"] = {
            "gt_frac_lift_vs_baseline": round(gt_lift, 6),
            "mean_delta_best_gt_vs_baseline": round(mean_delta, 6),
            "promote_micro_layer": bool(
                gt_lift > 0.0 and mean_delta >= 0.0 and not override_all
            ),
            "reject_override_all_full_replace": override_all,
            "note": "override_all replaces combo with micro-only; causal ret_1_lagged required.",
        }
    ...
    if best_gt:
        print(
            f"best_gt={best_gt.get('variant_id')} "
            f"gt_frac={best_gt.get('fraction_test_beats_always_bull_gt')} "
            f"mean={best_gt.get('mean_test_accuracy')}",
            flush=True,
        )
