...
# Balance: 93
# Purpose: Run Week-5 E1 penalty-based router scoring sweep.
# Keywords: track_a, week5, penalty_router, sweep, routing
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week5_penalty_router_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[str], list[float]]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e1 = next((x for x in experiments if str(x.get("id")) == "W5-E1"), {})
    cfg = e1.get("config") or {}
    domains = [str(x).strip().lower() for x in (cfg.get("risk_domains") or []) if str(x).strip()]
    penalties = [float(x) for x in (cfg.get("penalty_grid") or [])]
    return domains or ["ssot", "timing"], penalties or [0.05, 0.1, 0.15, 0.2]
...
def main() -> int:
    ...
    risk_domains, default_penalties = _read_e1_config(pack_doc)
    penalties = [float(x.strip()) for x in args.penalty_grid.split(",") if x.strip()] or default_penalties
    risk_domain_set = set(risk_domains)
    ...
    for penalty in penalties:
        ...
        switched_to_off = 0
        total = 0
        off_ratio_target = max(0.0, min(1.0, 1.0 - penalty))
        for cid, on_case in on_map.items():
            ...
            if domain in risk_domain_set:
                selected.append(on_case)
            else:
                use_off = _hash_unit(cid) <= off_ratio_target
        ...
        row = {
            "penalty": penalty,
            "risk_domains": sorted(risk_domain_set),
            "off_ratio_target": off_ratio_target,
    ...
    decision = "GO_W5_E2_CONFIDENCE_ROUTER" if recommended else "HOLD_W5_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week5_penalty_router_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W5-E2 adaptive confidence router sweep."
            if decision == "GO_W5_E2_CONFIDENCE_ROUTER"
            else "Keep baseline router-on and continue to W5-E2 for alternate scoring path."
        ),
