...
def main():
    ...
    survivor_ids=[str(x.get('candidate_id')) for x in surv if isinstance(x,dict) and x.get('candidate_id')]
    report={'schema':'bible_meaning_knowledge_ip_report_v1','generated_at_utc':now(),'research_only':True,'promotion_required':True,'source_track':'K','purpose':'knowledge_ip_only','not_for_trading_signal':True,'summary':{'candidate_count':len(cands),'survivor_count':len(surv)},'survivor_ids':survivor_ids,'survivors':[{'candidate_id':x.get('candidate_id'),'source_node_id':x.get('source_node_id'),'fusion_candidate_score':x.get('fusion_candidate_score'),'ci_low_defense_contrib':x.get('ci_low_defense_contrib')} for x in surv if isinstance(x,dict)],'storylines':[{'title':'Cross-reference cluster narrative','confidence':'medium'}]}
