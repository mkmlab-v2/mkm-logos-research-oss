...
# Balance: 93
# Purpose: Run Week-15 E2 confidence replay crossover with floor shadow sweep.
# Keywords: track_a, week15, confidence_crossover, floor_shadow, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week15_confidence_replay_crossover_sweep_v1.json"
...
def _read_e2_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[int]]:
    ...
    e2 = next((x for x in experiments if str(x.get("id")) == "W15-E2"), {})
...
def main() -> int:
        ...
        for floor_margin in floor_margins:
            for candidate_pool in candidate_pools:
                ...
                for cid, on_case in on_map.items():
                    ...
                    noise = (_hash_unit(f"w15-e2:{cid}:{crossover_weight}:{floor_margin}:{candidate_pool}") - 0.5) * 0.001
    ...
    decision = "GO_W15_E3_ADAPTIVE_THROTTLE_VETO" if recommended else "HOLD_W15_E2_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week15_confidence_replay_crossover_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W15-E3 adaptive throttle veto sweep."
            if decision == "GO_W15_E3_ADAPTIVE_THROTTLE_VETO"
            else "Keep baseline and continue to W15-E3 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
