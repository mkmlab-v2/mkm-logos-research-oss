"""Plan and apply prospect → production zone_ko_premium_cs template catalog merge."""
...
from scripts.extract_zone_ko_premium_cs_template_seeds_v1_lib import normalize_snippet, snippet_hash

_TEMPLATE_ID_RE = re.compile(r"^kcs_t(\d+)$", re.IGNORECASE)
_PROSPECT_ID_RE = re.compile(r"^kcs_p(\d+)$", re.IGNORECASE)
...
def next_production_template_id(existing_rows: list[dict[str, Any]]) -> str:
    ...
    for row in existing_rows:
        ...
        if n is not None:
            max_n = max(max_n, n)
    return f"kcs_t{max_n + 1:03d}"


def max_prospect_index(prospect_rows: list[dict[str, Any]], *, prefix: str = "kcs_p") -> int:
    max_n = 0
    for row in prospect_rows:
        tid = str(row.get("template_id") or "")
        m = _PROSPECT_ID_RE.match(tid)
        if m:
...
def production_row_from_prospect(prospect: dict[str, Any], *, template_id: str) -> dict[str, Any]:
    row: dict[str, Any] = {
        ...
        "shard_id": str(prospect.get("shard_id") or "zone_ko_premium_cs_v1"),
        "language": str(prospect.get("language") or "ko"),
        "snippet": normalize_snippet(str(prospect.get("snippet") or "")),
        "must_keep_terms": list(prospect.get("must_keep_terms") or []),
    }
    if prospect.get("template_id"):
        row["merged_from_prospect_id"] = str(prospect.get("template_id"))
    ...
    *,
    ...
    for prospect in prospect_rows:
        ...
        if not snippet:
            ...
            continue
        if "███" not in snippet:
            skipped.append({**prospect, "skip_reason": "missing_mask_token"})
            continue
