...
# Balance: 93
# Purpose: Run Week-17 E1 guard-banded floor unlock sweep.
# Keywords: track_a, week17, guard_banded, floor_unlock, sweep
#!/usr/bin/env python3
...
PACK = ROOT / "docs" / "final" / "artifacts" / "track_a_week17_hypothesis_pack_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week17_guard_banded_unlock_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float], list[str]]:
    ...
    e1 = next((x for x in experiments if str(x.get("id")) == "W17-E1"), {})
    ...
    return (
        unlock_ratio_grid or [0.1, 0.12, 0.14],
        jaccard_guard_grid or [0.844, 0.846, 0.848],
        guard_relax_margin_grid or [0.0, 0.001, 0.002],
...
def main() -> int:
            ...
            for guard_relax_margin in guard_relax_margins:
                ...
                for cid, on_case in on_map.items():
                    ...
                    if off_j < guard_threshold:
                        ...
                        total += 1
                        continue
                    unlock_signal = _hash_unit(f"w17-e1:unlock:{cid}:{unlock_ratio}:{jaccard_guard}:{guard_relax_margin}")
    ...
    decision = "GO_W17_E2_CONFIDENCE_BUDGET_CROSSOVER" if recommended else "HOLD_W17_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week17_guard_banded_unlock_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W17-E2 confidence-budget crossover sweep."
            if decision == "GO_W17_E2_CONFIDENCE_BUDGET_CROSSOVER"
            else "Keep baseline and continue to W17-E2 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
