#!/usr/bin/env python3
"""Build TKM encounter_sequence full post-export closure rollup (P33–P53) [HYPO]."""
...
OUT = ROOT / "reports/tkm_encounter_sequence_full_post_export_closure_v1_latest.json"
ARTIFACT = ROOT / "docs/final/artifacts/tkm_encounter_sequence_full_post_export_closure_v1_latest.json"

GATE_KEYS = tuple(f"p{n}" for n in range(33, 54))
...
def build() -> dict[str, Any]:
    ...
    return {
        "schema": "tkm_encounter_sequence_full_post_export_closure_v1",
        "version": "1.0.0",
        ...
        "non_gating": True,
        "research_only": True,
        "full_post_export_closure_ok": full_post_export_closure_ok,
        "gates": gates,
        ...
        "stack_phases_ko": "P33–P51 integrated → P52 export sync → P53 post-export obs → P54 full post-export closure",
        "note_ko": "P33–P53 full post-export closure [HYPO][NON_GATING]; Track A·auto-training·진단·처방 금지.",
        "reproduce": "py scripts/build_tkm_encounter_sequence_full_post_export_closure_v1.py",
    }
...
def main() -> int:
    ...
    print(json.dumps({"ok": doc.get("full_post_export_closure_ok"), "gates": doc.get("gates_ok_count")}))
    return 0 if doc.get("full_post_export_closure_ok") else 1
