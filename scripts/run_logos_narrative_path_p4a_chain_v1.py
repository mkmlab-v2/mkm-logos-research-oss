#!/usr/bin/env python3
"""P4a — Narrative 60→64 (P4 round-2) + bloom + inter-hop (B-track, HYPO).

Reproducible:
  py scripts/run_logos_narrative_path_p4a_chain_v1.py
"""
...
def main() -> int:
    ...
    parser.add_argument("--skip-drift-baseline", action="store_true")
    ...
    if not args.skip_drift_baseline:
        proc = subprocess.run(
            [
                sys.executable,
        ...
        if proc.returncode != 0:
            ...
            print("OK: drift baseline refreshed at narrative=64")
    ...
    if not args.skip_pytest:
        proc = subprocess.run(
            [sys.executable, "-m", "pytest", "tests/test_logos_narrative_path_p4a_v1.py", "-q"],
            cwd=ROOT,
            check=False,
        )
        if proc.returncode != 0:
            return proc.returncode
        print("OK: pytest p4a narrative")
