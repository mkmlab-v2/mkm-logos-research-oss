...
# Balance: 93
# Purpose: Consolidate Week-5 experiments into final policy replay decision.
# Keywords: track_a, week5, policy_replay, decision, commercialization
#!/usr/bin/env python3
...
E1 = ROOT / "docs" / "final" / "artifacts" / "track_a_week5_penalty_router_sweep_v1.json"
E2 = ROOT / "docs" / "final" / "artifacts" / "track_a_week5_confidence_router_sweep_v1.json"
E3 = ROOT / "docs" / "final" / "artifacts" / "track_a_week5_hybrid_router_mix_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week5_policy_replay_v1.json"
...
def main() -> int:
    ...
    # Quality tradeoff remains true when no week5 experiment passed its own jaccard gate.
    quality_tradeoff_flag = not any(
        bool((doc.get("viable_count", 0) or 0) > 0)
        for doc in (e1, e2, e3)
    )
    ...
    decision = (
        ...
        else "HOLD_W5_POLICY_REPLAY"
    )

    out_doc = {
        "schema": "track_a_week5_policy_replay_v1",
        ...
        "week5_summary": {
            "entry_decision": (pack.get("entry_condition") or {}).get("week4_decision"),
        ...
        "decision": decision,
        "next_action": (
            "Register Week-6 engine-internal compression-rule redesign track."
            if decision == "HOLD_W5_POLICY_REPLAY"
