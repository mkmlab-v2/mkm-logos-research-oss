#!/usr/bin/env python3
"""Push COMPRESSION_BTRACK pack via nlm CLI (preserves source titles).

SSOT pack: reports/notebooklm_compression_btrack_sync_pack_v1/
...
PACK = ROOT / "reports" / "notebooklm_compression_btrack_sync_pack_v1"
URL_FILE = ROOT / "reports" / "notebooklm_compression_btrack_notebook_url_v1.txt"
OUT = ROOT / "reports" / "notebooklm_compression_btrack_push_nlm_latest.json"
...
def main() -> int:
    if not (PACK / "index.json").is_file():
        raise SystemExit("run build_notebooklm_compression_btrack_sync_pack_v1.py first")
    nb = _notebook_id()
    ...
    doc = {"schema": "notebooklm_compression_btrack_push_nlm_latest", "notebook_id": nb, "rows": rows}
