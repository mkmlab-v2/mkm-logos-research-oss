#!/usr/bin/env python3
"""Build TKM encounter_sequence grand export bundle vault sync manifest [HYPO]."""
...
OUT = ROOT / "reports/tkm_encounter_sequence_grand_export_bundle_vault_sync_v1_latest.json"
ARTIFACT = ROOT / "docs/final/artifacts/tkm_encounter_sequence_grand_export_bundle_vault_sync_v1_latest.json"
MANIFEST_OUT = ROOT / "reports/tkm_encounter_sequence_grand_export_manifest_v1_latest.json"
MANIFEST_ARTIFACT = ROOT / "docs/final/artifacts/tkm_encounter_sequence_grand_export_manifest_v1_latest.json"

P57_GATE = ROOT / "docs/final/artifacts/tkm_encounter_sequence_p57_gate_v1_latest.json"
GRAND_CLOSURE = ROOT / "reports/tkm_encounter_sequence_grand_post_export_closure_v1_latest.json"
BASE_SYNC = ROOT / "reports/tkm_encounter_sequence_notebooklm_export_sync_v1_latest.json"
WEEKLY = ROOT / "reports/encounter_sequence_weekly_report_v1_latest.json"

GRAND_EXPORT_REL_PATHS = (
...
def _mirror_to_vault(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    target = vault / "btrack_artifacts_verified" / "tkm_encounter_sequence" / "grand"
...
def build(*, mirror_vault: bool = True) -> dict[str, Any]:
    p57 = _load(P57_GATE)
    grand = _load(GRAND_CLOSURE)
    base_sync = _load(BASE_SYNC)
    weekly = _load(WEEKLY)

    entries = [_file_entry(rel) for rel in GRAND_EXPORT_REL_PATHS]
    all_exist = all(e.get("exists") for e in entries)
    manifest = {
        "schema": "tkm_encounter_sequence_grand_export_manifest_v1",
        "version": "1.0.0",
    ...
    grand_export_bundle_vault_sync_ok = (
        p57.get("gate_ok") is True
        and grand.get("grand_post_export_closure_ok") is True
        and base_sync.get("export_sync_ok") is True
        and all_exist
        ...
        and base_sync.get("cloud_upload_forbidden") is True
    )

    return {
        "schema": "tkm_encounter_sequence_grand_export_bundle_vault_sync_v1",
        "version": "1.0.0",
        "generated_at_utc": _utc(),
        "lane": "track_b_hypo",
        ...
        "non_gating": True,
        "research_only": True,
        "grand_export_bundle_vault_sync_ok": grand_export_bundle_vault_sync_ok,
        "p57_gate_ok": p57.get("gate_ok"),
        "grand_post_export_closure_ok": grand.get("grand_post_export_closure_ok"),
        "base_export_sync_ok": base_sync.get("export_sync_ok"),
        ...
        "gates_ok_count": grand.get("gates_ok_count"),
        "cloud_upload_forbidden": True,
        ...
        "note_ko": "P58 grand export bundle vault sync [HYPO][NON_GATING]; P57 closure + vault grand/ mirror; cloud MCP upload 금지.",
        "reproduce": "py scripts/build_tkm_encounter_sequence_grand_export_bundle_vault_sync_v1.py",
    }
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "ok": doc.get("grand_export_bundle_vault_sync_ok"),
                "files": doc.get("manifest_files_present_count"),
                "vault_skipped": (doc.get("vault_mirror") or {}).get("vault_mirror_skipped"),
            }
        )
    )
    return 0 if doc.get("grand_export_bundle_vault_sync_ok") else 1
