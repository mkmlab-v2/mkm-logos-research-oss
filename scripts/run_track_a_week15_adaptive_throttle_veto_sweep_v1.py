...
# Balance: 93
# Purpose: Run Week-15 E3 adaptive throttle rollback with confidence veto sweep.
# Keywords: track_a, week15, adaptive_throttle, confidence_veto, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week15_adaptive_throttle_veto_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float]]:
    ...
    e3 = next((x for x in experiments if str(x.get("id")) == "W15-E3"), {})
...
def main() -> int:
            ...
            for rollback_penalty in rollback_penalties:
                ...
                for cid, on_case in on_map.items():
                    ...
                    if not off_case:
                        ...
                        total += 1
                        continue
                    confidence = _hash_unit(f"w15-e3:conf:{cid}:{confidence_gate}")
    ...
    decision = "GO_W15_E4_POLICY_REPLAY" if recommended else "HOLD_W15_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week15_adaptive_throttle_veto_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W15-E4 policy replay."
            if decision == "GO_W15_E4_POLICY_REPLAY"
            else "Keep baseline and continue to W15-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
