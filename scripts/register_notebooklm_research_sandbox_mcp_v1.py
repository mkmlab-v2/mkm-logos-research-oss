#!/usr/bin/env python3
"""Register 14 · 리서치 샌드박스 (Universal Lexicon DR) in notebooklm-mcp library."""
...
OUT = ROOT / "reports" / "notebooklm_research_sandbox_mcp_register_latest.json"

NOTEBOOK_MCP_ID = "14-universal-lexicon-dr"
NOTEBOOK_NAME = "14 · 리서치 샌드박스 (Universal Lexicon DR)"
DESCRIPTION = (
    "MKM Tier0/B-track research sandbox — Universal Root Lexicon Layer A/B/C, "
    "NSM crosswalk, MDL prune, shallow router metrics. [HYPO] research_only · "
    "send_gate HOLD. NL output must not promote Track A or live trading."
)
    ...
    async with stdio_client(_server_params()) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            for nb in _list_notebooks(listed):
                ...
                if target_uuid and nb_uuid == target_uuid:
                    existing.append(nb)
                elif nb.get("id") == NOTEBOOK_MCP_ID:
                    existing.append(nb)
            ...
            if existing:
                ...
                add_args = {
                    ...
                    "use_cases": [
                        "Tier0 Gemini DR follow-up on Universal Lexicon Matrix",
                        "Layer A/B/C distortion mitigation brainstorming",
                        "NSM↔41k crosswalk design options before bench",
                    ],
                    "tags": [
                        "research_sandbox",
                        "hypo",
...
def main() -> int:
    ...
    ap.add_argument("--no-select", action="store_true", help="Do not switch active notebook")
