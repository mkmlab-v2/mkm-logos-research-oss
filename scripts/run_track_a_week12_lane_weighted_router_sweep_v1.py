...
# Balance: 93
# Purpose: Run Week-12 E1 lane-weighted router allocation sweep.
# Keywords: track_a, week12, lane_weighted, router_allocation, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week12_lane_weighted_router_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[str]]:
    ...
    e1 = next((x for x in experiments if str(x.get("id")) == "W12-E1"), {})
    cfg = e1.get("config") or {}
    lane_weight_grid = [float(x) for x in (cfg.get("lane_weight_grid") or [])]
    ...
    return (
        lane_weight_grid or [0.52, 0.56, 0.6],
...
def main() -> int:
    ...
    lane_weights, jaccard_guards, protected_domains = _read_e1_config(pack_doc)
    ...
    for lane_weight in lane_weights:
        for jaccard_guard in jaccard_guards:
            selected: list[dict[str, Any]] = []
            switched_to_off = 0
            protected_on = 0
            jaccard_guard_blocked = 0
            total = 0
            for cid, on_case in on_map.items():
                ...
                if off_j < max(jaccard_guard, on_j - 0.002):
                    selected.append(on_case)
                    jaccard_guard_blocked += 1
                    total += 1
                    continue
                lane_signal = _hash_unit(f"w12-e1:lane:{cid}:{domain}")
                choose_signal = _hash_unit(f"w12-e1:choose:{cid}:{lane_weight}:{jaccard_guard}")
                effective_weight = max(0.0, min(1.0, lane_weight + (lane_signal - 0.5) * 0.2))
                use_off = choose_signal <= effective_weight
            ...
            row = {
                ...
                "protected_on_cases": protected_on,
                "jaccard_guard_blocked_cases": jaccard_guard_blocked,
    ...
    decision = "GO_W12_E2_QUALITY_TIEBREAK_REPLAY" if recommended else "HOLD_W12_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week12_lane_weighted_router_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "lane_weight_grid": lane_weights,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W12-E2 quality tie-break replay sweep."
            if decision == "GO_W12_E2_QUALITY_TIEBREAK_REPLAY"
            else "Keep baseline and continue to W12-E2 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
