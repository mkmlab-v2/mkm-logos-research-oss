...
# Balance: 88
# Purpose: Evaluate a structural lane-based decoder for swap_typo recovery.
...
PREREG_OUT = ART / "l1_inverse_decoder_swap_typo_lane_decoder_v1_preregister.json"
AB_OUT = ART / "l1_inverse_decoder_swap_typo_lane_decoder_v1_ab_v1.json"
...
def _nearest_vocab(token: str, vocab: set[str], k: int = 3) -> list[str]:
    if not vocab:
        return [token]
...
def _build_typo_lane_candidates(noisy_tokens: list[str], pos_vocab: dict[int, set[str]], cap: int = 32) -> set[str]:
    ...
    for i, tok in enumerate(noisy_tokens):
        ...
        if tok == "OOV_TOKEN" or "X" in tok:
            ...
            alts = _nearest_vocab(tok.replace("X", "A"), vocab, k=3)
...
def _lane_pool(
    ...
    order_lane = _generate_adjacent_transposition_candidates(repaired_tokens, max_steps=3)
    typo_lane = _build_typo_lane_candidates(noisy_tokens, pos_vocab, cap=48)
...
def main() -> int:
    ...
    prereg = {
        "schema": "l1_inverse_decoder_swap_typo_lane_decoder_v1_preregister",
        "generated_at_utc": _now_utc(),
        "research_only": True,
        "hypothesis": "Lane-based candidate generation (order lane + typo lane) can improve swap_typo without mixed regression.",
        "candidate": "swap_typo_lane_decoder_v1",
        "is_structural_new_path": True,
        "isolation": "swap_typo-focused candidate generation; production v4 path unchanged",
        "inputs": {
    ...
    out_doc = {
        "schema": "l1_inverse_decoder_swap_typo_lane_decoder_v1_ab",
        "generated_at_utc": _now_utc(),
        "research_only": True,
        "candidate": "swap_typo_lane_decoder_v1",
        "results": {
        ...
        "gate": {
            "checks": checks,
            "all_ok": all_ok,
            "decision": "GO_LANE_DECODER_V1" if all_ok else "HOLD_V4",
        },
        "artifacts": {
