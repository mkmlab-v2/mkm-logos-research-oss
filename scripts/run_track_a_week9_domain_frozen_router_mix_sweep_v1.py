...
# Balance: 93
# Purpose: Run Week-9 E1 domain-frozen router mix sweep.
# Keywords: track_a, week9, domain_frozen, router_mix, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week9_domain_frozen_router_mix_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[str], float]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e1 = next((x for x in experiments if str(x.get("id")) == "W9-E1"), {})
    cfg = e1.get("config") or {}
    off_ratio_grid = [float(x) for x in (cfg.get("off_ratio_grid") or [])]
    frozen_domains = [str(x).strip().lower() for x in (cfg.get("frozen_domains") or []) if str(x).strip()]
    max_nonrisk_off_ratio = float(cfg.get("max_nonrisk_off_ratio", 0.3))
    return off_ratio_grid or [0.18, 0.22, 0.26], frozen_domains or ["ssot", "timing"], max_nonrisk_off_ratio
...
def main() -> int:
    ...
    off_ratio_grid, frozen_domains, max_nonrisk_off_ratio = _read_e1_config(pack_doc)
    frozen_set = set(frozen_domains)
    ...
    for off_ratio in off_ratio_grid:
        ...
        switched_to_off = 0
        total = 0
        effective_nonrisk_ratio = max(0.0, min(max_nonrisk_off_ratio, off_ratio))
        for cid, on_case in on_map.items():
            ...
            if domain in frozen_set:
                selected.append(on_case)
                total += 1
                continue
            use_off = _hash_unit(f"w9-e1:{cid}:{domain}:{effective_nonrisk_ratio}") <= effective_nonrisk_ratio
        ...
        row = {
            "off_ratio": off_ratio,
            "effective_nonrisk_off_ratio": effective_nonrisk_ratio,
            "frozen_domains": sorted(frozen_set),
            "max_nonrisk_off_ratio": max_nonrisk_off_ratio,
    ...
    decision = "GO_W9_E2_JACCARD_PRIOR_RERANK" if recommended else "HOLD_W9_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week9_domain_frozen_router_mix_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W9-E2 jaccard-prior rerank sweep."
            if decision == "GO_W9_E2_JACCARD_PRIOR_RERANK"
            else "Keep baseline and continue to W9-E2 jaccard-prior rerank path."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
