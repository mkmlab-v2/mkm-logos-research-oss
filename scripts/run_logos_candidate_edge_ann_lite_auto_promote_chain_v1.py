#!/usr/bin/env python3
"""Auto chain: ANN-lite queue approve → signoff → gate → pending JSONL ([HYPO] B-track).
...
SIGNOFF_ANN = ROOT / "docs/final/artifacts/logos_candidate_edge_promotion_signoff_ann_lite_v1_latest.json"
DEFAULT_OUT = ROOT / "docs/final/artifacts/logos_candidate_edge_ann_lite_auto_promote_chain_v1_latest.json"
...
def main() -> int:
    ...
    for label, cmd in (
        ("review_decisions", [sys.executable, str(REVIEW_DECISIONS), "--auto-approve-ann-lite-primary", "--reviewer", args.approver]),
        ("signoff", [sys.executable, str(SIGNOFF), "--lane-id", "ann_lite", "--approver", args.approver]),
        (
            "promotion_gate",
            [
                sys.executable,
    ...
    if exit_code == 0:
        code, tail = _run([sys.executable, str(PROMOTE), "--lane-id", "ann_lite"])
    ...
    doc = {
        "schema": "logos_candidate_edge_ann_lite_auto_promote_chain_v1",
        ...
        "steps": steps,
        "artifacts": {
            "signoff_json": str(SIGNOFF_ANN.relative_to(ROOT)).replace("\\", "/"),
            "pending_jsonl": "docs/final/artifacts/bible_meaning_graph_edges_approved_pending_ann_lite_v1.jsonl",
            "promotion_json": "docs/final/artifacts/logos_candidate_edge_promotion_ann_lite_v1_latest.json",
        },
    }
