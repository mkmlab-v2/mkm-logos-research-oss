...
# Balance: 93
# Purpose: Run Week-19 E3 coupled veto throttle with expanded domain budget sweep.
# Keywords: track_a, week19, expanded_domain_budget, veto, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week19_expanded_domain_budget_veto_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float], list[int]]:
    ...
    e3 = next((x for x in experiments if str(x.get("id")) == "W19-E3"), {})
    ...
    return (
        confidence_gate_grid or [0.51, 0.55, 0.59],
        throttle_quota_grid or [0.14, 0.16, 0.18],
        rollback_penalty_grid or [0.03, 0.045, 0.06],
        domain_quota_cap_grid or [3, 4, 5],
    )
...
def main() -> int:
                ...
                for domain_quota_cap in domain_quota_caps:
                    ...
                    for cid, on_case in on_map.items():
                        ...
                        if not off_case:
                            ...
                            total += 1
                            continue
                        confidence = _hash_unit(f"w19-e3:conf:{cid}:{confidence_gate}:{domain_quota_cap}")
    ...
    decision = "GO_W19_E4_POLICY_REPLAY" if recommended else "HOLD_W19_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week19_expanded_domain_budget_veto_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W19-E4 policy replay."
            if decision == "GO_W19_E4_POLICY_REPLAY"
            else "Keep baseline and continue to W19-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
