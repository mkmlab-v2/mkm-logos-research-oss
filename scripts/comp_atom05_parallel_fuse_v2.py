#!/usr/bin/env python3
"""Parallel: handoff refresh + wire candidates + profile/stub pytest smoke."""
...
OUT = ROOT / "reports" / "constitution" / "btrack_pilot" / "comp_atom05_parallel_fuse_v2.json"
...
def _job_anchor07() -> dict:
    return _run_script("scripts/comp_anchor07_wire_atom_candidates_v1.py")


def _job_pytest_profile() -> dict:
    return _run_pytest(["tests/test_compression_profile_v1.py"])


def _job_pytest_v2_wire() -> dict:
    return _run_pytest(
        [
...
def main() -> int:
    ...
    jobs = [
        ...
        ("anchor07", _job_anchor07),
        ("pytest_profile", _job_pytest_profile),
        ("pytest_v2_wire", _job_pytest_v2_wire),
    ]
    results: list[dict] = []
    max_exit = 0
    ...
    doc = {
        "schema": "comp_atom05_parallel_fuse_v2",
        "generated_at_utc": _utc(),
        "tracks": results,
        "stub_bench_align": "profile_evaluate_report_kwargs_v2 economy+wire",
    }
