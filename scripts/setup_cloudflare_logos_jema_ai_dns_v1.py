#!/usr/bin/env python3
"""Create or update logos.jema-ai.com DNS (CNAME → app.jema-ai.com, proxied)."""
...
def _load_dotenv_token() -> None:
    ...
    for line in dotenv.read_text(encoding="utf-8").splitlines():
        ...
        if key in ("CLOUDFLARE_API_TOKEN", "CF_API_TOKEN", "MKM_CLOUDFLARE_JEMA_AI_DNS_TOKEN") and val:
...
def main() -> int:
    ...
    _load_dotenv_token()
    tok = (
        os.environ.get("MKM_CLOUDFLARE_JEMA_AI_DNS_TOKEN", "").strip()
    ...
    out = {
        "schema": "cloudflare_logos_jema_ai_dns_v1",
    ...
    report = (
        Path(os.environ.get("MKM_WORKSPACE_ROOT", r"C:\workspace"))
        / "reports"
        / "cloudflare_logos_jema_ai_dns_latest.json"
    )
