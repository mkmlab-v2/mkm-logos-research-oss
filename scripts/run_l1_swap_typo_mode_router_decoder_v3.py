...
# Balance: 89
# Purpose: Evaluate stronger latency-cut mode-router decoder v3.
...
PREREG_OUT = ART / "l1_inverse_decoder_swap_typo_mode_router_decoder_v3_preregister.json"
AB_OUT = ART / "l1_inverse_decoder_swap_typo_mode_router_decoder_v3_ab_v1.json"
...
def _cheap_score(c: str, pos_vocab: dict[int, set[str]], literal_channel: dict[int, str]) -> float:
    ...
    return (8.0 * lit_mismatch) - (0.9 * pos_fit)


def _dynamic_top_k(noisy_tokens: list[str], repaired_tokens: list[str], base: int = 48) -> int:
    # More aggressive for likely order-only cases.
    diff = sum(1 for a, b in zip(noisy_tokens, repaired_tokens) if a != b)
...
def _build_mode_pool(
    ...
    k = _dynamic_top_k(noisy_tokens, repaired_tokens, base=48)
    ranked = sorted(merged, key=lambda c: _cheap_score(c, pos_vocab=pos_vocab, literal_channel=literal_channel))
    return set(ranked[:k])
...
def _score_candidate(
    ...
    if mode == "swap_typo" and swap_dist > 8:
        # Early rejection for high-cost order paths.
        return 1e9 + swap_dist
    edit_sum = 0.0
    ...
    if mode == "swap_typo":
        return (14.0 * lit_mismatch) + (1.1 * swap_dist) + (0.7 * edit_sum) - (0.9 * cos_obs) - (0.7 * cos_src) - (
            1.0 * pos_fit
        )
...
def main() -> int:
    ...
    prereg = {
        "schema": "l1_inverse_decoder_swap_typo_mode_router_decoder_v3_preregister",
        "generated_at_utc": _now_utc(),
        "research_only": True,
        "hypothesis": "Aggressive pruning/early-exit can keep uplift while cutting swap_typo latency toward budget.",
    ...
    mixed_eval = _eval_mode(eval_seeds, args.samples, args.beam_size, args.noise_level, args.scoring_mode, None)
    swap_eval = _eval_mode(eval_seeds, args.samples, args.beam_size, args.noise_level, args.scoring_mode, "swap_typo")
    baseline = {
    ...
    delta = {
        ...
        "swap_typo_exact_delta": swap_eval["exact_restore_rate"] - baseline["swap_typo_exact"],
        "swap_typo_recovery_delta": swap_eval["recovery_rate"] - baseline["swap_typo_recovery"],
    }
    checks = {
    ...
    out_doc = {
        "schema": "l1_inverse_decoder_swap_typo_mode_router_decoder_v3_ab",
        "generated_at_utc": _now_utc(),
        "research_only": True,
        "candidate": "swap_typo_mode_router_decoder_v3",
        "results": {"mixed": mixed_eval, "swap_typo": swap_eval},
        "baseline_reference": baseline,
        "delta_vs_baseline": delta,
        "gate": {"checks": checks, "all_ok": all_ok, "decision": "GO_MODE_ROUTER_DECODER_V3" if all_ok else "HOLD_V4"},
