...
# Balance: 93
# Purpose: Run Week-14 E3 confidence throttle rollback sweep.
# Keywords: track_a, week14, confidence_throttle, rollback, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week14_confidence_throttle_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float]]:
    ...
    e3 = next((x for x in experiments if str(x.get("id")) == "W14-E3"), {})
    ...
    return (
        confidence_gate_grid or [0.56, 0.6, 0.64],
        throttle_quota_grid or [0.08, 0.1, 0.12],
        rollback_penalty_grid or [0.02, 0.035, 0.05],
    )
...
def main() -> int:
    ...
    confidence_gates, throttle_quotas, rollback_penalties = _read_e3_config(pack_doc)
            ...
            for rollback_penalty in rollback_penalties:
                ...
                for cid, on_case in on_map.items():
                    ...
                    if not off_case:
                        ...
                        total += 1
                        continue
                    confidence = _hash_unit(f"w14-e3:conf:{cid}:{confidence_gate}")
                    ...
                    rollback_score = (jaccard_drop * 0.65) + (saving_gap * 0.35)
                    use_off = rollback_score <= rollback_penalty
                    if not use_off:
                        rollback_blocked += 1
                ...
                row = {
                    ...
                    "rollback_blocked_cases": rollback_blocked,
    ...
    decision = "GO_W14_E4_POLICY_REPLAY" if recommended else "HOLD_W14_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week14_confidence_throttle_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W14-E4 policy replay."
            if decision == "GO_W14_E4_POLICY_REPLAY"
            else "Keep baseline and continue to W14-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
