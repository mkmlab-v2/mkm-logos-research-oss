...
# Balance: 88
# Purpose: Evaluate lane decoder v2 with swap_typo-focused scoring.
# Keywords: swap_typo, lane-decoder, candidate-generation, ab-test
#!/usr/bin/env python3
...
PREREG_OUT = ART / "l1_inverse_decoder_swap_typo_lane_decoder_v2_preregister.json"
AB_OUT = ART / "l1_inverse_decoder_swap_typo_lane_decoder_v2_ab_v1.json"
...
def _nearest_vocab(token: str, vocab: set[str], k: int) -> list[str]:
    if not vocab:
        return [token]
    ranked = sorted(vocab, key=lambda v: (_levenshtein(v, token), abs(len(v) - len(token)), v))
    return ranked[:k]


def _build_typo_lane_candidates(noisy_tokens: list[str], pos_vocab: dict[int, set[str]], cap: int) -> set[str]:
    ...
    for i, tok in enumerate(noisy_tokens):
        ...
        if tok in vocab:
            per_pos.append([tok])
            continue
        if tok == "OOV_TOKEN" or "X" in tok:
            uncertain_positions += 1
            clean_tok = tok.replace("X", "A")
            alts = _nearest_vocab(clean_tok, vocab, k=4)
            per_pos.append(alts if alts else [tok])
        else:
            per_pos.append([tok])
...
def _lane_pool(
    ...
    order_lane = _generate_adjacent_transposition_candidates(
        repaired_tokens,
        max_steps=4 if mode == "swap_typo" else 3,
    )
    typo_lane_cap = 96 if mode == "swap_typo" else 48
    typo_lane = _build_typo_lane_candidates(noisy_tokens, pos_vocab, cap=typo_lane_cap)
    return set(baseline) | order_lane | typo_lane
...
def _score_candidate(
    ...
    if mode == "swap_typo":
        return (14.0 * lit_mismatch) + (0.4 * swap_dist) + (2.2 * edit_sum) - (1.6 * cos_obs) - (0.9 * cos_src)
...
def main() -> int:
    ...
    prereg = {
        "schema": "l1_inverse_decoder_swap_typo_lane_decoder_v2_preregister",
        "generated_at_utc": _now_utc(),
        "research_only": True,
        "hypothesis": "Lane decoder v2 with swap_typo-specific scoring can recover typo lanes without mixed regression.",
        "candidate": "swap_typo_lane_decoder_v2",
        "is_structural_new_path": True,
        "isolation": "swap_typo-focused candidate generation/scoring; production v4 path unchanged",
        "inputs": {
    ...
    out_doc = {
        "schema": "l1_inverse_decoder_swap_typo_lane_decoder_v2_ab",
        "generated_at_utc": _now_utc(),
        "research_only": True,
        "candidate": "swap_typo_lane_decoder_v2",
        "results": {
        ...
        "gate": {
            "checks": checks,
            "all_ok": all_ok,
            "decision": "GO_LANE_DECODER_V2" if all_ok else "HOLD_V4",
        },
        "artifacts": {
