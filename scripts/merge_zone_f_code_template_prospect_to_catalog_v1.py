#!/usr/bin/env python3
"""Merge zone_f_code prospect template catalog into production catalog (gated).

Requires --human-approve-merge --reviewer. Default is dry-run plan only.
Optionally rebuilds manifest + coding deep pack twin gate after merge.
...
from scripts.merge_zone_f_code_template_prospect_v1_lib import (  # noqa: E402
    load_catalog_rows,
    plan_prospect_merge,
    write_catalog_jsonl,
)
...
DEFAULT_MANIFEST = ROOT / "codebook/templates/zone_f_code_templates_manifest_v1.json"
DEFAULT_REPORT = ROOT / "reports/zone_f_code_template_catalog_merge_v1_latest.json"
DEFAULT_ARTIFACT = ROOT / "docs/final/artifacts/zone_f_code_template_catalog_merge_v1_latest.json"
GATE_BUILDER = ROOT / "scripts/build_compression_coding_deep_pack_gate_v1.py"
...
def build_merge_report(
    *,
    ...
    return {
        "schema": "zone_f_code_template_catalog_merge_v1",
        ...
        "reproduce": "py scripts/merge_zone_f_code_template_prospect_to_catalog_v1.py",
    }


def main() -> int:
    ap = argparse.ArgumentParser(description="Merge prospect templates into production catalog (gated)")
