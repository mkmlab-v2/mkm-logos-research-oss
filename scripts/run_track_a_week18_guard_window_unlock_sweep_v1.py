...
# Balance: 93
# Purpose: Run Week-18 E1 guard-window selective unlock sweep.
# Keywords: track_a, week18, guard_window, unlock, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week18_guard_window_unlock_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float], list[str]]:
    ...
    e1 = next((x for x in experiments if str(x.get("id")) == "W18-E1"), {})
    ...
    return (
        unlock_ratio_grid or [0.12, 0.14, 0.16],
        jaccard_guard_grid or [0.843, 0.845, 0.847],
        guard_relax_margin_grid or [0.001, 0.002, 0.003],
...
def main() -> int:
            ...
            for guard_relax_margin in guard_relax_margins:
                ...
                for cid, on_case in on_map.items():
                    ...
                    if off_j < guard_threshold:
                        ...
                        total += 1
                        continue
                    unlock_signal = _hash_unit(f"w18-e1:unlock:{cid}:{unlock_ratio}:{jaccard_guard}:{guard_relax_margin}")
    ...
    decision = "GO_W18_E2_BUDGETED_CROSSOVER_SHADOW" if recommended else "HOLD_W18_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week18_guard_window_unlock_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W18-E2 budgeted crossover shadow sweep."
            if decision == "GO_W18_E2_BUDGETED_CROSSOVER_SHADOW"
            else "Keep baseline and continue to W18-E2 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
