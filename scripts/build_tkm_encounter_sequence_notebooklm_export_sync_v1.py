#!/usr/bin/env python3
"""Build TKM encounter_sequence NotebookLM export sync manifest [HYPO]."""
...
OUT = ROOT / "reports/tkm_encounter_sequence_notebooklm_export_sync_v1_latest.json"
ARTIFACT = ROOT / "docs/final/artifacts/tkm_encounter_sequence_notebooklm_export_sync_v1_latest.json"
MANIFEST_OUT = ROOT / "reports/tkm_encounter_sequence_notebooklm_export_manifest_v1_latest.json"
MANIFEST_ARTIFACT = ROOT / "docs/final/artifacts/tkm_encounter_sequence_notebooklm_export_manifest_v1_latest.json"
...
EXPORT_REL_PATHS = (
    ...
    "reports/tkm_encounter_sequence_full_stack_export_bundle_v1_latest.json",
    "docs/final/artifacts/tkm_encounter_sequence_full_stack_export_bundle_v1_latest.json",
    "reports/tkm_encounter_sequence_lens_stack_rollup_v1_latest.json",
    "docs/final/artifacts/tkm_encounter_sequence_lens_stack_rollup_v1_latest.json",
    "reports/encounter_sequence_weekly_report_v1_latest.json",
    "reports/tkm_encounter_sequence_ops_closure_v1_latest.json",
    "docs/final/artifacts/tkm_encounter_sequence_p51_gate_v1_latest.json",
)
...
def _mirror_to_vault(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    target = vault / "btrack_artifacts_verified" / "tkm_encounter_sequence"
    ...
    manifest_copy = MANIFEST_OUT if MANIFEST_OUT.is_file() else None
    if manifest_copy:
        shutil.copy2(manifest_copy, target / manifest_copy.name)
        copied.append(manifest_copy.name)
    return {
...
def build(*, mirror_vault: bool = True) -> dict[str, Any]:
    ...
    entries = [_file_entry(rel) for rel in EXPORT_REL_PATHS]
    all_exist = all(e.get("exists") for e in entries)
    manifest = {
        "schema": "tkm_encounter_sequence_notebooklm_export_manifest_v1",
        "version": "1.0.0",
    ...
    return {
        "schema": "tkm_encounter_sequence_notebooklm_export_sync_v1",
        "version": "1.0.0",
        "generated_at_utc": _utc(),
        "lane": "track_b_hypo",
        ...
        "non_gating": True,
        "research_only": True,
        "export_sync_ok": export_sync_ok,
        "p51_gate_ok": p51.get("gate_ok"),
        ...
        "note_ko": "P52 NotebookLM export sync manifest [HYPO][NON_GATING]; vault mirror only; cloud MCP upload 금지.",
        "reproduce": "py scripts/build_tkm_encounter_sequence_notebooklm_export_sync_v1.py",
    }
