...
# Balance: 88
# Purpose: Evaluate noise-mode router decoder with permutation candidates.
# Keywords: swap_typo, mode-router, permutation, candidate-generation, ab-test
#!/usr/bin/env python3
...
PREREG_OUT = ART / "l1_inverse_decoder_swap_typo_mode_router_decoder_v1_preregister.json"
AB_OUT = ART / "l1_inverse_decoder_swap_typo_mode_router_decoder_v1_ab_v1.json"
...
def _permutation_candidates(tokens: list[str], cap: int = 720) -> set[str]:
    # Tokens are 6-length in this benchmark corpus; full permutation space is still tractable.
    out: set[str] = set()
...
def _build_mode_pool(
    ...
    if mode == "swap_typo":
        # D25 core: open full order space from repaired token multiset.
        perms = _permutation_candidates(repaired_tokens, cap=720)
        return set(baseline) | perms
    # mixed keeps conservative search space to avoid regressions.
    return set(baseline)
...
def _score_candidate(
    ...
    if mode == "swap_typo":
        # Router path for order-heavy failures: rely on position fit + swap distance from repaired order.
        return (14.0 * lit_mismatch) + (1.3 * swap_dist) + (0.6 * edit_sum) - (0.8 * cos_obs) - (0.6 * cos_src) - (
            0.9 * pos_fit
        )
...
def main() -> int:
    ...
    prereg = {
        "schema": "l1_inverse_decoder_swap_typo_mode_router_decoder_v1_preregister",
        "generated_at_utc": _now_utc(),
        "research_only": True,
        "hypothesis": "Mode-routed candidate generation with permutation order pool can improve swap_typo order-heavy failures.",
    ...
    out_doc = {
        "schema": "l1_inverse_decoder_swap_typo_mode_router_decoder_v1_ab",
        "generated_at_utc": _now_utc(),
        "research_only": True,
        "candidate": "swap_typo_mode_router_decoder_v1",
        "results": {
        ...
        "gate": {
            "checks": checks,
            "all_ok": all_ok,
            "decision": "GO_MODE_ROUTER_DECODER_V1" if all_ok else "HOLD_V4",
        },
        "artifacts": {
