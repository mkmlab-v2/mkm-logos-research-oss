#!/usr/bin/env python3
"""Push consumer pack via nlm CLI (preserves source titles; prefer over MCP text paste).

SSOT pack: reports/notebooklm_consumer_sync_pack_v1/
URL: reports/notebooklm_consumer_notebook_url_v1.txt
"""
...
PACK = ROOT / "reports" / "notebooklm_consumer_sync_pack_v1"
URL_FILE = ROOT / "reports" / "notebooklm_consumer_notebook_url_v1.txt"
OUT = ROOT / "reports" / "notebooklm_consumer_push_nlm_latest.json"
...
def main() -> int:
    if not (PACK / "index.json").is_file():
        raise SystemExit("run build_notebooklm_consumer_sync_pack_v1.py first")
    nb = _notebook_id()
    ...
    doc = {"schema": "notebooklm_consumer_push_nlm_latest", "notebook_id": nb, "rows": rows}
