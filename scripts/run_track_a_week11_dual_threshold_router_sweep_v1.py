...
# Balance: 93
# Purpose: Run Week-11 E1 dual-threshold router governance sweep.
# Keywords: track_a, week11, dual_threshold, router_governance, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week11_dual_threshold_router_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[str]]:
    ...
    e1 = next((x for x in experiments if str(x.get("id")) == "W11-E1"), {})
    cfg = e1.get("config") or {}
    saving_trigger_grid = [float(x) for x in (cfg.get("saving_trigger_grid") or [])]
    ...
    return (
        saving_trigger_grid or [0.486, 0.488, 0.49],
...
def main() -> int:
    ...
    saving_triggers, jaccard_guards, protected_domains = _read_e1_config(pack_doc)
    ...
    for saving_trigger in saving_triggers:
        for jaccard_guard in jaccard_guards:
            ...
            for cid, on_case in on_map.items():
                ...
                if off_s < saving_trigger or off_j < jaccard_guard:
                    selected.append(on_case)
                    threshold_blocked += 1
                    total += 1
                    continue
                choose = _hash_unit(f"w11-e1:{cid}:{saving_trigger}:{jaccard_guard}")
                use_off = choose <= 0.5
            ...
            row = {
                ...
                "threshold_blocked_cases": threshold_blocked,
    ...
    decision = "GO_W11_E2_OFFPATH_QUOTA_BACKPRESSURE" if recommended else "HOLD_W11_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week11_dual_threshold_router_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "saving_trigger_grid": saving_triggers,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W11-E2 off-path quota backpressure sweep."
            if decision == "GO_W11_E2_OFFPATH_QUOTA_BACKPRESSURE"
            else "Keep baseline and continue to W11-E2 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
