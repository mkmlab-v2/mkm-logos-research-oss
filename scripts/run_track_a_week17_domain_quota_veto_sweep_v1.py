...
# Balance: 93
# Purpose: Run Week-17 E3 adaptive rollback veto with domain quota sweep.
# Keywords: track_a, week17, domain_quota, adaptive_veto, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week17_domain_quota_veto_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float], list[int]]:
    ...
    e3 = next((x for x in experiments if str(x.get("id")) == "W17-E3"), {})
    ...
    return (
        confidence_gate_grid or [0.55, 0.59, 0.63],
        throttle_quota_grid or [0.1, 0.12, 0.14],
        rollback_penalty_grid or [0.02, 0.03, 0.045],
        domain_quota_cap_grid or [1, 2, 3],
    )
...
def main() -> int:
                ...
                for domain_quota_cap in domain_quota_caps:
                    ...
                    for cid, on_case in on_map.items():
                        ...
                        if not off_case:
                            ...
                            total += 1
                            continue
                        confidence = _hash_unit(f"w17-e3:conf:{cid}:{confidence_gate}:{domain_quota_cap}")
    ...
    decision = "GO_W17_E4_POLICY_REPLAY" if recommended else "HOLD_W17_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week17_domain_quota_veto_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W17-E4 policy replay."
            if decision == "GO_W17_E4_POLICY_REPLAY"
            else "Keep baseline and continue to W17-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
