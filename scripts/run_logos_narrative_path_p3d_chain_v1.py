#!/usr/bin/env python3
"""P3d — Narrative 12→16 curation + lemma/inter-hop re-eval chain (B-track, HYPO).

Reproducible:
  py scripts/run_logos_narrative_path_p3d_chain_v1.py
"""
...
def main() -> int:
    ...
    if not args.skip_pytest:
        proc = subprocess.run(
            [sys.executable, "-m", "pytest", "tests/test_logos_narrative_path_p3d_v1.py", "-q"],
            cwd=ROOT,
            check=False,
        )
        if proc.returncode != 0:
            return proc.returncode
        print("OK: pytest p3d narrative")
