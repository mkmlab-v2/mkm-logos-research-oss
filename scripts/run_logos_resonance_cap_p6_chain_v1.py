#!/usr/bin/env python3
"""P6 — resonance_cap 32→40 bloom slice deepen (B-track, HYPO).

Narrative count unchanged (80); inter-hop not re-run.

Reproducible:
  py scripts/run_logos_resonance_cap_p6_chain_v1.py
"""
...
def main() -> int:
    ...
    if not args.skip_pytest:
        proc = subprocess.run(
            [sys.executable, "-m", "pytest", "tests/test_logos_resonance_cap_p6_v1.py", "-q"],
            cwd=ROOT,
            check=False,
        )
        if proc.returncode != 0:
            return proc.returncode
        print("OK: pytest resonance_cap p6")
