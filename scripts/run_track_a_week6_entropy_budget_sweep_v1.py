...
# Balance: 93
# Purpose: Run Week-6 E1 entropy-aware token drop budget sweep.
# Keywords: track_a, week6, entropy_budget, sweep, compression_rule
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week6_entropy_budget_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], dict[str, float]]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e1 = next((x for x in experiments if str(x.get("id")) == "W6-E1"), {})
    cfg = e1.get("config") or {}
    budget_grid = [float(x) for x in (cfg.get("drop_budget_grid") or [])]
    multipliers = {
        str(k).strip().lower(): float(v)
        for k, v in (cfg.get("risk_domain_budget_multiplier") or {}).items()
        if str(k).strip()
    }
    return budget_grid or [0.08, 0.1, 0.12, 0.14], multipliers or {"ssot": 0.6, "timing": 0.65}
...
def main() -> int:
    ...
    ap.add_argument("--budget-grid", type=str, default="")
    ap.add_argument("--saving-floor", type=float, default=0.475)
    ap.add_argument("--jaccard-floor", type=float, default=0.83)
    ...
    default_grid, risk_multipliers = _read_e1_config(pack_doc)
    budgets = [float(x.strip()) for x in args.budget_grid.split(",") if x.strip()] or default_grid
    ...
    for budget in budgets:
        ...
        for cid, on_case in on_map.items():
            ...
            domain_multiplier = float(risk_multipliers.get(domain, 1.0))
            # Lower effective budget in risk domains.
            effective_budget = max(0.0, min(1.0, budget * domain_multiplier))
            use_off = _hash_unit(f"{domain}:{cid}:budget") <= effective_budget
        ...
        row = {
            "drop_budget": budget,
            "risk_domain_budget_multiplier": risk_multipliers,
    ...
    decision = "GO_W6_E2_NGRAM_PRESERVE" if recommended else "HOLD_W6_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week6_entropy_budget_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "drop_budget_grid": budgets,
            "risk_domain_budget_multiplier": risk_multipliers,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W6-E2 n-gram preservation sweep."
            if decision == "GO_W6_E2_NGRAM_PRESERVE"
            else "Keep baseline and continue to W6-E2 to test rule-injection path."
        ),
