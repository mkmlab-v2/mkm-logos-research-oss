...
# Balance: 93
# Purpose: Consolidate Week-14 experiments into final policy replay decision.
# Keywords: track_a, week14, policy_replay, decision, commercialization
#!/usr/bin/env python3
...
E1 = ROOT / "docs" / "final" / "artifacts" / "track_a_week14_staged_lane_unlock_sweep_v1.json"
E2 = ROOT / "docs" / "final" / "artifacts" / "track_a_week14_replay_anneal_floor_shadow_sweep_v1.json"
E3 = ROOT / "docs" / "final" / "artifacts" / "track_a_week14_confidence_throttle_sweep_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week14_policy_replay_v1.json"
...
def _read_e4_config(pack: dict[str, Any]) -> tuple[float, bool]:
    ...
    e4 = next((x for x in exps if str(x.get("id")) == "W14-E4"), {})
...
def main() -> int:
    ...
    decision = (
        "GO_W14_POLICY_REPLAY"
        if policy_floor_ok and integrity_ok and quality_tradeoff_ok
        else "HOLD_W14_POLICY_REPLAY"
    )

    out_doc = {
        "schema": "track_a_week14_policy_replay_v1",
        ...
        "week14_summary": {
            "entry_decision": (pack.get("entry_condition") or {}).get("week13_decision"),
        ...
        "decision": decision,
        "next_action": (
            "Keep Track A commercialization HOLD and design Week-15 hypothesis pack."
            if decision == "HOLD_W14_POLICY_REPLAY"
