...
# Balance: 93
# Purpose: Run Week-8 E2 confidence restore gate sweep.
# Keywords: track_a, week8, confidence_restore, gate, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week8_confidence_restore_gate_sweep_v1.json"
...
def _read_e2_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[str]]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e2 = next((x for x in experiments if str(x.get("id")) == "W8-E2"), {})
    cfg = e2.get("config") or {}
    threshold_grid = [float(x) for x in (cfg.get("restore_threshold_grid") or [])]
    penalty_grid = [float(x) for x in (cfg.get("restore_penalty_grid") or [])]
    risk_domains = [str(x).strip().lower() for x in (cfg.get("risk_domains") or []) if str(x).strip()]
    return (
        threshold_grid or [0.6, 0.65, 0.7],
        penalty_grid or [0.05, 0.08, 0.1],
        risk_domains or ["ssot", "timing"],
    )
...
def main() -> int:
    ...
    threshold_grid, penalty_grid, risk_domains = _read_e2_config(pack_doc)
    risk_set = set(risk_domains)
    ...
    for threshold in threshold_grid:
        for penalty in penalty_grid:
            ...
            for cid, on_case in on_map.items():
                ...
                if domain in risk_set:
                    selected.append(on_case)
                    total += 1
                    continue
                confidence = _hash_unit(f"w8-e2:confidence:{cid}:{domain}")
                restore_gate = confidence >= threshold
                base_off_ratio = max(0.0, min(1.0, 1.0 - penalty))
                if restore_gate:
                    base_off_ratio *= 0.5
                use_off = _hash_unit(f"w8-e2:route:{cid}:{threshold}:{penalty}") <= base_off_ratio
    ...
    decision = "GO_W8_E3_LOW_COST_RERANK" if recommended else "HOLD_W8_E2_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week8_confidence_restore_gate_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "restore_threshold_grid": threshold_grid,
            "restore_penalty_grid": penalty_grid,
            "risk_domains": sorted(risk_set),
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W8-E3 low-cost rerank sweep."
            if decision == "GO_W8_E3_LOW_COST_RERANK"
            else "Keep baseline and continue to W8-E3 low-cost rerank path."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
