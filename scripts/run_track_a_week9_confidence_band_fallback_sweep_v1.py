...
# Balance: 93
# Purpose: Run Week-9 E3 confidence-band fallback sweep.
# Keywords: track_a, week9, confidence_band, fallback, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week9_confidence_band_fallback_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[str]]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e3 = next((x for x in experiments if str(x.get("id")) == "W9-E3"), {})
    cfg = e3.get("config") or {}
    confidence_band_grid = [float(x) for x in (cfg.get("confidence_band_grid") or [])]
    fallback_penalty_grid = [float(x) for x in (cfg.get("fallback_penalty_grid") or [])]
    critical_domains = [str(x).strip().lower() for x in (cfg.get("critical_domains") or []) if str(x).strip()]
    return (
        confidence_band_grid or [0.58, 0.62, 0.66],
        fallback_penalty_grid or [0.04, 0.06, 0.08],
        critical_domains or ["ssot", "timing"],
    )
...
def main() -> int:
    ...
    confidence_bands, fallback_penalties, critical_domains = _read_e3_config(pack_doc)
    critical_set = set(critical_domains)
    ...
    for band in confidence_bands:
        for penalty in fallback_penalties:
            ...
            for cid, on_case in on_map.items():
                ...
                if domain in critical_set:
                    selected.append(on_case)
                    fallback_to_on += 1
                    total += 1
                    continue
                confidence = _hash_unit(f"w9-e3:confidence:{cid}:{domain}")
                if confidence >= band:
                    # High confidence band falls back to router_on to preserve quality.
                    selected.append(on_case)
                    fallback_to_on += 1
                    total += 1
                    continue
                off_trigger = _hash_unit(f"w9-e3:route:{cid}:{band}:{penalty}")
                off_ratio = max(0.0, min(1.0, 0.35 - penalty))
                use_off = off_trigger <= off_ratio
            ...
            row = {
                ...
                "critical_domains": sorted(critical_set),
                "switched_to_router_off_cases": switched_to_off,
                "fallback_to_router_on_cases": fallback_to_on,
    ...
    decision = "GO_W9_E4_POLICY_REPLAY" if recommended else "HOLD_W9_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week9_confidence_band_fallback_sweep_v1",
        ...
        "inputs": {
            ...
            "fallback_penalty_grid": fallback_penalties,
            "critical_domains": sorted(critical_set),
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W9-E4 policy replay."
            if decision == "GO_W9_E4_POLICY_REPLAY"
            else "Keep baseline and continue to W9-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
