#!/usr/bin/env python3
"""Push 00_OPS command pack via nlm CLI (preserves source titles).

SSOT pack: reports/notebooklm_ops_command_sync_pack_v1/
URL: reports/notebooklm_ops_command_notebook_url_v1.txt
"""
...
URL_FILE = ROOT / "reports" / "notebooklm_ops_command_notebook_url_v1.txt"
OUT = ROOT / "reports" / "notebooklm_ops_command_push_nlm_latest.json"
...
def main() -> int:
    ...
    if not (PACK / "index.json").is_file():
        raise SystemExit("run build_notebooklm_ops_command_sync_pack_v1.py first")
    nb = _notebook_id()
    ...
    doc = {"schema": "notebooklm_ops_command_push_nlm_latest", "notebook_id": nb, "rows": rows}
