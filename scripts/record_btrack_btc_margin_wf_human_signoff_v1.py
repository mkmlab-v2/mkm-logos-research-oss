#!/usr/bin/env python3
"""Record human sign-off for BTC margin WF Type-A research shadow (not operational score)."""
...
DEFAULT_OUT = ROOT / "reports/btrack_btc_margin_wf_human_signoff_latest.json"
...
def main() -> int:
    ...
    ap.add_argument(
        "--acknowledge-btc-instrument-layer-change",
        action="store_true",
        help="Required to approve: confirms Type-A BTC shadow layer is intentional.",
    )
    ...
    if args.revoke:
        ...
        if not args.acknowledge_btc_instrument_layer_change:
            print("Refusing approve without --acknowledge-btc-instrument-layer-change", flush=True)
            return 2
