...
# Balance: 93
# Purpose: Run Week-16 E2 dual-threshold confidence crossover sweep.
# Keywords: track_a, week16, dual_threshold, confidence_crossover, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week16_dual_threshold_crossover_sweep_v1.json"
...
def _read_e2_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[int]]:
    ...
    e2 = next((x for x in experiments if str(x.get("id")) == "W16-E2"), {})
    ...
    return (
        crossover_weight_grid or [0.6, 0.65, 0.7],
        floor_shadow_margin_grid or [0.0, 0.0015, 0.003],
...
def main() -> int:
    ...
    crossover_weights, floor_margins, candidate_pools = _read_e2_config(pack_doc)
        ...
        for floor_margin in floor_margins:
            for candidate_pool in candidate_pools:
                ...
                for cid, on_case in on_map.items():
                    ...
                    noise = (_hash_unit(f"w16-e2:{cid}:{crossover_weight}:{floor_margin}:{candidate_pool}") - 0.5) * 0.001
    ...
    decision = "GO_W16_E3_ADAPTIVE_VETO_THROTTLE" if recommended else "HOLD_W16_E2_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week16_dual_threshold_crossover_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W16-E3 adaptive veto throttle sweep."
            if decision == "GO_W16_E3_ADAPTIVE_VETO_THROTTLE"
            else "Keep baseline and continue to W16-E3 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
