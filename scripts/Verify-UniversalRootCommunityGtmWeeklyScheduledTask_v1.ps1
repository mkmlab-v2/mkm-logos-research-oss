...
<#
.SYNOPSIS
  Spot-check MKM_UniversalRoot_CommunityGtm_Weekly scheduled task + SSOT tier4 registration.
#>
[CmdletBinding()]
param(
...
if (-not $OutJson) {
    $OutJson = Join-Path $root "reports\universal_root_community_gtm_weekly_scheduled_ops_readiness_v1_latest.json"
}

$runnerExpected = Join-Path $root "scripts\Invoke-UniversalRootCommunityGtmWeeklyRoutine_v1.ps1"
$stackPath = Join-Path $root "docs\final\artifacts\mkm_scheduler_solo_core_stack_v1.json"
$tier4Expected = "\\MKM_UniversalRoot_CommunityGtm_Weekly"
...
$payload = [ordered]@{
    schema                             = "universal_root_community_gtm_weekly_scheduled_ops_readiness_v1"
...
if ($null -ne $task) {
    ...
    if ($argStr -match 'Invoke-UniversalRootCommunityGtmWeeklyRoutine_v1\.ps1') {
