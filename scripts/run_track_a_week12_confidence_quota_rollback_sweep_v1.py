...
# Balance: 93
# Purpose: Run Week-12 E3 confidence-gated rollback with adaptive quota sweep.
# Keywords: track_a, week12, confidence_quota, rollback, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week12_confidence_quota_rollback_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float]]:
    ...
    e3 = next((x for x in experiments if str(x.get("id")) == "W12-E3"), {})
    cfg = e3.get("config") or {}
    confidence_gate_grid = [float(x) for x in (cfg.get("confidence_gate_grid") or [])]
    adaptive_quota_grid = [float(x) for x in (cfg.get("adaptive_quota_grid") or [])]
    rollback_penalty_grid = [float(x) for x in (cfg.get("rollback_penalty_grid") or [])]
    return (
        confidence_gate_grid or [0.6, 0.64, 0.68],
        adaptive_quota_grid or [0.08, 0.1, 0.12],
...
def main() -> int:
    ...
    confidence_gates, adaptive_quotas, rollback_penalties = _read_e3_config(pack_doc)
    ...
    for confidence_gate in confidence_gates:
        for adaptive_quota in adaptive_quotas:
            for rollback_penalty in rollback_penalties:
                ...
                rollback_to_on = 0
                total = 0
                quota_limit = max(0, int(total_cases * max(0.0, min(0.5, adaptive_quota))))
                for cid, on_case in on_map.items():
                    ...
                    confidence = _hash_unit(f"w12-e3:conf:{cid}:{confidence_gate}")
                    ...
                    overshoot = max(0.0, off_s - args.saving_floor)
                    rollback_score = jaccard_drop + overshoot

                    use_off = True
                    if rollback_score > rollback_penalty:
                        use_off = False
                        rollback_to_on += 1
    ...
    decision = "GO_W12_E4_POLICY_REPLAY" if recommended else "HOLD_W12_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week12_confidence_quota_rollback_sweep_v1",
        ...
        "inputs": {
            ...
            "adaptive_quota_grid": adaptive_quotas,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W12-E4 policy replay."
            if decision == "GO_W12_E4_POLICY_REPLAY"
            else "Keep baseline and continue to W12-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
