...
'@
        ...
        & py "scripts\eval_prophecy_hit_rate_v1.py" "--run-mode" "price" "--score-json" (Join-Path $WorkspaceRoot "docs\final\artifacts\btrack_prophecy_score_kospi_only_latest.json") "--output" (Join-Path $WorkspaceRoot "docs\final\artifacts\prophecy_hit_rate_eval_kospi_only_latest.json")
        if ($LASTEXITCODE -ne 0) {
            throw "eval_prophecy_hit_rate_v1.py (KOSPI-only) exit $LASTEXITCODE"
        }

        Write-Host "==> eval_prophecy_hit_rate_v1.py --run-mode price (BTC-only)" -ForegroundColor Cyan
        & py "scripts\eval_prophecy_hit_rate_v1.py" "--run-mode" "price" "--score-json" (Join-Path $WorkspaceRoot "docs\final\artifacts\btrack_prophecy_score_btc_only_latest.json") "--output" (Join-Path $WorkspaceRoot "docs\final\artifacts\prophecy_hit_rate_eval_btc_only_latest.json")
        if ($LASTEXITCODE -ne 0) {
            throw "eval_prophecy_hit_rate_v1.py (BTC-only) exit $LASTEXITCODE"
        }
...
payload = {
    ...
    "research_only": True,
    "window": {
        "recent_trading_days": ((cmp_.get("window") or {}).get("recent_trading_days")),
    },
    "overall": m(overall),
    "legs": {
        "kospi": m(kospi),
...
md = []
md.append("# Track C Prophecy Dual-Leg Brief")
md.append("")
md.append(f"- generated_at_utc: `{payload['generated_at_utc']}`")
md.append(f"- recent_trading_days: `{payload['window']['recent_trading_days']}`")
md.append(f"- overall_hit_rate: `{pct(payload['overall']['price_directional_hit_rate'])}` ({payload['overall']['price_hits']}/{payload['overall']['n_evaluated']})")
md.append(f"- kospi_hit_rate: `{pct(payload['legs']['kospi']['price_directional_hit_rate'])}` ({payload['legs']['kospi']['price_hits']}/{payload['legs']['kospi']['n_evaluated']})")
md.append(f"- btc_hit_rate: `{pct(payload['legs']['btc']['price_directional_hit_rate'])}` ({payload['legs']['btc']['price_hits']}/{payload['legs']['btc']['n_evaluated']})")
md.append(f"- btc_minus_kospi_hit_rate: `{payload['delta'].get('btc_minus_kospi_hit_rate')}`")
md.append("")
md.append("## Evidence")
md.append("- `docs/final/artifacts/prophecy_hit_rate_eval_latest.json`")
md.append("- `docs/final/artifacts/prophecy_hit_rate_eval_kospi_only_latest.json`")
md.append("- `docs/final/artifacts/prophecy_hit_rate_eval_btc_only_latest.json`")
md.append("- `docs/final/artifacts/prophecy_hit_rate_dual_leg_comparison_latest.json`")
md.append("")
md.append("> research_only: This brief is a measurement snapshot, not a live trading trigger.")
brief_md.write_text("\n".join(md) + "\n", encoding="utf-8")
...
if (-not $SkipEval) {
    ...
    if (-not $SkipDualLegCompare) {
        Write-Host "[OK] Eval:    docs/final/artifacts/prophecy_hit_rate_eval_kospi_only_latest.json" -ForegroundColor Green
        Write-Host "[OK] Eval:    docs/final/artifacts/prophecy_hit_rate_eval_btc_only_latest.json" -ForegroundColor Green
        ...
        Write-Host "[OK] Brief:   docs/final/artifacts/trackc_prophecy_dual_leg_brief_latest.md" -ForegroundColor Green
    }
}
