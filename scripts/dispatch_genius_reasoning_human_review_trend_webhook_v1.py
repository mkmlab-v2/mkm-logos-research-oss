#!/usr/bin/env python3
"""Dispatch webhook when genius human review trend gate is HOLD."""
...
DEFAULT_TREND = ART / "genius_reasoning_human_review_gate_trend_latest.json"
DEFAULT_OUT = ART / "genius_reasoning_human_review_gate_trend_dispatch_latest.json"
...
def main() -> int:
    ...
    status = str(trend.get("status") or "UNKNOWN").upper()
    current = trend.get("current") if isinstance(trend.get("current"), dict) else {}
    reasons = trend.get("reasons") if isinstance(trend.get("reasons"), list) else []
    should_dispatch = status == "HOLD"
    webhook = str(os.environ.get(args.webhook_env, "")).strip()

    out: dict[str, Any] = {
        "schema": "genius_reasoning_human_review_trend_dispatch_v1",
    ...
    if should_dispatch and webhook:
        payload = {
            "source": "genius_human_review_trend_v1",
            "severity": "WARNING",
            "status": status,
            "reasons": reasons,
            "current": current,
        ...
        out["dispatch"] = {"status": "skipped", "reason": "trend_not_hold"}
