...
# Balance: 93
# Purpose: Consolidate Week-17 experiments into final policy replay decision.
# Keywords: track_a, week17, policy_replay, decision, commercialization
#!/usr/bin/env python3
...
E1 = ROOT / "docs" / "final" / "artifacts" / "track_a_week17_guard_banded_unlock_sweep_v1.json"
E2 = ROOT / "docs" / "final" / "artifacts" / "track_a_week17_confidence_budget_crossover_sweep_v1.json"
E3 = ROOT / "docs" / "final" / "artifacts" / "track_a_week17_domain_quota_veto_sweep_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week17_policy_replay_v1.json"
...
def _read_e4_config(pack: dict[str, Any]) -> tuple[float, bool]:
    ...
    e4 = next((x for x in exps if str(x.get("id")) == "W17-E4"), {})
...
def main() -> int:
    ...
    decision = (
        "GO_W17_POLICY_REPLAY"
        if policy_floor_ok and integrity_ok and quality_tradeoff_ok
        else "HOLD_W17_POLICY_REPLAY"
    )

    out_doc = {
        "schema": "track_a_week17_policy_replay_v1",
        ...
        "week17_summary": {
            "entry_decision": (pack.get("entry_condition") or {}).get("week16_decision"),
        ...
        "decision": decision,
        "next_action": (
            "Keep Track A commercialization HOLD and design Week-18 hypothesis pack."
            if decision == "HOLD_W17_POLICY_REPLAY"
