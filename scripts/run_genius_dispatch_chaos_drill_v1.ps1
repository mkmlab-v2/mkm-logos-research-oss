<#
.SYNOPSIS
  Run genius dispatch chaos drill script.
#>
param(
...
$scriptPath = Join-Path $WorkspaceRoot "scripts\run_genius_dispatch_chaos_drill_v1.py"
$outJson = Join-Path $WorkspaceRoot "docs\final\artifacts\genius_dispatch_chaos_drill_latest.json"

& py $scriptPath --output-json $outJson
if ($LASTEXITCODE -ne 0) { throw "Genius dispatch chaos drill failed ($LASTEXITCODE)" }
...
Write-Host "DONE: genius dispatch chaos drill"
