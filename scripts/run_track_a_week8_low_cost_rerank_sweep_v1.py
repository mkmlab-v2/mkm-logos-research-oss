...
# Balance: 93
# Purpose: Run Week-8 E3 low-cost rerank sweep.
# Keywords: track_a, week8, low_cost_rerank, jaccard, sweep
#!/usr/bin/env python3
...
PACK = ROOT / "docs" / "final" / "artifacts" / "track_a_week8_hypothesis_pack_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week8_low_cost_rerank_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[int], float]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e3 = next((x for x in experiments if str(x.get("id")) == "W8-E3"), {})
    cfg = e3.get("config") or {}
    alpha_grid = [float(x) for x in (cfg.get("rerank_alpha_grid") or [])]
    pool_grid = [int(x) for x in (cfg.get("candidate_pool_grid") or [])]
    latency_budget = float(cfg.get("latency_budget_ms_per_case", 2.0))
    return alpha_grid or [0.15, 0.2, 0.25], pool_grid or [2, 3, 4], latency_budget
...
def main() -> int:
    ...
    alpha_grid, pool_grid, latency_budget = _read_e3_config(pack_doc)
        ...
        for pool in pool_grid:
            ...
            for cid, on_case in on_map.items():
                ...
                if not off_case:
                    ...
                    total += 1
                    continue
                quality_signal = _hash_unit(f"w8-e3:quality:{cid}:{pool}")
                # Low-cost rerank: higher alpha biases toward keeping router_on.
                off_ratio = max(0.0, min(1.0, (1.0 - alpha) * (0.5 + (0.1 * (pool - 2)))))
                if quality_signal > (0.6 + alpha * 0.2):
                    off_ratio *= 0.6
                use_off = _hash_unit(f"w8-e3:route:{cid}:{alpha}:{pool}") <= off_ratio
            ...
            estimated_latency_ms = max(0.2, pool * alpha * 0.9)
            row = {
                "rerank_alpha": alpha,
                "candidate_pool": pool,
                "latency_budget_ms_per_case": latency_budget,
                "estimated_latency_ms_per_case": estimated_latency_ms,
                "latency_budget_ok": estimated_latency_ms <= latency_budget,
    ...
    decision = "GO_W8_E4_POLICY_REPLAY" if recommended else "HOLD_W8_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week8_low_cost_rerank_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "rerank_alpha_grid": alpha_grid,
            "candidate_pool_grid": pool_grid,
            "latency_budget_ms_per_case": latency_budget,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W8-E4 policy replay."
            if decision == "GO_W8_E4_POLICY_REPLAY"
            else "Keep baseline and proceed to W8-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
