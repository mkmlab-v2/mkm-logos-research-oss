...
# Balance: 93
# Purpose: Run Week-5 E2 adaptive confidence router sweep.
# Keywords: track_a, week5, confidence_router, sweep, routing
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week5_confidence_router_sweep_v1.json"
...
def _read_e2_config(pack_doc: dict[str, Any]) -> tuple[list[float], dict[str, float]]:
    experiments = ((pack_doc.get("pack") or {}).get("experiments") or [])
    e2 = next((x for x in experiments if str(x.get("id")) == "W5-E2"), {})
    cfg = e2.get("config") or {}
    grid = [float(x) for x in (cfg.get("confidence_threshold_grid") or [])]
    overrides = {
        str(k).strip().lower(): float(v)
        for k, v in (cfg.get("domain_overrides") or {}).items()
        if str(k).strip()
    }
    return grid or [0.55, 0.6, 0.65, 0.7], overrides or {"ssot": 0.7, "timing": 0.68}
...
def main() -> int:
    ...
    ap.add_argument("--threshold-grid", type=str, default="")
    ...
    default_grid, domain_overrides = _read_e2_config(pack_doc)
    thresholds = [float(x.strip()) for x in args.threshold_grid.split(",") if x.strip()] or default_grid
    ...
    for threshold in thresholds:
        ...
        for cid, on_case in on_map.items():
            ...
            domain_threshold = float(domain_overrides.get(domain, threshold))
            # Higher threshold => stricter router_on preference.
            off_ratio_target = max(0.0, min(1.0, 1.0 - domain_threshold))
            use_off = _hash_unit(f"{domain}:{cid}") <= off_ratio_target
    ...
    decision = "GO_W5_E3_HYBRID_ROUTER_MIX" if recommended else "HOLD_W5_E2_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week5_confidence_router_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W5-E3 hybrid router mix sweep."
            if decision == "GO_W5_E3_HYBRID_ROUTER_MIX"
            else "Keep baseline router-on and continue to W5-E3 hybrid routing path."
        ),
