...
<#
.SYNOPSIS
  Spot-check MKM_Deployment_Axis_Isolation_Weekly scheduled task + SSOT tier4 registration.
#>
[CmdletBinding()]
param(
...
if (-not $OutJson) {
    $OutJson = Join-Path $root "reports\mkm_deployment_axis_isolation_weekly_scheduled_ops_readiness_v1_latest.json"
}

$runnerExpected = Join-Path $root "scripts\Invoke-MkmDeploymentAxisIsolationWeeklyRoutine_v1.ps1"
$stackPath = Join-Path $root "docs\final\artifacts\mkm_scheduler_solo_core_stack_v1.json"
$tier4Expected = "\\MKM_Deployment_Axis_Isolation_Weekly"
...
$payload = [ordered]@{
    schema                              = "mkm_deployment_axis_isolation_weekly_scheduled_ops_readiness_v1"
...
if ($null -ne $task) {
    ...
    if ($argStr -match 'Invoke-MkmDeploymentAxisIsolationWeeklyRoutine_v1\.ps1') {
