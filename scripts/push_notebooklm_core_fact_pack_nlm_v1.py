#!/usr/bin/env python3
"""Push MKM_CORE_FACT pack via nlm CLI (preserves source titles).

SSOT pack: reports/notebooklm_core_fact_sync_pack_v1/
...
PACK = ROOT / "reports" / "notebooklm_core_fact_sync_pack_v1"
URL_FILE = ROOT / "reports" / "notebooklm_core_fact_notebook_url_v1.txt"
OUT = ROOT / "reports" / "notebooklm_core_fact_push_nlm_latest.json"
...
def main() -> int:
    ...
    if not (PACK / "index.json").is_file():
        raise SystemExit("run build_notebooklm_core_fact_sync_pack_v1.py first")
    ...
    doc = {"schema": "notebooklm_core_fact_push_nlm_latest", "notebook_id": nb, "rows": rows}
