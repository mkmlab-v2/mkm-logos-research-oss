<#
.SYNOPSIS
  Apply phase-2 promotion (30%) when D35 decision says GO_PHASE2_30PCT.
#>
param(
    [string]$WorkspaceRoot = "C:\workspace",
    [string]$DecisionArtifact = "docs/final/artifacts/l1_inverse_decoder_mode_router_v3_phase2_promotion_decision_v1.json",
    [string]$PromotionEventOut = "docs/final/artifacts/l1_inverse_decoder_mode_router_v3_phase2_promotion_event_latest.json",
...
if ($decision -ne "GO_PHASE2_30PCT") {
    Write-Host "Decision is $decision. Phase2 promotion not applied."
    exit 0
}
...
$event = [ordered]@{
    schema = "l1_inverse_decoder_mode_router_v3_phase2_promotion_event_v1"
    generated_at_utc = (Get-Date).ToUniversalTime().ToString("o")
    decision_artifact = $decisionAbs
    decision = $decision
    applied = (-not $DryRun)
    target_phase = "phase_2"
    target_traffic_pct = 30
...
Write-Host "Phase2 promotion event written: $eventAbs"
