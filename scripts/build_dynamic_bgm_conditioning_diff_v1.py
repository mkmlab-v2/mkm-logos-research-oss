...
def apply_patch_to_conditioning(base: dict[str, Any], diff: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, val in patch.items():
        cond[key] = val
    merged.setdefault("upstream", {})["sasang_primary"] = diff.get("inputs", {}).get("sasang", merged.get("upstream", {}).get("sasang_primary"))
    exp = str(merged.get("provenance", {}).get("experiment_id") or "dynamic")
    merged.setdefault("provenance", {})["experiment_id"] = f"{exp}_hp{int(float(diff.get('inputs', {}).get('hp_pct', 0)) * 100):03d}"
    merged["notes"] = f"dynamic_bgm: hp_pct={diff.get('inputs', {}).get('hp_pct')} mood patch"
    return merged
...
def main() -> int:
    ...
    diff = build_dynamic_diff(base_conditioning=base, hp_pct=args.hp_pct, sasang=args.sasang or None)
