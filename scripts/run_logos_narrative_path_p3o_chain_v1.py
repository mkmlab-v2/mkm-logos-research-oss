#!/usr/bin/env python3
"""P3o — Narrative 56→60 + bloom slice + inter-hop (B-track, HYPO).

Closes P3 round-1 ladder 20→60. Reproducible:
  py scripts/run_logos_narrative_path_p3o_chain_v1.py
"""
...
def main() -> int:
    ...
    if not args.skip_pytest:
        proc = subprocess.run(
            [sys.executable, "-m", "pytest", "tests/test_logos_narrative_path_p3o_v1.py", "-q"],
            cwd=ROOT,
            check=False,
        )
        if proc.returncode != 0:
            return proc.returncode
        print("OK: pytest p3o narrative")
