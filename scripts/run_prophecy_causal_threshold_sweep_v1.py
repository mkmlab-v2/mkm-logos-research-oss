# @MKM12-METADATA
# Type: Logic
# Vector: {S:0.85, L:0.85, K:0.45, M:0.3}
# Balance: 91
# Purpose: Causal prior-return threshold sweep on prophecy panel
# Keywords: prophecy, causal, threshold, sweep, prior return, btrack
#!/usr/bin/env python3
"""Causal threshold sweep for prior-completed-return directional rule.

Rule (per row, per instrument):
  if prior_return <= low_thr: bear
  elif prior_return >= high_thr: bull
  else: neutral

Prior return is from the *previous completed daily session* (no eval-day lookahead).
This is B-track research-only evidence and not live routing logic.
"""
...
DEFAULT_OUT = ROOT / "docs" / "final" / "artifacts" / "prophecy_causal_threshold_sweep_v1_latest.json"
SCHEMA = "prophecy_causal_threshold_sweep_v1"
...
def _metrics_from_preds(rows: list[dict[str, Any]], preds: list[str]) -> dict[str, Any]:
    n = 0
    hit = 0
...
def _preds_for_thresholds(
    rows: list[dict[str, Any]],
    *,
    kospi_map: dict[str, float],
    btc_map: dict[str, float],
    low_thr: float,
    high_thr: float,
    ...
    for r in rows:
        ...
        mp = kospi_map if inst == "kospi" else btc_map
        pr = mp.get(ed)
        ...
        if p != old:
            changed += 1
    return preds, changed, missing


def main() -> int:
    ap = argparse.ArgumentParser(description="Causal prior-return threshold sweep (B-track).")
    ...
    ap.add_argument(
        ...
        help="Comma-separated threshold candidates (return fractions).",
    )
    ...
    base_metrics = _metrics_from_preds(rows, base_preds)
    ...
    for lo, hi in pairs:
        preds, changed, missing = _preds_for_thresholds(rows, kospi_map=km, btc_map=bm, low_thr=lo, high_thr=hi)
        m = _metrics_from_preds(rows, preds)
    ...
    always_bull_rate = sum(1 for r in rows if str(r.get("actual_direction") or "").strip().lower() == "bull") / len(rows) if rows else 0.0
    ...
    out = {
        ...
        "baseline": {"lane_id": "current_panel_prediction", "metrics": base_metrics},
        "control": {
            "lane_id": "always_bull_control",
            "price_directional_hit_rate": round(always_bull_rate, 6),
            "note": "Majority-class control on this panel.",
        },
        "best_candidate": best,
        "top_candidates": results_sorted[: max(1, int(args.top_k))],
        "sweep_note": "Causal prior-completed-return thresholds only; no same-day lookahead.",
    }
    ...
    if best:
        print(
            f"BASE={base_metrics['price_directional_hit_rate']} "
            f"BEST={best['metrics'].get('price_directional_hit_rate')} "
            f"lo={best['low_thr']} hi={best['high_thr']} changed={best['changed_rows']}"
        )
    return 0
