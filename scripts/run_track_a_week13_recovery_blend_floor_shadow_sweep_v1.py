...
# Balance: 93
# Purpose: Run Week-13 E2 recovery-first replay blend with floor shadow sweep.
# Keywords: track_a, week13, recovery_blend, floor_shadow, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week13_recovery_blend_floor_shadow_sweep_v1.json"
...
def _read_e2_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[int]]:
    ...
    e2 = next((x for x in experiments if str(x.get("id")) == "W13-E2"), {})
    cfg = e2.get("config") or {}
    recovery_weight_grid = [float(x) for x in (cfg.get("recovery_weight_grid") or [])]
    ...
    return (
        recovery_weight_grid or [0.6, 0.65, 0.7],
        floor_shadow_margin_grid or [0.0, 0.002, 0.004],
...
def main() -> int:
    ...
    recovery_weights, floor_margins, candidate_pools = _read_e2_config(pack_doc)
            ...
            for candidate_pool in candidate_pools:
                ...
                for cid, on_case in on_map.items():
                    ...
                    if off_s < floor_shadow:
                        ...
                        total += 1
                        continue
                    recovery_component = (off_j - on_j) * recovery_weight
                    saving_component = (off_s - on_s) * (1.0 - recovery_weight)
                    noise = (_hash_unit(f"w13-e2:{cid}:{recovery_weight}:{floor_margin}:{candidate_pool}") - 0.5) * 0.001
                    score = recovery_component + saving_component + noise
                    threshold = 0.0004 * max(1, candidate_pool - 2)
                    use_off = score > threshold
                    if use_off and (off_j >= on_j):
                        recovery_win_cases += 1
                ...
                row = {
                    ...
                    "recovery_win_cases": recovery_win_cases,
    ...
    decision = "GO_W13_E3_CONFIDENCE_ANNEAL" if recommended else "HOLD_W13_E2_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week13_recovery_blend_floor_shadow_sweep_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "active_a_report": str(args.active_a_report),
            "pack": str(args.pack),
            "recovery_weight_grid": recovery_weights,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W13-E3 confidence-penalty-anneal sweep."
            if decision == "GO_W13_E3_CONFIDENCE_ANNEAL"
            else "Keep baseline and continue to W13-E3 path with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
