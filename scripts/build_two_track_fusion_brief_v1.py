...
def main():
    ...
    ap.add_argument('--multi-symbol-gate-summary-json', default='docs/final/artifacts/multi_symbol_gate_summary_latest.json')
    a=ap.parse_args()
    fp,op=Path(a.fusion_report_json),Path(a.output_json)
    mp=Path(a.multi_symbol_json); sp=Path(a.multi_symbol_selector_json); gp=Path(a.gematria_ablation_json); sgp=Path(a.multi_symbol_gate_summary_json)
    ...
    gate_summary=(sg.get('summary') or {}) if isinstance(sg.get('summary'),dict) else {}
    out={'schema':'two_track_fusion_brief_v1','generated_at_utc':now(),'research_only':True,'promotion_required':True,'source_track':'K','headline':'Two-track fusion status',
         'cards':[{'id':'k_summary','value':k.get('summary',{})},
                  ...
                  {'id':'multi_symbol_gate_status','value':gate_summary.get('status')},
                  {'id':'multi_symbol_any_promotion_hold','value':gate_summary.get('any_promotion_hold')}],
         ...
         'sources':{'multi_symbol_json':str(mp) if mp.is_file() else None,'multi_symbol_selector_json':str(sp) if sp.is_file() else None,'gematria_ablation_json':str(gp) if gp.is_file() else None,'multi_symbol_gate_summary_json':str(sgp) if sgp.is_file() else None}}
