...
# Balance: 93
# Purpose: Run Week-20 E3 coupled veto throttle with widened domain budget sweep.
# Keywords: track_a, week20, widened_domain_budget, veto, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week20_widened_domain_budget_veto_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float], list[int]]:
    ...
    e3 = next((x for x in experiments if str(x.get("id")) == "W20-E3"), {})
    ...
    return (
        confidence_gate_grid or [0.49, 0.53, 0.57],
        throttle_quota_grid or [0.16, 0.18, 0.2],
        rollback_penalty_grid or [0.035, 0.05, 0.065],
        domain_quota_cap_grid or [4, 5, 6],
    )
...
def main() -> int:
                ...
                for domain_quota_cap in domain_quota_caps:
                    ...
                    for cid, on_case in on_map.items():
                        ...
                        if not off_case:
                            ...
                            total += 1
                            continue
                        confidence = _hash_unit(f"w20-e3:conf:{cid}:{confidence_gate}:{domain_quota_cap}")
    ...
    decision = "GO_W20_E4_POLICY_REPLAY" if recommended else "HOLD_W20_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week20_widened_domain_budget_veto_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W20-E4 policy replay."
            if decision == "GO_W20_E4_POLICY_REPLAY"
            else "Keep baseline and continue to W20-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
