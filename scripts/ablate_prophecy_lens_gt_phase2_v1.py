#!/usr/bin/env python3
"""[HYPO] Phase2 lens gt R&D — anti-tie constraint + bear-day weighted objective ablation."""
...
DEFAULT_OUT = ROOT / "reports/prophecy_lens_gt_phase2_ablation_v1_latest.json"
...
VARIANTS: list[dict[str, Any]] = [
    {
        "id": "prod_beat_bull_first_baseline",
        "args": ["--train-objective", "beat_bull_first"],
    },
    {
        "id": "anti_tie_margin_gt0",
        "args": ["--train-objective", "beat_bull_anti_tie"],
    },
    {
        "id": "anti_tie_min_margin_0p01",
        "args": ["--train-objective", "beat_bull_anti_tie", "--train-min-margin-vs-bull", "0.01"],
    },
    {
        "id": "anti_tie_min_margin_0p033",
        "args": ["--train-objective", "beat_bull_anti_tie", "--train-min-margin-vs-bull", "0.033333"],
    },
    {
        "id": "bear_day_weighted_first",
        "args": ["--train-objective", "bear_day_weighted_first"],
    },
    {
        "id": "bear_day_weighted_min_margin_0p01",
        "args": [
            "--train-objective",
            "bear_day_weighted_first",
            "--train-min-margin-vs-bull",
            "0.01",
        ],
    },
    {
        "id": "bear_day_weighted_strict_first",
        "args": ["--train-objective", "beat_bull_strict_first"],
    },
]
...
def main() -> int:
    ...
    ap.add_argument("--tmp-dir", type=Path, default=ROOT / "reports/tmp_lens_gt_phase2_ablation")
    args = ap.parse_args()
    ...
    report: dict[str, Any] = {
        "schema": "prophecy_lens_gt_phase2_ablation_v1",
        "generated_at_utc": _utc(),
        "research_only": True,
        "hypothesis_tag": "[HYPO]",
        "track_a_auto_promote": False,
        "send_gate": "HOLD",
        "phase": "2_anti_tie_and_bear_day_weighted",
        ...
        "variants": results,
        "verdict_ko": (
            f"phase2 anti-tie+bear-weighted; best_gt={(best_gt or {}).get('variant_id')} "
            f"gt_frac={(best_gt or {}).get('fraction_test_beats_always_bull_gt')} "
            f"max_margin={(best_gt or {}).get('max_fold_margin_vs_prod_control')}"
        ),
        "ledger_line": (
            f"Lens gt phase2: best_gt={(best_gt or {}).get('variant_id')} "
            f"gt_frac={(best_gt or {}).get('fraction_test_beats_always_bull_gt')} "
            f"mean={(best_gt or {}).get('mean_test_accuracy')}; prod gt unchanged; no oper promotion."
        ),
        "operator_lines": [
            "- [LENS-P2] research_only; anti-tie + bear-day-weighted objectives.",
            f"- [LENS-P2] best_gt={(best_gt or {}).get('variant_id')} "
            f"gt_frac={(best_gt or {}).get('fraction_test_beats_always_bull_gt')} "
