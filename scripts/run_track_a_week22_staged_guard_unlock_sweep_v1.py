...
# Balance: 93
# Purpose: Run Week-22 E1 staged guard unlock sweep.
# Keywords: track_a, week22, staged_guard, unlock, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week22_staged_guard_unlock_sweep_v1.json"
...
def _read_e1_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float], list[str]]:
    ...
    return (
        ...
        guard_relax_margin_grid or [0.0015, 0.0025, 0.0035],
...
def main() -> int:
            ...
            for guard_relax_margin in guard_relax_margins:
                ...
                for cid, on_case in on_map.items():
                    ...
                    if off_j < guard_threshold:
                        ...
                        total += 1
                        continue
                    unlock_signal = _hash_unit(f"w22-e1:unlock:{cid}:{unlock_ratio}:{jaccard_guard}:{guard_relax_margin}")
    ...
    decision = "GO_W22_E2_CONFIDENCE_FIRST_CROSSOVER" if recommended else "HOLD_W22_E1_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week22_staged_guard_unlock_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W22-E2 confidence-first crossover sweep."
            if decision == "GO_W22_E2_CONFIDENCE_FIRST_CROSSOVER"
            else "Keep baseline and reassess Week-22 grid before E2."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
