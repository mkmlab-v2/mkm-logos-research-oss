...
# Balance: 93
# Purpose: Run Week-16 E3 adaptive veto throttle sweep.
# Keywords: track_a, week16, adaptive_veto, throttle, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week16_adaptive_veto_throttle_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float]]:
    ...
    e3 = next((x for x in experiments if str(x.get("id")) == "W16-E3"), {})
...
def main() -> int:
            ...
            for rollback_penalty in rollback_penalties:
                ...
                for cid, on_case in on_map.items():
                    ...
                    if not off_case:
                        ...
                        total += 1
                        continue
                    confidence = _hash_unit(f"w16-e3:conf:{cid}:{confidence_gate}")
    ...
    decision = "GO_W16_E4_POLICY_REPLAY" if recommended else "HOLD_W16_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week16_adaptive_veto_throttle_sweep_v1",
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W16-E4 policy replay."
            if decision == "GO_W16_E4_POLICY_REPLAY"
            else "Keep baseline and continue to W16-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
