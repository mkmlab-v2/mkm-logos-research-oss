...
# Balance: 93
# Purpose: Consolidate Week-18 experiments into final policy replay decision.
# Keywords: track_a, week18, policy_replay, decision, commercialization
#!/usr/bin/env python3
...
E1 = ROOT / "docs" / "final" / "artifacts" / "track_a_week18_guard_window_unlock_sweep_v1.json"
E2 = ROOT / "docs" / "final" / "artifacts" / "track_a_week18_budgeted_crossover_shadow_sweep_v1.json"
E3 = ROOT / "docs" / "final" / "artifacts" / "track_a_week18_coupled_veto_domain_budget_sweep_v1.json"
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week18_policy_replay_v1.json"
...
def _read_e4_config(pack: dict[str, Any]) -> tuple[float, bool]:
    ...
    e4 = next((x for x in exps if str(x.get("id")) == "W18-E4"), {})
...
def main() -> int:
    ...
    decision = (
        "GO_W18_POLICY_REPLAY"
        if policy_floor_ok and integrity_ok and quality_tradeoff_ok
        else "HOLD_W18_POLICY_REPLAY"
    )

    out_doc = {
        "schema": "track_a_week18_policy_replay_v1",
        ...
        "week18_summary": {
            "entry_decision": (pack.get("entry_condition") or {}).get("week17_decision"),
        ...
        "decision": decision,
        "next_action": (
            "Keep Track A commercialization HOLD and design Week-19 hypothesis pack."
            if decision == "HOLD_W18_POLICY_REPLAY"
