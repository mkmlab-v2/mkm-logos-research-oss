#!/usr/bin/env python3
"""Register Clinician NL notebook in notebooklm-mcp library + select as active."""
...
OUT = ROOT / "reports" / "notebooklm_clinician_mcp_register_latest.json"

NOTEBOOK_MCP_ID = "20-clinician-physician-gold-20"
NOTEBOOK_NAME = "20_CLINICIAN_PHYSICIAN_GOLD_2026Q2"
DESCRIPTION = (
    "JEMA-AI Clinician physician_gold — SOAP·CDSS·care bundle·dual-lane. "
    "consumer_survey_only·Track A KPI cross-cite forbidden."
)
...
def _pick_canonical_id(candidates: list[dict]) -> str:
    """Prefer exact NOTEBOOK_MCP_ID, then shortest id (avoid -1 suffix duplicates)."""
            ...
            if existing:
                ...
                if prune_duplicates and len(existing) > 1:
                    dup_ids = [
                        x["id"]
                        for x in existing
                        if x.get("id") and x["id"] != resolved_id
                    ]
                ...
                add_args = {
                    ...
                    "use_cases": [
                        "한의사 진료 보조 브리핑",
                        "CDSS·care bundle 경계 확인",
                        "consumer_survey_only cross-talk 방지",
                    ],
                }
...
def main() -> int:
    ...
    ap.add_argument(
        "--no-select",
        action="store_true",
        help="register only; keep current active notebook",
    )
    ap.add_argument(
        "--no-prune-duplicates",
        action="store_true",
        help="do not remove duplicate library entries for the same notebook URL",
    )
    args = ap.parse_args()
