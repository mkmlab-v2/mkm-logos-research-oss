...
# Balance: 93
# Purpose: Run Week-13 E3 confidence rollback with penalty annealing sweep.
# Keywords: track_a, week13, confidence_anneal, rollback, sweep
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week13_confidence_penalty_anneal_sweep_v1.json"
...
def _read_e3_config(pack_doc: dict[str, Any]) -> tuple[list[float], list[float], list[float]]:
    ...
    e3 = next((x for x in experiments if str(x.get("id")) == "W13-E3"), {})
    cfg = e3.get("config") or {}
    confidence_gate_grid = [float(x) for x in (cfg.get("confidence_gate_grid") or [])]
    quota_grid = [float(x) for x in (cfg.get("quota_grid") or [])]
    anneal_penalty_grid = [float(x) for x in (cfg.get("anneal_penalty_grid") or [])]
    return (
        confidence_gate_grid or [0.58, 0.62, 0.66],
        quota_grid or [0.08, 0.1, 0.12],
        anneal_penalty_grid or [0.025, 0.04, 0.055],
    )
...
def main() -> int:
    ...
    confidence_gates, quotas, anneal_penalties = _read_e3_config(pack_doc)
    ...
    for confidence_gate in confidence_gates:
        for quota in quotas:
            for anneal_penalty in anneal_penalties:
                ...
                anneal_blocked = 0
                total = 0
                quota_limit = max(0, int(total_cases * max(0.0, min(0.5, quota))))
                for cid, on_case in on_map.items():
                    ...
                    if not off_case:
                        ...
                        total += 1
                        continue
                    confidence = _hash_unit(f"w13-e3:conf:{cid}:{confidence_gate}")
                    ...
                    anneal_score = (jaccard_drop * 0.7) + (saving_gap * 0.3)
                    use_off = anneal_score <= anneal_penalty
                    if not use_off:
                        anneal_blocked += 1
                ...
                row = {
                    ...
                    "anneal_blocked_cases": anneal_blocked,
    ...
    decision = "GO_W13_E4_POLICY_REPLAY" if recommended else "HOLD_W13_E3_NO_VIABLE"

    out_doc = {
        "schema": "track_a_week13_confidence_penalty_anneal_sweep_v1",
        ...
        "inputs": {
            ...
            "quota_grid": quotas,
            "anneal_penalty_grid": anneal_penalties,
        ...
        "decision": decision,
        "next_action": (
            "Proceed to W13-E4 policy replay."
            if decision == "GO_W13_E4_POLICY_REPLAY"
            else "Keep baseline and continue to W13-E4 replay with HOLD evidence."
        ),
        "runs": sorted(
            runs,
            key=lambda r: (
