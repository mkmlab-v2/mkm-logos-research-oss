#!/usr/bin/env python3
"""P3m — Narrative 48→52 + bloom slice + inter-hop (B-track, HYPO).

Reproducible:
  py scripts/run_logos_narrative_path_p3m_chain_v1.py
"""
...
def main() -> int:
    ...
    if not args.skip_pytest:
        proc = subprocess.run(
            [sys.executable, "-m", "pytest", "tests/test_logos_narrative_path_p3m_v1.py", "-q"],
            cwd=ROOT,
            check=False,
        )
        if proc.returncode != 0:
            return proc.returncode
        print("OK: pytest p3m narrative")
