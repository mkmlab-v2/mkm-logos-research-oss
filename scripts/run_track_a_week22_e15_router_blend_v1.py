...
# Balance: 93
# Purpose: Run Week-22 E1.5b using evaluate_report router-blend candidate.
# Keywords: track_a, week22, e15b, router_blend, evaluate_report
#!/usr/bin/env python3
...
OUT = ROOT / "docs" / "final" / "artifacts" / "track_a_week22_e15_router_blend_v1.json"
...
def main() -> int:
    ...
    ap.add_argument("--drop-pp", type=float, default=1.2)
    ap.add_argument("--saving-gain-pp", type=float, default=1.0)
    args = ap.parse_args()
    ...
    report = evaluate_report(
        ...
        router_blend_allow_nonrisk_jaccard_drop_pp=float(args.drop_pp),
        router_blend_min_saving_gain_pp=float(args.saving_gain_pp),
    )
    ...
    decision = "GO_W22_E15B_ROUTER_BLEND" if all(gate.values()) else "HOLD_W22_E15B_NO_VIABLE"

    blend_applied_cases = 0
    for row in metrics.get("cases", []) or []:
        route = row.get("route") or {}
        if isinstance(route, dict) and route.get("router_blend_applied") is True:
            blend_applied_cases += 1

    out_doc: dict[str, Any] = {
        "schema": "track_a_week22_e15_router_blend_v1",
        "generated_at_utc": _now_utc(),
        "inputs": {
            "input": str(args.input),
            "drop_pp": float(args.drop_pp),
            "saving_gain_pp": float(args.saving_gain_pp),
            "floors": {
        ...
        "metrics": {
            ...
            "blend_applied_cases": blend_applied_cases,
        },
        "gate": gate,
        "decision": decision,
    }
    ...
    print(json.dumps({"ok": True, "out": str(out_path), "decision": decision, "blend_applied_cases": blend_applied_cases}, ensure_ascii=False))
    return 0
