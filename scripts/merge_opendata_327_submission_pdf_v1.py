...
def main() -> int:
    ...
    doc = {
        ...
        "cover_part_a": "manual_kstartup_form_required",
        "merge_order": result["parts"],
        "size_mb": result["size_mb"],
        "max_size_mb": 30,
        "under_size_limit": result["size_mb"] < 30,
        "boundary_ack": "Merged B(+C+D) only. Insert K-Startup cover (A) before upload.",
    }
    ...
    if not args.no_gates_update and GATES_LATEST.is_file():
        ...
        ws["step_4_merge"] = "bcd_merged_no_cover_a"
        ws["step_5_size_check"] = "ok_under_30mb" if doc["under_size_limit"] else "over_30mb_review"
