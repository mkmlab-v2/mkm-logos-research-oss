...
| 라벨 | 의미 | 사용 기준 |
|------|------|-----------|
| `[FACT]` | 검증·추적 가능한 진술 | 대상 SSOT 경로, `loc/refs/status`, 스크립트·pytest·로그 아티팩트로 역추적 가능 |
| `[HYPO]` | 잠정·대기·TBD | 작업 표의 `TBD`, 대기 큐, Proxy 후보; 승격 전까지 단정 금지 |
| `[VISION]` | 운영·완료 목표 | SOP·완료선·잠금 규칙; 목표 상태이지 현재 상태 단정 아님 |
| `[NON-MEDICAL]` | 비의료 고지 | 건강·체질 관련 문구 포함 시 필수 |

체크리스트 본문의 `ENTRY_*` 표·절차는 위 라벨을 기준으로 읽는다: 앵커 미확정 행은 `[HYPO]`, 스키마 테스트·경로·로그로 닫힌 항목은 `[FACT]`에 해당한다.
