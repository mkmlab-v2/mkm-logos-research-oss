...
**Smartfarm B2B 공개 URL:** **`https://farm.jema-ai.com`** (본선). 허브 `jema-ai.com/smartfarm`·`app.jema-ai.com/smartfarm`은 Cloudflare/nginx가 `/consumer`로 보낼 수 있음 — 대외 CTA·허브 카드는 **farm 서브도메인**만 사용. 앱 middleware는 허브 `/smartfarm` → `farm.jema-ai.com` 308(배포본).
...
**모노레포 공통 — 웹·제안서·랜딩 보안·IP·카피:** Hostinger/mkmlife·no1kmedi·마케팅 정적 자산을 편집할 때 **`docs/final/PUBLIC_FACING_SECURITY_AND_IP_COPY_CHECKLIST_V1.md`**(워크스페이스 루트 `C:\workspace`)를 함께 본다 — API 키·내부 호스트 비노출, 핵심 이론·미공개 파이프라인 비노출, 과장·규제 민감 표현 회피. Track C 상용 상한은 **`docs/final/TRACK_C_IP_BUSINESS_PLAN_2026-04-17.md`** 와 정합. 본 §10은 **UI 배지·컴포넌트 계약**에 특화되어 있고, 전 채널 공통 규칙은 전자 체크리스트가 우선한다.
...
- 랜딩·스토어·외부 PDF까지 포함한 **비밀·IP·톤** 점검은 **`docs/final/PUBLIC_FACING_SECURITY_AND_IP_COPY_CHECKLIST_V1.md`** 로 한 번 더 고정한다(본 §10.1 배지와 중복이 아니라 **상위 채널 공통** 레이어).
