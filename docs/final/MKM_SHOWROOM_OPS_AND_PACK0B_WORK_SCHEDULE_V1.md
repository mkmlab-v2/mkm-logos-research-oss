...
- [x] **2026-05-14** **`--run-inference-eval`** — `--profile golden_fit_smoke`와 동일 세션에서는 **`--inference-profile-key` 생략 시 자동으로 train 프로파일과 정렬**(레포 `run_pack0b_deterministic_lora_pipeline_v1.py` `resolve_inference_profile_key`). 명시 `train_default`+다른 `--profile` 조합은 여전히 불일치 가능. 산출 `reports/myeongri_deterministic_lora_locked_eval_inference_eval_latest.json` `ok=true`, `alignment_pass_rate=0.0`(2-step 스모크·품질 판정 아님).  
...
| 2026-05-17 | 3 | **Git 위생 융합(한 줄):** `git fetch gitea`로 `dev`/`main` 정렬 확인; `_pr_sasang_promotion`의 `main`을 `internal/main`과 맞춤; bare에서 **`dev`가 `main`과 동일 SHA**이면 `git push gitea --delete dev` 후 **`git fetch gitea --prune`**으로 stale 원격 추적 제거; 로컬 `dev`는 **`gitea/main` 추적** 유지; 장문 브리핑은 스냅샷 대신 **CENTRAL 체크포인트 + 본 표**만 갱신 |
| 2026-05-17 | 3 | **실행:** `ls-remote`로 `main`=`dev`(동일 SHA) 확인 → **`git push gitea --delete dev`** OK → **`git fetch gitea --prune`**; 원격 **`refs/heads/dev` 없음**(중복 브랜치 제거) |
...
| 2026-05-14 | 6 | 픽스처 `--run-train` 2step TinyLlama→`pack0b_fixture_smoke_latest`; inference-eval ok(`alignment_pass_rate=0` 스모크); pytest pack0b+copy_guard 6 pass |
| 2026-05-14 | 6 | `run_pack0b…`: `--inference-profile-key` 생략 시 `--profile`과 자동 정렬(`resolve_inference_profile_key`)+pytest |
