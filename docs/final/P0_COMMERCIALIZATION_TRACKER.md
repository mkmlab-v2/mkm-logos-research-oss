...
| 예언 prior 임계값 스윕 (B-track, [HYPO]) | `docs/final/artifacts/prophecy_prior_threshold_sweep_v1_latest.json` — 동일 `btrack_prophecy_score`·KOSPI CSV 위에서 `run_prophecy_restoration_spike.py`의 `--prior-return-threshold` 격자 재실행 요약; 본선·실매매 트리거 아님 |
...
| 경로 스모크(로컬) | `scripts/verify_p0_constitution_gate_paths.ps1` — `$required` 배열 기준 **경로 존재만** 점검(OK 출력의 `N checked`는 스크립트 변경 시 변동 — **고정 개수를 브리핑 FACT로 쓰지 말 것**). **2026-04-18**: `$required`를 **본 워크스페이스 실제 파일**과 정합; main 전체 트리에만 있는 스크립트는 목록에서 빠질 수 있음 — 제외 목록은 스크립트 상단 주석·복구 시 목록 재확대. |
| Bio 논문 SNP 사이드카 조인(§8.1) | `tests/test_bio_paper_snp_join_chain_smoke_v1.py` pytest 성공; `.github/workflows/bio-paper-snp-sidecar-smoke.yml` 및 `dual-regime-integrity.yml` 해당 단계; 매핑 선행 점검 `scripts/check_bio_paper_snp_mapping_coverage_v1.py`(선택 `--strict` / exit **2**) |
| VPS L1 스텁 본선 RTT 후보(측정 후) | `docs/final/artifacts/bench_l1_api_load_summary_vps_latest.json` + `docs/final/artifacts/bench_runs/bench_l1_api_load_vps_*.json` — 절차 `docs/final/BENCH_L1_API_LOAD_VPS_RUNBOOK.md`; 로컬 `bench_l1_api_load_summary_latest.json`과 **혼동 금지**. **최신(2026-05-16):** `vps-mkmlife` `/opt/mkm-destiny-ai-41e38ec6` triplet 3회 pull → `compression_board_ms_vps_bench_triplet_v1_latest.json` **p95 median ~710ms** (665–847), `error_rate=0.0`. **이전 스냅샷(2026-04-10, ~293ms)과 동일 부하·시점 아님** — 대외 인용 시 `generated_at_utc`만. **운영 SLA 최종 확정**은 본 산출물·승격 게이트와 별도 |
