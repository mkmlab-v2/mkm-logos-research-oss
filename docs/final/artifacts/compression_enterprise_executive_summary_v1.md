...
|---------------------|--------|--------|
| Track A token economy at policy floor | **47.12%** global saving; `ultra_saving_policy_ok: true` @ floor **0.47** | `reports/constitution/btrack_pilot/ultra_compression_kpi_summary_latest.json` → `active_kpi` |
| Bench reconstruction proxy (Jaccard, not “meaning %”) | avg **~0.885**, min **0.667** (40 cases) | `docs/final/artifacts/MULTILENS_ULTRA_COMPRESSION_ACTIVE_REPORT_V1.json` |
| Sensitive-term integrity on bench | avg **1.0**, violations **0** | same active report |
| Health-domain pinpoint (cmp2_014) | Jaccard **0.875** (was **0.625** pre targeted shard soft-terms) | same active report, case `cmp2_014` |
| Policy decision record | floor **0.49 → 0.47** after master codebook lexicon join | `docs/final/artifacts/track_a_policy_floor_decision_v1.json` |
...
1. **Narrow corridor:** We publish an explicit saving floor (**0.47**) and report pass/fail against it—not a post-hoc boast after the fact.  
...
> Our compression API is designed for enterprise operators who must cut LLM token spend without silent quality drift. On our current frozen benchmark (40 cases), Track A delivers approximately **47%** token reduction while meeting a published policy floor of **0.47**, with reconstruction fidelity measured by Jaccard and sensitive-term integrity at **1.0** on that bench. We ship decision JSON, KPI summaries, and active reports so your team can reproduce results—not marketing slides alone. This is observability and governance for compression profiles; it is not a trading signal, clinical tool, or guarantee of lossless restoration in production.

---

## Copy-paste paragraph (KO, 내부 제안서 초안)

> MKM 압축 레이어는 “더 똑똑한 모델”이 아니라 **측정·게이트·동결 아티팩트**를 제공합니다. 현행 고정 벤치(40건) 기준 Track A는 전역 토큰 절감 **약 47%**, 정책 하한 **0.47** 충족(`ultra_saving_policy_ok`), 민감어 무결성 **1.0**입니다. 복원 품질은 Jaccard(단어 겹침 프록시)로 보고하며 “의미 N%”로 단정하지 않습니다. 건강 도메인 대표 케이스(cmp2_014)는 **0.875**까지 개선했으나, 이는 샤드·정책 핀포인트의 결과이며 전 도메인 자동 보장이 아닙니다. 본선 트레이딩·실매매와 벤치 KPI는 합선하지 않습니다.
...
py scripts/report_multilens_performance_eval.py  # if refreshing KPI summary separately
```
