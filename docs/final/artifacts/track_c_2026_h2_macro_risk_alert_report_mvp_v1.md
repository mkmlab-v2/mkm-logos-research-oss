...
- **status:** `DRAFT_AUTO_FILLED` — 마지막 자동 갱신 UTC `2026-05-03T16:37:03Z`. 목차·근거 경로 동결; §2 본문은 스크립트가 아티팩트에서 채움.
...
_본 절은 `build_track_c_macro_risk_mvp_filled_v1.py`가 디스크 아티팩트에서 생성했습니다. 생성 시각(UTC): `2026-05-03T16:37:03Z`._
...
| 불확실성 | n8n 일일 점검 `overall=fail`; `n8n_health=fail`; 스냅샷 시각 `2026-05-03T16:37:00.4700394Z`; Pre-News 작업 건강도 `healthy=True`, `result_category=OK` (`generated_at_utc=2026-05-02T21:50:03Z`); 주간 리포트 창 `7d`, 창 내 실행 `14`회 (`generated_at_utc=2026-05-02T21:30:05Z`); 과거 스냅샷은 현재 시장과 다를 수 있음; 법무 검토 전 대외 확정 금지. |
...
1. **권장 한 방:** `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/Run-TrackCRecommendedAutoChain_v1.ps1` — n8n 일일 점검(실패 시에도 계속) → MVP/B2B 갱신 → copy guard → 오케스트레이터 번들·noop 스모크.
2. 또는 단독: `py scripts/build_track_c_macro_risk_mvp_filled_v1.py` 로 §2 자동 채움(아티팩트 기준); 필요 시 `--dry-run`.
3. 위 §3 경로에서 **최신 JSON** 존재 확인 (`mtime`/파이프라인 exit 0).
4. §2 수동 보정 시에도 인용 각주에 §3 경로 병기.
5. `py scripts/check_track_c_copy_guard_v1.py` 로 본 파일 및 대외 PDF 초안 복사본 검사.
6. 법무 검토 후 대외 버전 `APPROVED` 표기.
