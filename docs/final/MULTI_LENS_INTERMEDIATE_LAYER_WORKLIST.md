...
**Athena CENTRAL 체크포인트·Logos insight bundle v1:** `tests/test_athena_checkpoint.py` 및 `tests/test_logos_insight_bundle_schema_v1.py`·`tests/test_build_logos_insight_bundle_v1.py`는 `run_fact_lock_bundle.ps1` **3d2·3d3**(메타 봉투 직후)·CI `dual-regime-integrity.yml`(Athena §28·Logos 단계)과 정합. non-degraded 스키마 예: `docs/final/schemas/logos_insight_bundle_v1.non_degraded.example.json`, 재생성 `py scripts/materialize_logos_insight_bundle_non_degraded_example_v1.py`.
