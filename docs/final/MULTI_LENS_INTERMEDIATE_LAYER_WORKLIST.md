...
**3d3a — STT audit + 쇼룸 Trust Visualization thin slice:** Logos insight bundle pytest 직후 `tests/test_append_stt_routing_audit_log_v1.py`·`tests/test_build_showroom_trust_visualization_slice_v1.py`(CI `STT routing audit log v1 + showroom trust viz thin slice` 단계와 동일; `run_fact_lock_bundle.ps1` **3d3a**). 로컬 생략: `-SkipSttRoutingAuditShowroomTrustSlice`. 산출(로컬·gitignore): `scripts/build_showroom_trust_visualization_slice_v1.py` → `projects/bitcoin-trading/ops/windows-rehearsal/jemaai-cloud-mvp/showroom_trust_visualization_slice_v0.json` + 정적 `public_showroom_trust_visualization_v0.html`.
