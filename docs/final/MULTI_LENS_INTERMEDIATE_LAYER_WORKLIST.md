...
**3d3a — STT audit + 쇼룸 Trust Visualization thin slice:** `tests/test_stt_routing_audit_log_schema_v1.py`·`tests/test_append_stt_routing_audit_log_v1.py`·`tests/test_build_showroom_trust_visualization_slice_v1.py`(CI `dual-regime-integrity.yml` 스텝 `STT routing audit log v1 + showroom trust viz thin slice`와 동일 커맨드). **`run_fact_lock_bundle.ps1`**에서는 Logos insight bundle pytest **직후**에 실행(생략: `-SkipSttRoutingAuditShowroomTrustSlice`). **CI YAML 순서**에서는 `Lens music prompt PoC M26-M30` 묶음 **직후**이며 Logos insight bundle보다 앞선다 — pytest는 격리. 산출(로컬·gitignore): `scripts/build_showroom_trust_visualization_slice_v1.py` → `projects/bitcoin-trading/ops/windows-rehearsal/jemaai-cloud-mvp/showroom_trust_visualization_slice_v0.json` + 정적 `public_showroom_trust_visualization_v0.html`.
