...
**보강 (2026-06-20 — Domain adapter shallow routing `[HYPO]`):** `docs/final/artifacts/mkm_ops_sync_bridge_v1.json` `domain_adapters`(pixel_battalion·lens_audio) · `scripts/check_mkm_ops_sync_bridge_domain_adapters_v1.py` · `scripts/build_mkm_ops_memory_domain_adapters_overlay_v1.py` · `scripts/Run-DomainAdapterPointerSmoke_v1.ps1` · `scripts/apply_mkmlife_pixel_language_local_sprite_paths_v1.py` · design lane pack `prism_ops_pixel_battalion_gate`·`prism_ops_lens_audio_gate` · `Invoke-MkmOpsMemoryIndexRoutine_v1.ps1 -Lane design`. **Logos external B2B:** `scripts/build_logos_graph_studio_external_b2b_meeting_kit_v1.py` → `reports/logos_graph_studio_external_b2b_meeting_kit_v1_latest.json`. **서피스 태그: 운영 SSOT (CI 미포함).** Track A·SEND 자동 합선 없음.
