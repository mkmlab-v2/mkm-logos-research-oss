...
| 통찰 캡 임계치 드리프트 경보 | `scripts/alert_aramaic_insight_cap_threshold_drift_v1.py` → `docs/final/artifacts/aramaic_insight_cap_bucket_threshold_drift_alert_latest.json` | 입력 `reports/ops/aramaic_insight_cap_bucket_threshold_history.jsonl`( **`--history-jsonl`** 로 오버라이드), 산출 **`--output-json`** (기본값은 위 경로). CI·회귀는 `tests/test_alert_aramaic_insight_cap_threshold_drift_v1.py`가 tmp만 사용 |
