...
| 환자 제공용 통합 번들 v1 (SOAP + 렌즈 슬롯) | `docs/final/schemas/patient_care_bundle_v1.schema.json` | `clinical_soap_v1`와 `patient_slots`(core·sasang·`myeongni_ref` **[HYPO]**·`logos_opt` **[NON_GATING]**) 분리; `boundary_contract` 고정; 예시 `docs/final/schemas/patient_care_bundle_v1.minimal.example.json`; SOAP 스텁 `--soap-json` 예시 `tests/fixtures/patient_care_bundle_soap_stub_v1.example.json`; 슬롯 Markdown 템플릿 `docs/final/artifacts/patient_care_bundle_slot_templates_ko_v1.json`·생성 정책 `docs/final/artifacts/patient_care_bundle_generation_policy_v1.default.json`·적용 `scripts/apply_patient_care_bundle_slot_templates_v1.py`·정책 검증 `scripts/validate_patient_care_bundle_against_policy_v1.py`·단일 MD 렌더 `scripts/render_patient_care_bundle_markdown_v1.py`; 명리 리포트 연결 `scripts/assemble_patient_care_bundle_with_myeongni_v1.py`(선택 `--cds-envelope-json`·`--validate`); CDS 선검증 후 조립 `scripts/build_patient_care_bundle_from_km_cds_chain_v1.py`; 회귀 `tests/test_patient_care_bundle_v1_schema.py`·`tests/test_assemble_patient_care_bundle_with_myeongni_v1.py`·`tests/test_build_patient_care_bundle_from_km_cds_chain_v1.py`·`tests/test_patient_care_bundle_templates_policy_render_v1.py`; 명리·Logos는 임상 게이팅·임신 예후 예측에 사용 금지 |
