...
**보강 (2026-05-13 — Track A Phase2 산출 재생성기 경로 드리프트 Fact-Lock):** `AGENTS.md`·`P0_COMMERCIALIZATION_TRACKER.md`·`MKM_PROMOTION_GATE_CHECKLIST_B_TO_A_C_V1.md`·CI `dual-regime-integrity.yml` PR path 필터에 기재된 **`scripts/run_track_a_conversational_cost_simulation.py`**, **`scripts/run_track_a_shadow_corpus_eval.py`**, **`scripts/run_track_a_metering_summary.py`**, **`scripts/run_track_a_metering_weekly_report.py`**, **`scripts/check_track_a_metering_band_gate.py`**, **`scripts/build_track_a_signal_light_report.py`**, **`scripts/run_track_a_commercialization_daily_chain.ps1`** 등은 **현재 워크스페이스 디스크에 없음**(레포 역사에도 미등장; 재생성 루프는 구현·복구 전까지 **불가**). `docs/final/artifacts/track_a_*_latest.json` 류는 `.gitignore`로 로컬 재생성물이며, 파일이 있어도 **`ts_utc` 등 산출 메타와 생산기 존재를 함께 확인**해야 함 — `scripts/build_mkm_promotion_gate_evidence_bundle_v1.py`가 G4~G6에 `producer_present`·`status`(pass / optional_missing / stale_snapshot / producer_gap)를 기록해 **경로만으로 “방금 재실행됨”을 단정하지 않도록** 고정. G12 휴먼 게이트·압축 §9 G2 정책 경계는 불변.
