...
**보강 (2026-05-05 — 도메인×쇼룸 배치):** §1.1.3 표 — `docs/final/MKM_DOMAIN_PORTFOLIO_POINTER_V1.md` **§1.1**(jemaai.cloud·mkmlife.com·jema-ai.com 등 **어느 호스트에 어떤 쇼룸·체험 UI**를 둘지·디자인 톤); P0 `scripts/verify_p0_constitution_gate_paths.ps1`.
...
| 도메인×쇼룸·체험 표면 배치·디자인 의도 | `docs/final/MKM_DOMAIN_PORTFOLIO_POINTER_V1.md` §1.1 |
...
| Showroom 정적 배포(로컬/스테이징 복사) | `projects/bitcoin-trading/ops/windows-rehearsal/deploy_showroom_static.ps1` | `-WebRoot` 또는 `JEMAAI_WEB_ROOT`(Process/User); 미설정 시 기본 `projects/bitcoin-trading/ops/windows-rehearsal/.showroom_staging/`(gitignore)에 `public_showroom_poll.html`·`showroom_public_bundle_v1.json` 복사. 본선 nginx는 수동. `-NoDefaultStaging`이면 미설정 시 SKIP. |
