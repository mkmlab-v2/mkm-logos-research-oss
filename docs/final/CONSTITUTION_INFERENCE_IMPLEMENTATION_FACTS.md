...
| MKM Inter-Agent Encoding status (RQ-019 M1–M3) | `scripts/build_mkm_inter_agent_encoding_status_v1.py` → `docs/final/artifacts/mkm_inter_agent_encoding_status_latest.json` | M1 v2 packet-only expand(pytest+in-process) · M2 `mkm_inter_agent_wire_profile_v0.json` · M3 L1 spike 대외 톤 초안. `rq_019_milestones_core_ready`는 JSON 필드로만 판정; **RQ-019 OPEN**·Lingua Franca·상용 SLA 미단정. 맵: `mkm_inter_agent_encoding_sota_map_v1.md`. 회귀: `tests/test_build_mkm_inter_agent_encoding_status_v1.py` |
...
| MKM Inter-Agent smoke (원클릭·CI) | `scripts/Invoke-MkmInterAgentEncodingSmoke_v1.ps1` · CI `.github/workflows/inter-agent-encoding-smoke.yml` | pytest 3종 + status `--strict-exit` + worked example emit. **live HTTP 없음**(로컬 curl은 `mkm_inter_agent_first_message_live_http_v1.json`). IR 복붙: `mkm_inter_agent_ir_snippet_v1.md` |
