...
| 의미 통찰 후보 추출기 | `scripts/extract_bible_meaning_insight_candidates_v1.py` → `docs/final/artifacts/bible_meaning_insight_candidates_latest.json` | 허브 구절·레짐 반복 군집·top-k 추론 경로 후보를 자동 추출; CLI 스모크 `tests/test_extract_bible_meaning_insight_candidates_v1.py` |
...
| 통찰 생존 평가 | `scripts/build_insight_survivor_eval_v1.py` → `docs/final/artifacts/insight_survivor_eval_latest.json` | raw 통찰 후보를 `drawdown_avoidance / false_positive_cost / walkforward_repro` 지표로 평가; CLI 스모크 `tests/test_build_insight_survivor_eval_v1.py` |
| 통찰 생존 선발 | `scripts/select_insight_survivor_candidates_v1.py` → `docs/final/artifacts/insight_survivor_candidates_latest.json` | 평가 결과 중 임계치 통과 후보만 survivor로 선발(상위 N 제한); CLI 스모크 `tests/test_select_insight_survivor_candidates_v1.py` |
...
| 통찰 캡 버킷 임계치 스윕 | `scripts/sweep_aramaic_insight_cap_bucket_thresholds_v1.py` → `docs/final/artifacts/aramaic_insight_cap_bucket_threshold_sweep_latest.json` | audit log 기반으로 `mid/high` 임계치와 low/mid/high cap 조합을 스윕해 추천 후보 산출; CLI 스모크 `tests/test_sweep_aramaic_insight_cap_bucket_thresholds_v1.py` |
| 통찰 캡 버킷 임계치 적용 | `scripts/apply_aramaic_insight_cap_bucket_threshold_recommendation_v1.py` → `docs/final/artifacts/aramaic_insight_cap_bucket_threshold_recommended_latest.json` | 스윕 best 후보를 점수/섀도우 실행 인자로 승격; CLI 스모크 `tests/test_apply_aramaic_insight_cap_bucket_threshold_recommendation_v1.py` |
| 통찰 캡 임계치 히스토리 | `scripts/report_aramaic_insight_cap_threshold_history_v1.py` → `reports/ops/aramaic_insight_cap_bucket_threshold_history.jsonl` | 추천 임계치 스냅샷을 실행 이력으로 누적; `--history-jsonl`로 격리 경로 가능; CLI 스모크 `tests/test_report_aramaic_insight_cap_threshold_history_v1.py` |
| 통찰 캡 임계치 드리프트 경보 | `scripts/alert_aramaic_insight_cap_threshold_drift_v1.py` → `docs/final/artifacts/aramaic_insight_cap_bucket_threshold_drift_alert_latest.json` | `--history-jsonl`·`--output-json`(상대=레포 루트)·`--drift-threshold`(기본 `1e-4`). 히스토리에서 `schema=aramaic_insight_cap_bucket_threshold_history_row_v1`인 `recommended` 스냅샷만 순서대로 수집해 **마지막 2개**를 비교; `reason` ∈ `insufficient_history` \| `stable_below_threshold` \| `threshold_drift`. 산출에 `max_abs_delta`·`drifted_keys`·`deltas`·`history_recommended_snapshots`. 회귀 `tests/test_alert_aramaic_insight_cap_threshold_drift_v1.py`(tmp 격리) |
...
| 임계값 주간 스케줄 등록·readiness | _(레포에 `register_aramaic_mvp_threshold_weekly_task.ps1` / `verify_aramaic_mvp_threshold_weekly_task_readiness.ps1` 없음 — Task Scheduler 수동 등록)_ | 주간 Action 예: 스윕 → 적용 → (선택) 추세 경보. 태스크 확인은 로컬 `Get-ScheduledTask` |
...
| Aramaic B-track graph pipeline (CI = Fact-Lock 번들 3d2b, pytest 23종) | CI `.github/workflows/dual-regime-integrity.yml` `Aramaic B-track graph pipeline smoke` 단계와 동일 목록을 `scripts/run_fact_lock_bundle.ps1` 기본에 포함; 생략 `-SkipAramaicBtrackGraphPipelineSmoke` |
