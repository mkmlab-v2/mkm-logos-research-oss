...
| Track C showroom 번들 체인 (권장 원클릭) | `scripts/build_showroom_track_c_bundle_chain_v1.ps1` | (1) freshness sidecar → (2) `build_showroom_topology_radar_snapshot_v1.py` (`docs/final/artifacts/showroom_topology_radar_snapshot_v1_latest.json`; 기본 `--allow-stub-ref`, 엄격 `-TopologyRadarSnapshotStrict`) → (3) `build_showroom_display_bundle.ps1` → (4) `validate_showroom_public_bundle.py`. 선택 `-SkipFreshnessSidecar`·`-SkipTopologyRadarSnapshot`·`-SkipValidate`. 스펙 §4.2·§4.4·`JEMAAI_CLOUD_PUBLIC_SHOWROOM_SPEC.md`. |
...
| 쇼룸 체인 로컬 검증 | `projects/bitcoin-trading/ops/windows-rehearsal/verify_showroom_bundle_chain.ps1` | 빌드 시 **`scripts/build_showroom_track_c_bundle_chain_v1.ps1`**(freshness·topology snapshot emit·번들·검증 포함; 옵션 `-SkipBuild`). 이후 visual/contract/lint·`pytest`; `-WithStagingCopy` 시 `.showroom_staging/` 복사 테스트. CI: `.github/workflows/showroom-bundle-validate.yml`. |
...
| Fact-Safe 리스크 주기 갱신(로컬, 주문 없음) | `scripts/Run-FactSafeRiskProfileSyncChain_v1.ps1` → `scripts/Register-FactSafeRiskProfileSyncTask.ps1` | 기본: `sync_fact_safe_risk_profile.py --repo-source` → `run_conditional_action_gate_v1.py --backend api --dry-run --skip-human-approval --skip-frame-payload` → `build_trading_go_nogo_status_v1.py`. 레거시 n8n 태그: 러너 `-UseN8nSource` 또는 등록 `Register-FactSafeRiskProfileSyncTask.ps1 -UseN8nSourceInTask`. 기본 예약 작업명 `\MKM-FactSafe-RiskProfile-Sync-4H`(기본 **4시간**마다; `-IntervalHours`/`-Remove`). `MKM_WORKSPACE_MAINTENANCE` 시 sync는 스킵될 수 있음. **n8n 축소·로컬 SSOT 이관 원칙**은 아래 **§13.1.b** 참조. |
...
1. **현행 기본(2026-05-15 이후):** Fact-Safe 체인은 `sync_fact_safe_risk_profile.py --repo-source`로 `risk_profile`에 `repo.fact_safe_sync.v1` / `repo_shadow`를 기록한다. 레거시 `n8n.regime_watch.v5` / `n8n_shadow`는 러너 `Run-FactSafeRiskProfileSyncChain_v1.ps1 -UseN8nSource` 또는 동 스크립트의 `--n8n-source`로만 유지한다.
...
3. **`n8n.*` → 비 n8n `source` 이전**: 기존 프로필이 `n8n.*` 태그일 때 메타데이터를 덮어쓰려면 `sync_fact_safe_risk_profile.py`의 **`--allow-n8n-metadata-downgrade`**와 `tests/test_sync_fact_safe_risk_profile.py` 회귀로 **의도적 이전**만 허용한다(실수로 소스 라벨만 바뀌는 것 방지).
