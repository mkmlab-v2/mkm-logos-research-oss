...
| 거버넌스 입력 | `docs/final/artifacts/integrated_governance_v1_latest.json` | `schema: integrated_governance_v1`. 빌더: `scripts/build_integrated_governance_v1.py`. **HOLD**(`final_regime`·`final_action_allowed`)는 `_is_hold()`로 판정. **보강 (2026-05-13):** 빌더가 선택적으로 `lens_music_hormone_trend_latest.json`·`reports/lens_music_symbolic_audio_promotion_gate_latest.json`를 읽어 **`lens_music_m31_operational_digest_v1`**(비생물학적 메타포·**3엔진 합성 판정과 무관**)를 같은 JSON에 첨부 — Track C 대시보드·ECC 입력과 **합선 없음**. |
