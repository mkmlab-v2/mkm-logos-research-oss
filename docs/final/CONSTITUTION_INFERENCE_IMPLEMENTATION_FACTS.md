...
| Logos 벡터 ANN 라이트 빌드 v1 (해시 스텁·SQLite) | `scripts/build_logos_vector_index_ann_lite_v1.py` → `docs/final/artifacts/logos_vector_index_ann_lite_v1_latest.json` · `logos_vector_index_ann_lite_v1.sqlite`; 공유 스텁 `scripts/logos_vector_hash_stub_v1.py`; Top-K 점검(브루트포스·비의미) `scripts/query_logos_vector_index_ann_lite_v1.py` | 계약 `LOGOS_VECTOR_INDEX_ANN_LITE_BUILD_REPORT_V1_CONTRACT.json`; 스키마 `logos_vector_index_ann_lite_build_report_v1`·`logos_vector_ann_lite_query_result_v1`; **`embedding_mode=hash_stub_v1`**; 기본 구절 원천=매니페스트 `input_path`·`--max-verses` 기본 500; 회귀 `tests/test_logos_vector_index_ann_lite_v1.py`; 실제 NL 임베딩은 `policy.status=active` 및 별도 임베더 연동 후 |
