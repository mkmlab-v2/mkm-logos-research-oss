...
| 내부 시맨틱+RAG 번역 브리지 번들 v1 (B-track·lab) | `scripts/build_semantic_rag_bridge_insight_bundle_v1.py` → 기본 `docs/final/artifacts/semantic_rag_bridge_insight_bundle_v1_latest.json`; 스키마·예시 `docs/final/schemas/semantic_rag_bridge_insight_bundle_v1.schema.json` 동명; 설계 매핑 `docs/final/INTERNAL_SEMANTIC_RAG_4D_ARCH_OUTLINE_V1.md` §4.1 | 입력 `--rag-json`·`--slots-json`·`--calibration-kind`·`--calibration-artifact`; **`--philosophy-pilot-json`** 시 `philosophy_lane_rag_pilot_v1`의 `blocks[]`를 `rag_evidence`로 병합(총 24건 cap); `jsonschema` 설치 시 `--strict`로 스키마 실패 exit 2; Premium multi-lens 리포트 hits 자동 변환은 미구현·호출측 정규화; 회귀 `tests/test_semantic_rag_bridge_insight_bundle_schema_v1.py`·`tests/test_build_semantic_rag_bridge_insight_bundle_v1.py`; P0 `verify_p0_constitution_gate_paths.ps1`; A-track·실매매 트리거 금지 |
...
| Logos insight bundle v1 (계약·minimal) | `docs/final/schemas/logos_insight_bundle_v1.schema.json` · `docs/final/schemas/logos_insight_bundle_v1.minimal.example.json` · `tests/test_logos_insight_bundle_schema_v1.py` · **`scripts/build_logos_insight_bundle_v1.py`** → `docs/final/artifacts/logos_insight_bundle_v1_latest.json`(기본) · `tests/test_build_logos_insight_bundle_v1.py` | 업스트림 집계·중립 `tension_axis_id`·`[HYPO]`·NON_GATING; 교리 라벨 금지 |
