...
| 통찰 캡 임계치 드리프트 경보 | `scripts/alert_aramaic_insight_cap_threshold_drift_v1.py` → `docs/final/artifacts/aramaic_insight_cap_bucket_threshold_drift_alert_latest.json` | 입력 `--history-jsonl`(기본 `reports/ops/aramaic_insight_cap_bucket_threshold_history.jsonl`), 출력 `--output-json`(기본 위 산출 경로); 상대 경로는 레포 루트 기준. 회귀는 `tmp_path`로 격리 |
