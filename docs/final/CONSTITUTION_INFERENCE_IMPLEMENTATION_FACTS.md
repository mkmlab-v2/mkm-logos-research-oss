...
| 통찰 캡 임계치 드리프트 경보 | `scripts/alert_aramaic_insight_cap_threshold_drift_v1.py` → `docs/final/artifacts/aramaic_insight_cap_bucket_threshold_drift_alert_latest.json` | 최근 추천값 변화량이 임계치를 넘으면 경보 산출(기본 산출 경로 고정); 회귀 `tests/test_alert_aramaic_insight_cap_threshold_drift_v1.py`는 기존 파일 없을 때 생성 산출물 정리 |
