...
**보강 (2026-05-13 — Visualization v0: STT audit JSONL + trust read-only Track C slice):** `scripts/append_stt_routing_audit_log_v1.py`가 `docs/final/schemas/stt_routing_audit_log_v1.schema.json` 검증 후 JSONL append; `scripts/summarize_stt_routing_audit_log_v1.py`가 `reports/stt_routing_audit_log_v1_summary_latest.json` 롤업. `scripts/build_mkm_trackc_ops_dashboard_v1.py`가 `docs/final/schemas/trust_visualization_panel_v0.example.json`에서 `trackc.trust_visualization_v0`(요약·Logos `non_gating`), STT 요약 또는 NODATA 시 `trackc.stt_routing_audit_log_slice` 노출; `evidence`에 경로 포인터. 회귀 `tests/test_append_stt_routing_audit_log_v1.py`, `tests/test_build_mkm_trackc_ops_dashboard_v1.py`; `verify_p0` 필수 목록·CI `dual-regime-integrity.yml` PR paths 포함. B-track·읽기 전용; A-track 실매매 자동 합선 없음.
