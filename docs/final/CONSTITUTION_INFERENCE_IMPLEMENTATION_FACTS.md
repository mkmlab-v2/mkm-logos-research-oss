...
**보강 (2026-05-13 — Pack 0-B golden→SFT→LoRA 학습 경로):** `scripts/convert_myeongri_golden_to_sft_instruction_jsonl_v1.py` → 로컬 `data/training/myeongri_deterministic_lora_sft_train_v1.jsonl`(비추적) → `scripts/train_mkm_prophecy_lora_windows_fallback_v1.py`; 어댑터 출력 디렉터리는 `docs/final/artifacts/myeongri_deterministic_lora_model_profiles_v1.json`의 `adapter_repo_relative` 하위(가중치 비추적). 회귀 `tests/test_convert_myeongri_golden_to_sft_instruction_jsonl_v1.py`.
