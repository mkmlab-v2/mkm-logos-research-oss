...
| 조건부 시그널 게이트 → USDM 단발(API) | `projects/bitcoin-trading/scripts/run_conditional_action_gate_v1.py` | Fact-Safe `risk_profile_fact_safe_latest.json` 등 평가 후 **`--backend api`** → `projects/bitcoin-trading/scripts/execute_binance_usdm_single_order_v1.py`(기본 **테스트넷**; 실제 체결은 게이트 통과 시에도 **`--pass-live`** 명시 시만). 메인넷은 실행기 **`--mainnet`**(이중 확인 권고). 웹훅 레거시: `run_conditional_signal_webhook_v1.py`. **파일럿(루트):** `scripts/Run-BinanceUsdmPilotSmoke.ps1`(dry 기본; 테스트넷 `-LiveTestnet -AcknowledgeLiveTestnet -RiskJson`; **소액 메인넷** `-LiveMainnetSmall -AcknowledgeLiveMainnetSmall -AcknowledgeIrreversibleLoss -RiskJson`(픽스처 금지)·`-MaxMainnetQty`/env `MKM_PILOT_MAINNET_MAX_QTY`). **게이트 dry CLI 회귀:** `tests/test_run_conditional_action_gate_v1_cli_smoke.py`·픽스처 `tests/fixtures/risk_profile_fact_safe_gate_pass_minimal_v1.json`. 체크리스트 HTML: `docs/binance_usdm_signal_webhook_setup_checklist_v1.html`. |
