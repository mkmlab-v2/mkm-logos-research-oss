...
1. **현행 기본**: 위 표의 Fact-Safe 체인은 `sync_fact_safe_risk_profile.py --n8n-source`로 `risk_profile`에 `n8n.regime_watch.v5` / `n8n_shadow` 메타를 기록할 수 있다.
...
4. **헌법 게이트 allowlist**: `verify_constitution_gates.ps1`는 `constitution_gates_v1.json`의 `allowed_risk_combinations`에 대해 `risk_profile_fact_safe_latest.json`의 `source`/`mode`를 검사한다. **새 조합**을 쓰려면 **반드시** 해당 JSON을 먼저 갱신하지 않으면 Phase 1 헌법 게이트가 실패한다.
