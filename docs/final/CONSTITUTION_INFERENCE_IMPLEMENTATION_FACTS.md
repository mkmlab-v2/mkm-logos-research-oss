...
| Aramaic B-track graph pipeline (CI = Fact-Lock 번들 3d2b, pytest 24종) | CI `.github/workflows/dual-regime-integrity.yml` `Aramaic B-track graph pipeline smoke` 단계와 동일 목록을 `scripts/run_fact_lock_bundle.ps1` 기본에 포함; 생략 `-SkipAramaicBtrackGraphPipelineSmoke` |
...
| 통찰 캡 임계치 드리프트 경보 JSON Schema 계약 | `tests/test_aramaic_insight_cap_threshold_drift_alert_schema_v1.py` |
