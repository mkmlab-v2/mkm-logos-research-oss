...
**연대기·H: 병행 근거(비-Git 아카이브 포함):** `docs/final/COMPRESSION_RESTORATION_EVOLUTION_INDEX_V1.md`.
...
- **POST /v1/research/l1_side_channel/wire/secure** (OpenAPI **v1.2.0+**, `encodeL1SideChannelSecureWire`): **research lane only** — same adaptive wire payload, then wraps bytes into `mkm_secure_payload_envelope_v1` via AES-256-GCM (`scripts/core/secure_payload_envelope_v1.py`, provider in `compression_token_api_stub.py`). A/B key bulkhead is environment-scoped (`MKM_ENVELOPE_A_TRACK_KEY_B64`, `MKM_ENVELOPE_B_TRACK_KEY_B64`); response carries `integrity_flags.secure_envelope=true` and track/codec metadata. This route is **not** Track A commercial SLA and must not be treated as v2 Trust Packet cutover.
- **§9.2 draft HTTP bench / VPS 생산 RTT 후보:** 로컬 집계 SSOT `docs/final/artifacts/bench_l1_api_load_summary_latest.json`; 본선에서 동일 스크립트로 측정한 후보는 **`bench_l1_api_load_summary_vps_latest.json`** (혼동 금지). 런 JSON에는 `research_only`·`draft_benchmark`가 포함될 수 있어 **운영 SLA 최종 확정**과 구분. 절차·팩트체크 정렬: `docs/final/BENCH_L1_API_LOAD_VPS_RUNBOOK.md`.
- **L1 inverse decoder OOV/literal follow-up (legacy, 2026-04-10):** `docs/final/artifacts/l1_inverse_decoder_spike_test_summary_oov_literal_latest.json` 기준 `avg_exact_restore_rate=0.5777777778`, `determinism_delta=0.0444444444`; mode breakdown `docs/final/artifacts/l1_inverse_decoder_noise_mode_breakdown_oov_literal_latest.json`에서 `oov` 평균 exact=1.0, hardest mode=`swap_typo`.
- **Waiting-queue state/net-source stabilization (2026-04-10, Windows monthly check):** `run_waiting_queue_monthly_check.ps1 -SkipBtrackGates -SkipNetSourceFallbackAutoHold` 실행 로그에서 `dual_regime_state_advisory=state_clamp_stable`, Slack 라인 `NET_SOURCE_PRIMARY`, `net_source_fallback_streak=0`, 최종 `Completed successfully` 확인(기존 `state_signal_not_wired`/fallback throw 오탐 경로와 분리).
