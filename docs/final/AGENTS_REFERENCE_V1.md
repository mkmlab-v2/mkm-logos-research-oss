# AGENTS_REFERENCE — 워크스페이스 에이전트 상세 참조 (비주입 · on-demand)

**역할**: 루트 `AGENTS.md` **주입층(slim)** 의 확장본이다. 매 턴 자동 주입되지 않음 — 레인·페르소나·도메인 작업 시 `@docs/final/AGENTS_REFERENCE_V1.md` 또는 Read.

**주입층 SSOT**: 루트 `AGENTS.md` · 규칙 다이어트: `py scripts/check_cursor_rules_context_diet_v1.py --strict`

---

# AGENTS — 워크스페이스 에이전트 SSOT (레거시 제목 · 본문 참조용)

**역할**: 상세 규칙·페르소나 표·도메인 핸드오프 전문. 일상 채팅 시작은 **slim `AGENTS.md`** 만으로 충분할 때가 많다.
...
- **Aramaic MVP 일일·Track T survivor health POST 생략 (Windows):** `scripts/Register-AramaicMvpDailyTask.ps1 -SurvivorHealthAlertDryRun`으로 재등록하거나, `scripts/run_aramaic_raw_oos_audit_accumulator_v1.ps1 -SurvivorHealthAlertDryRun`; 체인만 직접이면 `scripts/run_aramaic_mvp_chain_v1.ps1 -SurvivorHealthAlertDryRun` 또는 환경 **`MKM_ARAMAIC_SURVIVOR_HEALTH_ALERT_DRY_RUN`**=`1`|`true`|`yes`(루트 `.env.example` 주석). 예약 인자 확인: `scripts/Verify-AramaicMvpDailyTaskReadiness.ps1` → `survivor_health_alert_dry_run_switch_in_task_action`. 표·경로: `docs/final/CONSTITUTION_INFERENCE_IMPLEMENTATION_FACTS.md` Aramaic 절.
...
- **다중 채팅 작업 일기(Ops slice):** 하루 **한 파일**에 Thread 1–10 섹션으로 요약을 쌓는다. 기본 경로 `reports/daily_thread_work_YYYY-MM-DD.md`(통상 `reports/**` 비추적). 동기화: `py scripts/athena_daily_thread_log_sync_v1.py --thread N --bullet "…"`(반복). 옵시디언 등 **전체 경로**는 User 환경 `MKM_DAILY_THREAD_LOG_PATH`(파일 또는 디렉터리). 템플릿 `docs/final/templates/DAILY_THREAD_WORK_LOG.template.md` · 에이전트 규칙 `@.cursor/rules/daily-thread-diary-sync.mdc`(또는 Rules에서 해당 규칙 활성화). 트리거 예: 「오늘 일기 동기화」「일기 반영」. **임무·MISSION·Phase 표**는 루트 **`MISSION_LOG.md`** + **`MISSION_LOG.template.md`** 「채팅창 작업일정 앵커」가 SSOT(일기 파일로 대체하지 않음). 장기 정책·정체성 한 줄은 여전히 `CENTRAL`·체크포인트가 SSOT.
...
### Cursor User Rules — minimal v1 (권장 · 컨텍스트 다이어트)

**일상:** `docs/final/artifacts/cursor_user_rules_minimal_v1.txt` 를 User 탭에 복붙. slim `AGENTS.md`·`.cursorrules`와 **중복 붙이지 않음**.

### Recommended Cursor User Rules — User 탭 합본 (장문 · 핸드오프 예외 시만)

Cursor **Settings → Rules for AI**(**User** 탭)에 **아래 코드 펜스** — minimal v1과 **동시 사용 금지**. 핸드오프·일기 분기가 필요할 때만 합본으로 교체.
...
세션 시작(권장)
- 작업이 크거나 방향을 맞출 때는 docs/final/CENTRAL_AGENT_MEMORY_V1.md 를 읽고 진행 단계·Fact-Lock을 짧게 맞춘 뒤 작업한다. 다단계 DoD·**MISSION·Phase 표**는 루트 `MISSION_LOG.md`(템플릿 `MISSION_LOG.template.md` §채팅창 작업일정 앵커)를 유지한다.
...
- **Cursor:** `.cursor/rules/gut-brain-metaphor-agent-v1.mdc` (`alwaysApply: false` · Track C 시 `@`); Track C 카피 초안 `.cursor/skills/mkm-gut-brain-trackc-comms/SKILL.md`.
...
2. **`.cursor/rules/central-agent-memory.mdc`** (코어 alwaysApply) — 정체성·재개·Fact-Lock 지문. 마무리 **[A]/[B] 강요 금지**는 루트 `.cursorrules` TITAN과 동일. 레거시 요약만: `.cursor/rules/sovereign-central-command.mdc` (`alwaysApply: false` · `@` 로드).
...
| 【쇼룸 publish】 | Track C 번들·검증·API ingest·VPS sync·듀얼 호스트 스모크 | `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\Invoke-ShowroomTrackCPublishRoutine_v1.ps1` |
...
| **논문 디스크 4칸** · paper verdict · LIT_REVIEW 대조 | Disk verdict(Tier A–D) · Reproduce · raw/repair_v2 · Promotion — artifact/exit 0 only; support-fire; no Track A·GTM | `@.cursor/rules/mkm-paper-disk-verdict-four-slot-v1.mdc` · 예시 `PROPHECY_EDGE_*` · `MKM_DUAL_PLANE_*_V0_2` |
| 【Bounded lane shadow】 | External Cortex bounded loop: P0 + pytest 3종 + dry-run invoke (shadow only) | `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\Invoke-MkmPersonaHealth_v1.ps1 -Persona BoundedLaneLoopShadow` |
...
| 【Design 레인】 | 허브·no1kmedi UI: shell gate + ui shell contract + marketing-copy + design-tokens (배포·VPS 없음) | `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\Invoke-MkmPersonaHealth_v1.ps1 -Persona DesignLane` |
| 【Design 레인 결선】 | 멀티도메인 라이브 스모크·P0 포함 (주간·머지 전) | `powershell … -File scripts\Invoke-MkmPersonaHealth_v1.ps1 -Persona MkmDomainDesignClosure` |

### Design 레인 자동 진입 (12AI · 페르소나 라우팅)

**트리거(동등):** 「Design 레인」「쇼룸 레인」「Design/Showroom」·`@MISSION_LOG` **Design 행**·편집 경로가 `projects/no1kmedi/src/app/hub/**` · `projects/no1kmedi/src/components/shell/Universe*` · `projects/no1kmedi/marketing-site/public-copy.json` · jemaai 쇼룸 정적(Track C)일 때 — **다른 답변·코딩보다 먼저** 아래를 따른다. **4AI 코어 ≠ Design 레인**(명명 혼용 금지). 상세 규칙: `.cursor/rules/design-lane.mdc` · 스킬 `.cursor/skills/mkm-design-lane/SKILL.md`.

| 단계 | SSOT (선독 3+1) | 검증 (종료 전 · exit 0) |
|------|-----------------|-------------------------|
| 1 | `MISSION_LOG.md` — **Design/Showroom 행** 다음 1타만 | — |
| 2 | `docs/final/MKM_DOMAIN_PORTFOLIO_POINTER_V1.md` **§1.1·§1.1b** | — |
| 3 | `projects/no1kmedi/marketing-site/public-copy.json` (카피·`hub_links`) | `projects/no1kmedi`에서 `npm run check:marketing-copy` |
| 4 | 허브 편집 시 `docs/final/artifacts/mkm_universe_hub_shell_contract_v2_draft.json` | `py scripts/check_mkm_universe_hub_shell_v2.py` |
| 5 | `docs/final/PUBLIC_FACING_SECURITY_AND_IP_COPY_CHECKLIST_V1.md` v1.7 (과장·%·Track A KPI 금지) | `npm run check:design-tokens` (no1kmedi) |

**원클릭(일상):** 【Design 레인】→ `Invoke-MkmPersonaHealth_v1.ps1 -Persona DesignLane` (내부 `Invoke-MkmDesignLaneRoutine_v1.ps1`). **배포·nginx·`sync_showroom_to_vps`** 는 별 턴(【쇼룸 publish】). **완료** = 게이트 exit 0 + `MISSION_LOG` Design 행 다음 1타만 갱신.
...
- **좁은 암행어사 한 바퀴(SafeOps·MCP·비밀·no1kmedi API 등):** `scripts/Invoke-AmsaengEosaGovernanceCycle.ps1` — 범위는 `docs/final/artifacts/amsaeng_eosa_governance_scope_v1.json`. SafeOps 기본은 `trading_go_no_go_latest.json`이 Trinity **LOCKED**로만 `NO_GO`일 때 verify를 실패로 보지 않음(정책 자세); 감사 시 `Invoke-AmsaengEosaGovernanceCycle.ps1 -StrictTradingGoNoGo` 또는 `Invoke-SafeOpsSurfaceCheck.ps1 -StrictTradingGoNoGo`.
...
- **DR bench mini v1(선택, B-track):** `pwsh scripts/run_workspace_automation_health.ps1 -IncludeDeepResearchBenchSmoke` — offline pytest 2종 + harness `--require-entry-level` + ops paste. 빠른 단축: `-DeepResearchBenchSmokeOnly`. Fact-Lock: `run_fact_lock_bundle.ps1 -IncludeDeepResearchBenchSmoke`. online strict: `Invoke-MkmDeepResearchBenchWeekly_v1.ps1`. CONSTITUTION DR bench mini 표.
...
- **환자 통합 번들 v1 (SOAP + MKM 렌즈 슬롯, Fact-Lock):** 스키마 `docs/final/schemas/patient_care_bundle_v1.schema.json` · §9 표 · 원클릭 `scripts/Invoke-PatientCareBundleAssemblePatientFacing_v1.ps1`(템플릿+정책+MD 고정; 선택 `-PolicyJson`·`-DryRun`) · `scripts/assemble_patient_care_bundle_with_myeongni_v1.py`(`--apply-slot-templates`·`--validate-policy`·`--render-md-out` 선택) · CDS 선행 시 `scripts/build_patient_care_bundle_from_km_cds_chain_v1.py` · 슬롯 템플릿/정책/렌더 `docs/final/artifacts/patient_care_bundle_slot_templates_ko_v1.json` 등 + `apply_*` / `validate_*` / `render_*` 스크립트 — 명리·Logos는 [HYPO]/[NON_GATING]; 임상 게이팅·임신 예후 예측 합선 금지.
...
- **심층 서술 트리거(B-track):** 지휘관이 **B-track**임을 밝히고 **분석 축**(가격 Field 관측만, 환경·일반예언 서사만, 또는 둘을 **오버레이만** 하되 **한 방정식으로 합치지 않음**)과 **원하는 서술 깊이**를 명시하면, 에이전트는 **단답으로 끝내지 않도록** 전제·근거·한계를 구조화해 우선한다. **렌즈 명칭**은 `사상`/`명리`/`성경(Logos)`만 쓰고 운영 레이어명(`Macro` 등)을 렌즈명으로 혼용하지 않는다. **지수·장 일진을 개인 출생 사주로 자동 치환하는 경로는 없음**; 시장 쪽 명리 오버레이는 문서·`market_myeongni` 절 범위만 Fact-Lock. **명리 결정론 JSON**과 날씨·가격·일반예언 **원천 데이터 자동 합선 금지**(`docs/final/CENTRAL_AGENT_MEMORY_V1.md` 「명리 렌즈 고도화 v1」).
- **채점·학습 경계(Fact-Lock):** 가격 적중 채점은 아래 **가격 적중 채점 체인**·`CONSTITUTION_INFERENCE_IMPLEMENTATION_FACTS.md` 일일 B-Track·Prophecy Hit Rate 절; 일반·환경 예측은 `GENERAL_PROPHECY_SCHEMA_V1`·`eval_general_prophecy_brier_score.py` 등 **별 축**. LoRA·골든·프로모션 게이트는 **스크립트·제품별로 분리**되어 있으므로, **채팅 가설이 곧바로 특정 LoRA 학습에 흡수된다**고 서술하지 않는다—고정은 **해당 스크립트·산출 JSON·게이트**만 SSOT.
...
- **쇼룸 정적 → VPS (워크스페이스 루트에서):** `pwsh .\scripts\sync_showroom_to_vps.ps1 -RefreshStaging` — 래퍼가 `projects/bitcoin-trading/ops/windows-rehearsal/sync_showroom_to_vps.ps1`로 위임. `sync_showroom_to_vps.ps1`만 입력하면 PATH에 없어 실패할 수 있음. scp 비밀번호 반복 완화: User 환경에 `MKM_VPS_SCP_EXTRA_ARGS`(예: `-i` 키경로); **단일 `scp`는 `deploy_showroom_static` 스테이징 8종과 동일**(토폴로지·Trust·사주 정적 포함, 스테이징에 있을 때); 업로드 후 nginx 자동 reload는 `JEMAAI_VPS_RELOAD_NGINX=1`일 때만(의도 확인 후). 상세 표: `CONSTITUTION_INFERENCE_IMPLEMENTATION_FACTS.md`(Showroom VPS 전송 행). **주간 권장(수동, Task Scheduler 생략 기본):** `docs/final/MKM_SHOWROOM_OPS_AND_PACK0B_WORK_SCHEDULE_V1.md` Phase 3 한 줄 블록.
