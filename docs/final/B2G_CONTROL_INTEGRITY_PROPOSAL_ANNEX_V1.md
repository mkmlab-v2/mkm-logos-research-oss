...
**문서 버전:** v0.1.2 · **고정일:** 2026-05-13 — §1.1 향후 확장성(의료·조건부) 단락 추가; §2 앞에 보안·리스크 절명 명시 (§8·DoD 번호 유지)
...
| 구분 | 내용 |
|------|------|
| 각주(외부) | 본 문서 **§4** URL만 제안서 각주에 실음. |
| 별첨(내부) | 레포 경로·스크립트명은 입찰 기관 정책에 따라 **별첨 SSOT 목록**으로 분리 가능. |
| 법무 | 본 문단은 **법률 자문·규제 적합성 판단을 대체하지 않음**. |

---

## 1.1 향후 확장성 (제안서 **[확장성 및 향후 비즈니스 모델]** 삽입용)

**[타 규제 도메인(의료·임상 보조)으로의 제어 아키텍처 확장 로드맵]**  
본 과제에서 설계·운영하는 ‘RAG 기반 근거 매핑 및 HITL(인간 개입) 통제 게이트’ 아키텍처는 금융·공공을 넘어 의료와 같은 고위험 규제 산업의 보조 시스템으로 확장이 가능합니다. 예를 들어 한의학 임상 보조 워크플로에 본 아키텍처를 적용할 경우, AI는 방대한 문헌 및 임상 가이드라인을 검색해 참고용 초안만 제시하며, 시스템은 이를 자동 기록하지 않고 보류(HOLD)합니다. 이후 담당 의료진(한의사)이 직접 근거를 확인하고 최종 승인(GO)해야만 EMR(전자의무기록)로 이관되도록 설계하여 의료 통제권을 엄격히 보장합니다. 단, 실제 의료 현장 도입을 위해서는 별도의 임상 유효성 검증 및 의료기기 소프트웨어(SaMD) 규제 인허가 절차가 **제품·배포 형태에 따라 해당될 경우** 필수적이므로, 당사는 이를 중장기 R&D 및 컴플라이언스 로드맵으로 편성하여 단계적으로 접근할 계획입니다.

---

## 2. 보안 및 리스크 관리 방안 (Anthropic 각주 포함 — 제안서 삽입용)

아래 한국어 단락(동일 논리의 영문은 §3)이 **제안서「통제·보안·리스크」절**에 들어갈 본문이다. 외부 인용은 **§4 각주 [1][2]**만 제안서 각주로 실고, URL 본문은 본 Annex §8 실측과 제출 직전 재확인을 따른다.
...
- [ ] 제안서 원고에 **§1.1 확장성** 단락 + **§2.1 통제·보안**(또는 §3 영문) + **§4 각주 2개** 연결
