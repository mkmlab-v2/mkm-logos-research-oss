...
| 라벨 | 의미 | 사용 기준 |
|------|------|-----------|
| `[FACT]` | 재현·추적 가능한 진술 | 절대 경로·구현 표·섹션 6 참조 파일; “코드 존재”는 호출 가능한 모듈로 한정 |
| `[HYPO]` | 개념 스케치·병치 가설 | 섹션 4 브리지·심리 국면 **설명용** 병치; 인과·예측 수식 단정 금지 |
| `[VISION]` | 미구현·로드맵 | Logos–Risk 단일 어댑터 등 **향후** 항목; 현재 동작 단정 아님 |
| `[NON-MEDICAL]` | 비의료 고지 | 본 문서는 Logos·금융 레짐 경계 정리; 의료 진단·치료 주장과 무관 |

섹션 2–3 금지·격벽 문장은 `[FACT]`(정책 고정)에 가깝고, “나란히 제시 가능”은 `[HYPO]`와 `[VISION]` 사이에서 검증 설계 후에만 `[FACT]`로 승격한다.
