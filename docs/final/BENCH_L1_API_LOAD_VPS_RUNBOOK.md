...
- **B)** 클론은 있는데 `cd`만 모를 때: `bash "$(find /opt /home /var/www /srv /root -path '*/scripts/deploy/linux/find_mkm_workspace.sh' 2>/dev/null | head -1)"` 로 루트를 stdout에 받거나, 찾은 뒤 `bash …/find_mkm_workspace.sh` 실행. 레포가 전혀 없으면 실패( D 참고).
- **C)** `run_bench_l1_api_load_vps.sh`는 **스크립트 파일이 있는 위치**로부터 레포 루트를 계산하므로, **`/root`에서도** 다음처럼 **절대 경로**로 호출하면 된다:  
  `BASE_URL=http://127.0.0.1:8010 bash /실제/클론경로/scripts/deploy/linux/run_bench_l1_api_load_vps.sh`  
  (이 경우 `cd`는 불필요.)
...
chmod +x scripts/deploy/linux/run_bench_l1_api_load_vps.sh
# 선택: export BENCH_SERVER_PID=$(pgrep -f 'uvicorn.*8010' | head -1)
BASE_URL=http://127.0.0.1:8010 bash scripts/deploy/linux/run_bench_l1_api_load_vps.sh
```

산출:

- 런 파일: `docs/final/artifacts/bench_runs/bench_l1_api_load_vps_*.json`
- 집계: `docs/final/artifacts/bench_l1_api_load_summary_vps_latest.json` (`bench_l1_api_load_summary_v2` + `bench_environment`)
...
```bash
python3 scripts/aggregate_bench_l1_api_load_runs.py \
  --input-dir docs/final/artifacts/bench_runs \
  --glob 'bench_l1_api_load_vps_*.json' \
  --bench-environment vps_same_host \
  --out docs/final/artifacts/bench_l1_api_load_summary_vps_latest.json
```
...
chmod +x scripts/deploy/linux/run_bench_l1_api_load_vps.sh scripts/deploy/linux/find_mkm_workspace.sh
...
bash scripts/deploy/linux/run_bench_l1_api_load_vps.sh
