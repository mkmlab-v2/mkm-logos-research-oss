...
- **V2 재측정 (패치 후):** `scripts/report_multilens_performance_eval.py`의 `evaluate_report`는 **strategy=A · intensity=extreme** 경로에서 샤드 `must_keep_soft_terms`를 **병합하지 않음** (`strategy == C` 이고 `intensity == high` 일 때만 병합). 따라서 **`MULTILENS_BRIDGE_POLICY_AB_OFF`와 동일 프로파일** 재실행 산출 `docs/final/artifacts/MULTILENS_V2_BRIDGE_OFF_AFTER_SHARD_PATCH_V1.json` — 평균 Jaccard **~0.735** · 절약 **~0.491** (기존 `MULTILENS_BRIDGE_POLICY_AB_OFF_V1.json`과 동일). 참고: `MULTILENS_V2_STRATEGY_C_HIGH_AFTER_SHARD_PATCH_V1.json`은 **C/high**로 병합이 켜진 별 프로파일(수치 **~0.899** Jaccard / **~0.266** 절약)이며 AB_OFF와 직접 비교하면 안 됨.
