...
## Track A Sprint Update (2026-04-15, W12-E1)

- 실행: `py scripts/run_track_a_week12_lane_weighted_router_sweep_v1.py`
- 산출물: `docs/final/artifacts/track_a_week12_lane_weighted_router_sweep_v1.json`
- 결과: `decision=HOLD_W12_E1_NO_VIABLE`, `viable_count=0`
- 핵심 수치: baseline(`saving=0.46911`, `jaccard=0.84884`, `integrity=1.0`), best run(동일: `saving=0.46911`, `jaccard=0.84884`, `integrity=1.0`)
- 게이트 판정: lane-weighted 할당에서도 `jaccard_guard_blocked_cases=30`으로 비보호 레인이 전부 차단되어 `router_off` 전환이 0건, saving floor `0.49` 및 jaccard floor `0.85` 모두 미달
- 다음 1스텝: W12-E2(`run_track_a_week12_quality_tiebreak_replay_sweep_v1.py`) 구현/실행으로 quality-first tie-break replay 검증
