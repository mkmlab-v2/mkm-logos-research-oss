...
- **last_updated_utc:** 2026-05-14T15:02:14Z
- **owner:** (선택)
...
- **2026-05-14T15:02:14Z** — 권장 번들: STT rows_total=12, verify_p0 719, pytest 24, prophecy closure_ok, run_fact_lock_bundle exit 0 (~177s); artifacts commit (trade sync log restored).
- **2026-05-14T14:55:10Z** — 진행: STT rows_total=10, verify_p0 719, pytest 24, prophecy closure_ok, run_fact_lock_bundle exit 0 (~174s); artifact refresh pending commit.
- **2026-05-14T14:51:44Z** — Prophecy closure bundle closure_ok true + Git sanity OK (post-AthenaBundle)
- **2026-05-14T14:50:42Z** — AthenaBundle Fact-Lock full pass exit 0 (137+pytest chains; premium queue skip allow-missing; safe_ops ok)
- **2026-05-14T14:41:10Z** — Fact-Lock bundle full run exit 0 (~170s): integrity_guard, dual-regime+Aramaic+STT+CDS+multilens+VA fusion+sasang-saju+integrated governance+safe_ops; artifacts refreshed.
- **2026-05-14T14:34:48Z** — Scheduled auto: STT rows_total=8, verify_p0 719, pytest 24, prophecy closure_ok true (bundle 137/3skip).
- **2026-05-14T14:32:36Z** — Scheduled auto: STT rows_total=8, verify_p0 719, pytest 24, prophecy bundle 137/3skip closure_ok true (skips).
- **2026-05-14T14:32:19Z** — Pack0-B: inference profile defaults to --profile when --inference-profile-key omitted (resolve_inference_profile_key); MKM_SHOWROOM Phase6 doc; P0 719 OK
- **2026-05-14T14:29:32Z** — Scheduled auto: STT append×2+summarize (rows_total=6), verify_p0 719, pytest 24 STT+showroom+control, prophecy closure_ok true (SkipLiveSyncPull/GoNoGo/Webhook).
- **2026-05-14T14:28:46Z** — Pack0-B Phase6: fixture 2-step train+inference-eval golden_fit_smoke profile; pytest pack0b+copy_guard 6 pass; schedule doc updated
- **2026-05-14T14:23:58Z** — Autonomous ops chain: STT append+summarize, verify_p0 719, pytest 17+19, prophecy closure closure_ok true (SkipLiveSyncPull/GoNoGo/Webhook); tracked artifact noise restored.
- **2026-05-14T14:23:07Z** — Pack0-B: bulk v1-2026-05-14 N1000 K100 + eval train+locked reports ok
- **2026-05-14T14:04:28Z** — Showroom: sync_showroom_to_vps -RefreshStaging exit 0 (scp jemaai); verify_p0 719; pytest control_integrity+showroom 19 pass
- **2026-05-16T11:00:00Z** — Stream4: `pytest` 3d3a+showroom bundle 19·예언 클로저 `closure_ok: true`(live sync·go/nogo·webhook 생략); `reports/prophecy_lane_closure_bundle_v1_latest.json`
- **2026-05-16T10:00:00Z** — 쇼룸 일정 Phase0: Control-Integrity smoke 7·`verify_p0` 719; 스테이징 추적 파일 HEAD 복원; `MKM_SHOWROOM`·`CURRENT_OPS` 갱신; 다음 push-internal·main 병합
- **2026-05-16T09:30:00Z** — Stream2: STT `append_stt_routing_audit_log_v1`×2 + `summarize_stt_routing_audit_log_v1` → `reports/stt_routing_audit_log_v1.jsonl`·`reports/stt_routing_audit_log_v1_summary_latest.json`(비추적); 실버 템플릿 §Stream 2 명령 고정; 다음 COGS 월소계·운영 JSONL
- **2026-05-14T13:49:39Z** — RQ-009: internal disclaimer pack silver_tech_legal_disclaimer_draft_v1.md; TRACK_C PUBLIC RQ pointers; P0 719 OK
- **2026-05-16T08:00:00Z** — Stream1: Visualization v0 — `TRACK_C` §3.6·`PUBLIC` §3 Fact-Lock·`[NON_GATING]` 역참조·`CURRENT_OPS` 완료 반영; 다음 Stream2 실측 append·COGS 템플릿
- **2026-05-14T10:24:36Z** — 세션 종료: 실버 COGS·STT stt_routing_audit_log_v1·RQ-009 핸드오프; 다음 오프라인 COGS·법무(D); 다른 작업으로 전환
- **2026-05-15T12:30:00Z** — Silver COGS `[HYPO]` 템플릿 압축·`CURRENT_OPS` 2026-05-15 슬라이스 단일 병합·STT `stt_routing_audit_log_v1` 스키마 포인터·스냅+CENTRAL 동기
...
| 예언(B-track) | 권장 측정: `py scripts/run_prophecy_btrack_recommended_eval_chain_v1.py` 또는 원클릭 `py … --auto-sweep-and-apply` / `scripts/Run-BtrackRecommendedEvalAutoSweep_v1.ps1`(주간 등록 `Register-BtrackRecommendedEvalAutoSweepWeeklyTask.ps1`; S4U 묶음 `Register-MkmBtrackProphecyTasksRunWhenLoggedOff_v1.ps1`). dual 게이트: **`combined_all_passed=false`**, 최근 권고 **`promotion_recommendation=soft_band_review`**(렌즈·instrument 소프트+공유 통과, 엄격 승격은 아님). **패널 24h 웹훅:** 기본은 **ALERT_1·3 실패 시만** POST(ALERT_2만 실패면 웹훅 생략·exit 1 유지); 레거시 전체 알림은 `-IncludeAlert2InWebhook`. | `reports/prophecy_btrack_recommended_eval_chain_summary_v1_latest.json` · `reports/prophecy_promotion_gates_recommended_chain_v1_latest.json` · `scripts/Check-ProphecyPanel24hAlerts.ps1` · `tests/test_check_prophecy_panel_24h_alerts_routing_v1.py` |
...
1. **예언(B-track):** 권장 체인·`reports/prophecy_promotion_gates_recommended_chain_v1_latest.json`; 패널 24h는 `Check-ProphecyPanel24hAlerts.ps1 -SkipWebhook` 스모크·`reports/prophecy_panel_24h_alerts_latest.json`; ALERT_3 구조면 dual refresh; (선택) `Register-ProphecyPanel24hAlertsTask.ps1` 도움말에 웹훅 라우팅 한 줄 보강.
2. **원격 반영:** `scripts/push-internal.ps1`(또는 워크플로); `gitea/main`은 `SoloDev-MergeFeatureToGiteaMain.ps1 -DryRun` 선행; GitHub는 `Push-GitHub-Explicit.ps1 -Acknowledge` 예외만.
