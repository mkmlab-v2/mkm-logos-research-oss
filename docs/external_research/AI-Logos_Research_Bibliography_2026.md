...
## 문서 라벨 규칙 (Fact-Safe 고정)

| 라벨 | 의미 | 사용 기준 |
|------|------|-----------|
| `[FACT]` | 외부 검증 완료 사실 | 논문/DOI/공식 데이터 페이지로 역추적 가능해야 함 |
| `[HYPO]` | 내부 가설·실험 결과 | B-track 내부 검증 단계, 대외 합의/재현 미완료 |
| `[VISION]` | 전략·로드맵·포지셔닝 | 목표 상태 기술, 현재 성능/사실로 단정 금지 |
| `[NON-MEDICAL]` | 비의료 고지 | 건강·체질 관련 문구 포함 시 의학적 효능 주장 차단 |

### Tier 1 수치 표기 템플릿 (재사용용)

Tier 1(학술 팩트)에서 수치를 쓸 때는 아래 3요소를 한 줄로 함께 적는다.

- `출처`: 논문명/DOI(or arXiv ID)
- `셋 구성`: 학습셋 N, 검증셋 N, 전문가 검토 N
- `지표`: MAE/정확도/범위 적합률(정의 포함)

예시:

- `[FACT]` 출처: arXiv:2407.12013, 셋 구성: C-14 측정 사본(학습) + 추가 사본(예측) + 전문가 검토 135, 지표: 범위 적합률 79~85%(전문가 realism 평가), MAE 약 27.9~30.7년(LOO 검증 기준).

다음 조치: `notebook_get`으로 B 노트북 소스 수·제목을 갱신하고 Confirmed/TBD 표를 1회 동기화한다(자동 수행 완료 / 잔여: NotebookLM 접근 세션 확인 필요).
