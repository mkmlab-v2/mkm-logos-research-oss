...
def _rebuild_record(root: Path, relative: str, all_paths: tuple[str, ...]) -> tuple[dict, os.stat_result]:
    ...
    record = dict(record)
    record["relations"] = [
        {"target": target, "kind": "literal_path_reference"}
...
def update_index(
    ...
    *,
    ...
    removed_count = len(set(previous_rows) - set(canonical_paths))
    ...
    document = {"schema_version": SCHEMA_VERSION, "sources": sources}
