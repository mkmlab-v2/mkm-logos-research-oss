...
- **전 세계 사용자 출생 입력 (IANA TZ, DST 안전 권장):** `scripts/saju_birth_resolver_v1.py` — 절대시각 **`birth_instant_utc`(ISO Z) + `iana_tz`** 가 1순위; 로컬 벽시계만 쓸 때는 DST 겉넘김 구간 에러 처리. CLI 예: `python scripts/run_saju_global_birth_v1.py --utc-instant 1992-03-12T17:00:00Z --iana-tz Asia/Seoul`. 계약 스키마: `docs/final/artifacts/schemas/saju_global_birth_request_v1.schema.json`.
