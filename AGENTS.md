...
- **번들 한 방**: `scripts/run_fact_lock_bundle.ps1` — 루트에서 실행; 맥락·완료 정의는 `docs/final/MULTI_LENS_INTERMEDIATE_LAYER_WORKLIST.md` 하단. 기본에 CI `dual-regime-integrity.yml`과 동일 **Aramaic B-track graph pipeline smoke**(pytest 23종, 단계 **3d2b**; 생략 `-SkipAramaicBtrackGraphPipelineSmoke`)·B-track **§3.8.4** VA→fusion→감사 체인 + `va_tag_boost_v1` 정책 골든 pytest가 포함되며, VA 생략은 `-SkipVaFusionControlIntegritySmoke`.
