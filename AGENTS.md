...
- **상징→오디오 B-track §3.9 (연구 승격 번들, 선택):** `py scripts/check_lens_music_symbolic_audio_promotion_gate_v1.py` — M0–M3 관련 pytest 번들; exit **0** 이면 `B_TRACK_RESEARCH_PROMOTION_READY`(레포 연구 준비). **Track A 상용 오디오·Track C 1차 GTM 자동 승격 아님**(`track_wall`). SSOT: `docs/final/TRACK_C_IP_BUSINESS_PLAN_2026-04-17.md` §3.9.2 · `CONSTITUTION_INFERENCE_IMPLEMENTATION_FACTS.md` 보강.
